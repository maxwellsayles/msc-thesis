...
\]
Since $b$ is unique $\bmod{~2a}$, we reduce $b \bmod{2a}$ and repeat while $\mathfrak a$ is not reduced.  Pseudo-code is given in Algorithm \ref{alg:reduce}.
...
Rather than computing the product form directly and then computing the reduced ideal representative, we observe that
\[
	\frac{b}{a} = \frac{b_2 + Ua_2/s}{(a_1/s)(a_2/s)} 
	= \frac{s^2 b_2+sUa_2}{a_1a_2}
	\approx \frac{sU}{a_1}
\]
where $U$ is given by Equation \ref{eq:idealProductU}.  We compute the continued fraction expansion of $sU/a_1 = \langle q_0, q_1, \dots, q_i, \phi_{i+1} \rangle$ using
\begin{align*}
