...
\]
where $s_i \in \{-1, 1\}$ and $a_i, b_i \in \ZZgez$.  We will be particularly interested in double-base representations for $p=2$ and $q=3$ as the principal developments of this thesis are exponentiation in the ideal class group for which we have fast squaring and cubing algorithms.

As an example of of a double-base representation, consider the number $n=23814216$ as above.  Given the bases $p=2$ and $q=3$, one representation of $n$ is
\[
	2^3 3^2 -2^{13} 3^2 +2^{15} 3^6 = 72 - 73728 + 23887872 = 23814216.
\]
Factoring common terms, we can express $n$ as
\[
	2^3 3^2 -2^{13} 3^2 +2^{15} 3^6 = 2^3 3^2 (1 + 2^{10} (-1 + 2^2 3^4)) = 23814216.
\]
The exponentiation of $g^n$ for some $g \in \mathcal G$ and $n \in \ZZ$ is then given by
\[
	g^{23814216} = g^{2^3 3^2 -2^{13} 3^2 +2^{15} 3^6} = g^{2^3 3^2 (1 + 2^{10} (-1 + 2^2 3^4))}
\]
requiring 15 squares, 6 cubes, 2 multiplications, and 1 inverse.  Recall, the binary representation of $n=23814216$ was 
\[
	23814216 = 2^3+2^6+2^{13}+2^{14}+2^{16}+2^{17}+2^{19}+2^{21}+2^{22}+2^{24}
\]
having 10 terms, while the Non-Adjacent Form  was
\[
	23814216 = 2^3+2^6-2^{13}-2^{15}-2^{18}-2^{20}-2^{23}+2^{25}
\]
having 8 terms.  The double base representation given above
\[
	23814216 = 2^3 3^2 -2^{13} 3^2 +2^{15} 3^6
\]
contains only three terms.  Note that while the Non-Adjacent Form has in expectation $\floor{\log_2 n}/3$ terms while a binary representation has in expectation $\floor{\log_2 n}/2$ terms, in both cases the number of terms are bound by $O(\log n)$.  In the case of 2,3 double-base representations, it was proved in \cite{Ciet2005} that the number of terms is bound by $O(\log n / \log \log n)$.  Therefore, even if there is no known efficient way to compute $g^3$ or $g^2$, for large enough $n$, the number of terms, and correspondingly the number of operations, will be asymptotically fewer than either a Non-Adjacent Form exponentiation or a binary exponentiation.  Naturally, the cost of a 2,3 double-base exponentiation is even less if the corresponding cost to compute $g^2$ is less than the cost to compute $g \cdot g$, or the cost to compute $g^3$ is less than the cost to compute $g^2 g$. 


\bigbreak
\subsection{Chains vs Representations}

Chain's are by definition... and can be exponentiated this way...

Representations are by definition... and can be exponentiated this way...
