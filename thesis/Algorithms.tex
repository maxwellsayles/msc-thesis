...
%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}[h]
\caption{Computes $g^n$ for immutable $n$ using only one inverse and a R2L NAF. Concept from \cite[p.4]{Joye2000}}
...
\STATE $i \gets 0$
\STATE $R \gets 1_G$ \COMMENT{product of positive exponents}
\STATE $Q \gets 1_G$ \COMMENT{product of negative exponents}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}[h]
\caption{Computes $g^n$ given $n$ as a chained 2,3 partition.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}[h]
\caption{Computes $g^n$ given $n$ in 2,3 representation. Concept from \cite[Section 3.2]{Meloni2009}.}
\label{alg:yaos}
\begin{algorithmic}[1]
\REQUIRE $g \in G,$ \\
$n = \sum_{i=1}^k s_i2^{a_i}3^{b_i},$ \\
...
\STATE $a_\Delta \gets a_1-a_2$
\STATE $R \gets \left( T_{b_1} \right)^{2^{a_\Delta}}$ \COMMENT{by squaring $a_1-a_2$ number of times}
\STATE $i \gets 2$
\WHILE {$i < k$}
	\STATE $R \gets R \cdot {T_{b_i}}^{s_i}$
	\STATE $a_\Delta \gets a_i - a_{i+1}$
	\STATE $R \gets R ^ {2^{a_\Delta}}$ \COMMENT{by squaring $a_i - a_{i+1}$ number of times}
	\STATE $i \gets i + 1$
\ENDWHILE
...
%%%%%%%%%%%%%%%%%
\begin{algorithm}[h]
\caption{Computes a 2,3 strictly chained representation from low order to high order. Adapted from \cite{Ciet2006}.}
\label{alg:r2lDbnsChain}
\begin{algorithmic}[1]
\REQUIRE $n \in \ZZgez$
\ENSURE $n=\sum_{i=1}^k s_i2^{a_i}3^{b_i}$ where \\
   $0 \le a_1 \le ... \le a_k \in \ZZ$ \\
   $0 \le b_1 \le ... \le b_k \in \ZZ$ \\
   $s_1, ..., s_k \in \{-1, 1\}$
\STATE $a \gets 0$
