...
\bigbreak
Given $\omega_0$ defined above, the module $[1, \omega_0]$ is the module of algebraic integers in the quadratic number field $\KK=\QQ(\sqrt{\Delta_0})$.  We denote this as $\OO_{\KK}$.  The order $\OO_\KK$ is of maximal size since any order $\OO_\Delta = [1,f\omega_0]$ is a subring of $\OO_\KK$. In general, by definition \ref{defn:quadraticInteger}, we can express the order $\OO_\Delta$ as the $\ZZ$-module:
\[
...
\end{equation}
where $s$ is the largest term dividing both generators of the $\ZZ$-module, such that $s$, $a$, and $b$ are unique. Note that $b$ is unique $\bmod~2a$ since
\[
