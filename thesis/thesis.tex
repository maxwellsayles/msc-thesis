...
The group of equivalence classes of binary quadratic forms is isomorphic to the ideal class group of imaginary quadratic fields, as shown by Fr{\"o}lich and Taylor \cite{Frolich1993}.  Throughout this thesis, we represent elements of the ideal class group using reduced representative ideals.  We denote the equivalence class $[\mathfrak a]$ for a reduced representative $\mathfrak a$ using the $\ZZ$\mbox{-}module $\mathfrak a = [a, (b + \sqrt\Delta)/2]$.  In our  implementation we also maintain a third term $c = (b^2 - \Delta)/4a$.  We note that the variables $a$ and $b$ are the same in the ideal representative $[a, (b + \sqrt\Delta)/2]$ as in the binary quadratic form $ax^2 + bxy + cy^2$ with discriminant $\Delta$. Furthermore, both necessarily share the same value $c$.  As such, we adapt our discussion of the SPAR factoring algorithm to the language of ideal classes.
...
TODO: Give the algorithm in an algorithm section.

TODO: we might not need to introduce \emph{smooth} since we can talk about the order dividing the primorial instead.
...
Since we do not know if $\ord(\aclass)$ is smooth, we instead bound $k$ such that $2^k$ is no larger than the number of elements in the class group.  Schnorr and Lenstra \cite[p.291]{Schnorr1984} use $k = \floor{\log_2{\sqrt N}}$.  Therefore, we only need compute $\bclass^{\left(2^k\right)}$ for the smallest $k \le \floor{\log_2{\sqrt N}}$ such that $\bclass^{\left(2^k\right)} = \idclass$ if such a $k$ exists. If such a $k$ does not exist, then the algorithm continues with the second stage.
...
\]
until there is some $j < k$ such that $[\mathfrak c_j] = [\mathfrak c_k]$.  By the function $f$, we can compute $u$ and $v$ such that $[\mathfrak c_j]=\cclass^u$ and $[\mathfrak c_k]=\cclass^v$.  The order of $\cclass$ is then a multiple of $h = v - u$.  We can compute an ambiguous class representative by computing $\dclass = \bclass^h$ and then computing $\dclass^{\left(2^k\right)}$ for the smallest $k \le \floor{\log_2{\sqrt N}}$ as before.  Assuming that such a $k$ exists, then $\dclass^{\left(2^{k-1}\right)}$ is an ambiguous class representative and we can attempt to factor $N$.
...
TODO: This is \emph{bounded} primorial steps.
...
In practice, the efficiency of the SuperSPAR factoring algorithm depends on the smoothness of the order of the ideal class $\aclass$ with respect to the power primorial $E$ chosen.  The larger the divisor that $\ord(\aclass)$ and $E$ have in common, the smaller the order of $\cclass = \aclass^{\left(2^k\right)E}$ and the smaller the bound $b$ we need for the baby steps and giant steps.  Changing class groups by changing the square free multiplier $k$ for the discriminant $\Delta$ also changes the order of random elements in the new group.  As such, we can trade off the number of ideal classes or multipliers $k$ against the largest prime used in the power primorial $E$.

Furthermore, Schnorr and Lenstra \cite[p.293]{Schnorr1984} advise the use of smaller exponents $e'_i = \max \{v : {p_i}^v \le p_t \}$ for the first $t$ primes $p_i \le N^{1/2r}$ when computing the exponent $E$ in practice.  As the exponent $e_i$ in the prime power ${p_i}^{e_i}$ increases, the likelihood of a random integer being divisible by that prime power decreases.  For this reason, we bound the exponents $e_i$ of the prime powers ${p_i}^{e_i}$ separately from the bound on the largest prime used in $E$.

Finally, we note that selecting a bound $b=mP_n$ for the primorial steps phase implies that the number of group operations performed during this phase of the algorithm dominates over the number of group operations required to exponentiate $\aclass^E$.  In addition, as $P_n$ grows, computing baby steps coprime to $b$ becomes computational intractable.  For this reason, we select a separate bound on the largest prime used for the primorial $P_n$ in the bound $b$.

Ideally, we want to minimize the expected number of group operations that lead to a factorization of the integer $N$.  The parameters involved are the number of prime ideal classes $\aclass$ tried before switching multipliers and class groups, the bound on the largest prime in the exponent $E$, a bound on the exponents $e_i$ of the prime powers ${p_i}^{e_i}$ whose product is the exponents $E$, and a bound on the largest prime used in the bound $b$ on the number of baby steps.

TODO: Mention that giant steps are twice last baby step rounded up, to take advantage of inversions.


Assuming these parameters are known, the complete SuperSPAR is given in Algorithm \ref{alg:superSpar}.
...
TODO: We actually choose a prime bound for $E$ as $B = \Delta^{1/u}$ for some $u$ and use considerably smaller $e_i$ for the prime powers.

TODO: Then we choose a different prime bound $B' \le B$ for coprime baby steps.  Note that we take baby steps up to $b = mB'$ and then giant steps as a multiple of $b'$ as before.

The reason is that we can use a much larger primorial for exponentiation than it is reasonable to take baby steps to a multiple of.  By taking fewer baby steps and giant steps, we might not reach the actual bound $h_\Delta$, but then we trade off the time spent on order finding with the number of multipliers used.

\subsection{SuperSPAR Complexity}

- Bound on largest prime in power primorial for large exponentiation.
- Bound on exponents of powers primorial
- Bound on primorial steps
- 
