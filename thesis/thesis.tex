...
This chapter discusses the experiments and results that directed our implementation of the SuperSPAR integer factoring algorithm, discussed in Section \ref{sec:superSpar}.  The goal of this chapter is to show that SPAR can be modified and implemented so that it is the fastest available factoring implementation for many integer sizes no larger than 128-bits.  One way in which factoring integers of these sizes is useful is after the sieve step of more powerful integer factoring algorithms, such as the number field sieve (see \cite[\S 6.2]{Crandall2001}).  Factoring integers no larger than 128-bits is of particular interest in this thesis since Chapter \ref{chap:xgcdExperiments} demonstrates practical improvements for computing the extended GCD using our 32-bit, 64-bit, and 128-bits implementations, while Chapter \ref{chap:idealExperiments} demonstrates practical improvements for ideal class group arithmetic for orders with discriminant no larger than 118-bits.  This is relevant to our implementation of SuperSPAR, since SuperSPAR is based on the SPAR factoring algorithm (discussed in Section \ref{sec:spar}), which is based on arithmetic in the ideal class group.

SPAR factors an integer $N$ by attempting to compute the order of a random ideal class in an order of discriminant $-kN$ for some square free integer $k$.  SPAR uses a combination of exponentiation to a large power consisting of many small primes followed by a random walk search.  Our implementation of SuperSPAR improves upon SPAR by using a bounded primorial steps search, due to Sutherland \cite{Sutherland2007} and discussed in Subsection \ref{subsec:boundedPrimorialSteps}.  This is a practical improvement over a random walk search since a bounded primorial steps search has an asymptotically faster worst case (see Subsection \ref{subsec:boundedPrimorialSteps}).  Our implementation of SuperSPAR also improves upon the exponentiation stage of SPAR.  In this stage, SPAR exponentiates a random ideal class to the product of many small primes.  For SuperSPAR, we precompute 2,3 representations of these exponents based on the experiments in Chapter \ref{chap:powExperiments}, which demonstrates that the $L$ best approximations technique leads to an improvement over the binary exponentiation method in the context of our implementation of ideal class arithmetic.  


\begin{itemize}
\item Why we think we can improve SPAR
\item Justify statistics over experimentation, but experimentation when possible
\item Introduce each subsection
\item Redescribe the details of our implementation and the platform (x64) for experimentation.
\item Briefly state the results that we are going to show.
\end{itemize}
