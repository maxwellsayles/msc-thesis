...
Recall from Section \ref{sec:idealArithmetic} that the inverse of an ideal $\ideal a = [a, (b + \sqrt\Delta)/2]$ is the (possibly fractional) ideal
\[
...
\begin{algorithm}[h]
\caption{Computes a 2,3 strictly chained representation from low order to high order (Ciet et al \cite{Ciet2006}).}
...
\label{subsec:implementationOfIdealArithmetic}
...
\begin{comment}
The sieve algorithm iterates for each prime $2 \le p_j \le p_w$ and performs $b / p_j$ arithmetic operations.  This gives a bound of
\[
	\sum_{p_j \in \{2, 3, 5, ..., p_w\}} \frac{b}{p_j}
	\le \sum_{i=1}^{p_w} \frac{b}{i}
	= bp_w \sum_{i=1}^{p_w} \frac{1}{i}
	\in O(b p_w \ln p_w)
\]
arithmetic operations.  On the other hand, the coprime wheel iterates 

Assume for the inductive hypothesis that
\[
	\sum_{i=1}^w \phi(P_i) \le \frac{3}{2} \phi(P_w).
\]
The base case $\phi(P_2) + \phi(P_1) = 3 \le \frac{3}{2} \phi(P_2)$.  The inductive step is
\begin{align*}
	\sum_{i=1}^{w+1} \phi(P_i) &= \phi(P_{w+1}) + \sum_{i=1}^w \phi(P_i) \\
	&\le \phi(P_{w+1}) + \frac{3}{2} \phi(P_w) & \textrm{\{By the inductive hypothesis.\}} \\
	&\le \phi(P_{w+1}) + (p_{w+1}-1)\frac{1}{2} \phi(P_w) & \textrm{\{True for all $p_w \ge 3$.\}} \\
	&= \phi(P_{w+1}) + \frac{1}{2} \phi(P_{w+1}) & \textrm{\{Since $(p_{w+1}\phi(P_w) = \phi(P_{w+1}$.\}} \\
	&= \frac{3}{2} \phi(P_{w+1}),
\end{align*}
and it holds that for all $w$ that
\[
	\sum_{i=1}^w \phi(P_i) \le \frac{3}{2} \phi(P_w).
\]
\end{comment}
...
80 & 0.40664 & 0.80502 & 0.90738 & 0.93051 & 0.94722 & 0.95499 & 0.96228 & 0.96856 \\
	\hline
\end{tabular}
\caption{The probability that the order of an ideal class $\bclass = \aclass ^ E$ is coprime to the product $E = \prod {p_i}^{e_i}$ for primes $p_i$ where $e_i \le \floor{\log_{p_i} B}$ for $3 \le p_i \le 11$, $e_i = 1$ for all $p_i > 11$, and $e_1 \le \floor{\log_2 h_\Delta}$ for $p_1 = 2$.}
\label{tab:primePowerBound}
\end{table}

While such bounds do work well in practice, the purpose of this Section is to optimize the performance of our implementation of SuperSPAR.  As such, we iterated the bound $B$ for the exponentiation phase and timed the algorithm in practice. Table \ref{tab:actualPrimePowerBound} shows the average time to split semiprime integers\footnote{The sample set consists of 100,000 semiprime integers of each bit size, such that each integer was the product of two primes of equal size.} choosing the exponent $E$ such that odd primes $p_i \le B$ have $e_i = \floor{\log_{p_i} B}$ and $p_i > B$ have $e_i = 1$.  The primorial and multiplier in the search phase are chosen to give good performance on average and are fixed for each integer bit size.  Our implementation of SuperSPAR uses exponents with prime power bounds that correspond to the best times for each bit range.

% TODO: consider redoing this table with the 100,000 set of semiprimes
% TODO: interesting that after determining the best primorial for each bit size,
...
2187 & = & $3^7$ & 0.06274 & 0.11096 & 0.25596 & 0.69223 & 1.82186 & 4.18447 & 9.34884 \\
	\hline
\end{tabular}
\caption{Average time (in milliseconds) to factor semiprime integers using the exponent $E = \prod_{p_i > 2} {p_i}^{e_i}$ with $e_i = \floor{\log_{p_i} B}$ when $p_i \le B$ and $e_i = 1$ otherwise.  The lowest time for each integer range is underlined.}
...
16 & 0.04251 & 0.08431 & 0.20248 & 0.57780 & 1.60844 & 3.97845 & 8.97971 \\
	\hline
\end{tabular}
\caption{The average time (in milliseconds) to split an integer $N$ using no more than a fixed number of ideal classes for each class group.  The lowest time for each integer range is underlined.}
...
\mygraph{hashing-open-vs-chained}{fig:ssparHashing}{The average time to split an integer using either open addressing or chaining with list heads for collision resolution in a hash table.}
...
Theoretically, SuperSPAR takes the number of baby steps and giant steps proportional to the largest prime, $p_t$, used in the exponent of the exponentiation stage, such that $p_t \le N^{1/2r}$, $p_t$ is as large as possible, and $r = \sqrt{\ln N / \ln \ln N}$ (see Section \ref{sec:superSpar}).  The idea being that the search stage should use no more than a constant number of operations more than the exponentiation stage.   However, by choosing the number of baby steps and giant steps independently from the exponent used in the exponentiation stage, SuperSPAR can perform better in practice.  The algorithm takes baby steps coprime to a multiple of some primorial.  One could evaluate all multiples of primorials up to a certain bound, but this is more work than is necessary.  This section discusses the method used by our implementation to generate bounds useful for searching.  These bounds are later evaluated empirically using SuperSPAR to split integers of various sizes in Section \ref{sec:ssparEmpirical}.

The success of the search stage depends on the largest exponent $2js$ generated by the giant steps -- call this the \emph{search range}.  The idea here is to generate a set $\stepBounds$ of primorial and multiplier pairs used as candidates for the baby step bound $s$.  For each pair, let $\stepCount$ be a function from some multiple $m$ of the primorial $P_w$ to the total number of baby steps and giant steps taken during the search stage of the algorithm,
\begin{equation}
\label{eq:stepCount}
...
\]
But this is only possible when $w_i = w_j$ and then for $\stepCount(m_i, w_i) = \stepCount(m_j, w_j)$ and $\lastStep(m_i, w_i) = \lastStep(m_j, w_j)$ to be true, $m_i = m_j$ must also be true.
\end{proof}
...
\end{align*}
The set is ordered according to Algorithm \ref{alg:stepBounds}.  The number of steps for each corresponding pair is
\begin{align*}
...
\end{align*}
and the search reach, i.e.\ the value of the largest exponent generated by the giant steps, for each corresponding pair is
\begin{align*}
...
Exponents for the exponentiation stage were generated using the $L$ best approximations technique of Section \ref{sec:expBestApproximations}.  The number of candidates $L=2^k$ was selected such that using $L'=2^{k+1}$ did not produce a 2,3 representation with lower cost.  The value in practice was $L=2048$.


\mygraph{search-56-bits-primorial-x}{fig:search56PrimorialX}{TODO}
\mygraph{search-56-bits-steps-x}{fig:search56StepX}{TODO}
\mygraph{search-56-bits-primorial-x-steps-min}{fig:search56StepX}{TODO}
\mygraph{search-56-bits-primorial-min-steps-x}{fig:search56PrimorialMin}{TODO}

\mygraph{search-64-bits-primorial-x}{fig:search64PrimorialX}{TODO}
\mygraph{search-64-bits-steps-x}{fig:search64StepX}{TODO}
\mygraph{search-64-bits-primorial-x-steps-min}{fig:search64StepX}{TODO}
\mygraph{search-64-bits-primorial-min-steps-x}{fig:search64PrimorialMin}{TODO}


Quadratic samples at 1/3 and 2/3 and throws away the larger third.


% TODO: show that the largest primorial used for searching is $P_5$.
% TODO: show that a prime power bound of $B=49$ is a good choice.
...
\section{Improvements to Ideal Arithmetic}

Time basic composition/squaring against NUCOMP/NUDUPL.

Ideal reduction uses a Euclidean like algorithm.  Try adapting the extended GCD computations from Section \ref{sec:eea} to ideal reduction.

Comparison with ideal arithmetic from ANTL (QIR) library.
...
Suppose we assume that when there's a collision in the hash table that $\pm2bj - i$ is a multiple of the order, i.e.\ we don't ignore the $b$ component when hashing and we lookup both $\bclass^{+2bj}$ abd $\bclass^{-2bj}$.  Then we perform 2 table lookups per giant step, but once there is a collision, we don't have to test which one is the order.  This is probably slower because we do considerably more table lookups than we do exponentiation tests, i.e.\ there's two looks up for each giant step for each multiplier used and only one exponentiate, vs only one lookup for each giant step for each multiplier, but two exponentiations.
