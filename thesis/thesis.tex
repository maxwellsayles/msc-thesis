...
\FOR{$1 \le j \le m\phi(P_w)$}
	\IF{$\bclass^{2bj}$ is in the table, first lookup $i$}
		\STATE $h \gets$ odd part of $(2bj - i)E$, then go to line \ref{goto:tryAmbiguous}  \COMMENT{positive giant-step}
	\ELSIF{$\bclass^{-2bj}$ is in the table, first lookup $i$}
		\STATE $h \gets$ odd part of $(2bj + i)E$, then go to line \ref{goto:tryAmbiguous} \COMMENT{inverted giant-step}
	\ENDIF
\ENDFOR
...
% TODO: Cite something from literature?
% TODO: State why it's called a "Wheel"?
...
The SuperSPAR Algorithm \ref{alg:superSpar} (and more generally, the bounded primorial steps algorithm \ref{alg:primorialSteps}) requires a lookup table\footnote{The lookup table maps group elements $\bclass^i$ to exponents $i$.}.  Baby steps mapping group elements $\bclass^i$ to exponents $i$ coprime to some primorial $P_w$ are stored in the lookup table, while giant steps for group elements $\bclass^{2bj}$ and $\bclass^{-2bj}$ are retrieved from the lookup table.  As such, the performance of the algorithm directly corresponds to the performance of the lookup table.  This Section describes the experimental results for SuperSPAR using two different implementations of lookup tables, as well as the method used to map group elements to exponents in our implementation.

Our implementation of idea class arithmetic represents an ideal class $\aclass \in Cl_\Delta$ using the triple $(a, b, c)$ where $\ideal a = [a, (b + \sqrt\Delta)/2]$ is a reduced representative for the ideal class and $c = (b^2 - \Delta)/4a$ (see Subsection \ref{subsec:implementationOfIdealArithmetic}).  Since inversion of ideal classes is virtually free, SuperSPAR computes giant steps for ideal classes $\bclass^{2bj}$ and $\bclass^{-2bj}$, but rather than perform two table lookups, we take advantage of how the inverse of an ideal class is computed.  Recall form Subsection \ref{subsec:idealClassInverse} that the inverse of an ideal class for a representative $[a, (b + \sqrt\Delta)/2]$ is given by the representative $[a, (-b + \sqrt\Delta)]$.  Also notice that given $a$, $c$, and $\Delta$, that $|b| = +\sqrt{\Delta + 4ac}$.  As such, by restricting our representation of an ideal class to the pair $(a, c)$ when indexing the table, our implementation is able to look up both $\bclass^{2bj}$ and $\bclass^{-2bj}$ using a single table access.  However, once a giant step finds some $\bclass^i$ already in the table, both the exponents $i-2bj$ and $i+2bj$ must be tested in order to determine which one is a multiple of the order of $\bclass$.

The lookup table is implemented as a hash table with 32-bit keys and 32-bit values (both as unsigned integers).  Our implementation maps the pair $(a, c)$ associated with an ideal class to a 32-bit hash key by computing
\[
	\hash(a, c) = (2654435761 \times (2654435761a + c)) \bmod 2^{32}.
\]
The multiplier 2654435761 was chosen because it is the largest prime less than $2^{32} \times (\sqrt 5 - 1)/2$.  We use this value since $(\sqrt 5 - 1)/2$ has been shown by Knuth \cite[Section~6.4]{Knuth1998} to have good scattering properties\footnote{Other multipliers were also tried but had little impact on performance.}.  Since the number of baby steps is a function of the size of the integer to be factored (see Section \ref{sec:ssparEmpirical}), the maximum number of entries in the hash table is known at initialization.  We found that a table of $2^k$ slots for the smallest $k$ such that $2^k \ge 3m\phi(P_2)$, where $m\phi(P_w)$ is the number of baby steps, worked well in practice\footnote{Different multiples of the number of baby steps were tried, as well as setting the number of slots to a prime near some multiple of the number of baby steps. There was little difference in performance, with the exception of a table having too few slots (resulting in an insertion failing), or a table having an excessive number of slots (and table initialization dominated).}.  Using a power of 2 for the number of slots has the added benefit that once a hash key is computed, the corresponding slot is found by using a binary $\band$ operation.

Regardless of the hash function used, multiple elements may hash to the same slot.  Two methods of collision resolution were considered.  The first method is chaining with list heads (see \cite[Subsection~11.2]{Cormen2001}).  Each slot consists of the 32-bit value for the specified key, as well as a 32-bit index into an overflow table.  The entry indexed in the overflow table also consists of a 32-bit value for the same key, and another index into the overflow table for the next value of the same key.  This is essentially a linked list, where the head of the list is stored directly in the slot of the hash table .  Special constants are used to indicate that a slot is empty (e.g.\ 0x00000000), or that the index into the overflow table should not be followed (e.g.\ 0xFFFFFFFF).

The second method considered is open addressing, and in particular, we implemented the same probing function as in CPython \cite{Kuchling2008}.  In this method, each slot consists of a 32-bit hash key, as well as the 32-bit value associated with that hash key.  Any hash key can occupy any slot, and a special pair of constants are used to indicate that a slot is unoccupied, e.g.\ (0xAAAAAAAA, 0x55555555).   When an element is hashed, the table is probed at slot indices $s_1, s_2, ...$ generated by a probing function until a slot is found where either the hash key occupying the slot is the same as the hash key for the element in question, or the slot is unoccupied.  Our implementation defines probe indices using
\begin{align*}
	s_1 &= \hash(a,b) \bmod 2^k \\
	s_i &= \left( 5s_{i-1} + 1 + \floor{\hash(a,b)/2^{5i}} \right) \bmod 2^k.
\end{align*}

For the same set of semiprime integers as in Table \ref{tab:actualPrimePowerBound}, and with the exponent for the exponentiation stage, and the primorial and multiplier for the search stage chosen to give good performance on average, Table TODO shows the average time to split semiprime integers of the specified size using each of the collision resolution methods described above.
