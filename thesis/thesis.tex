...
% TODO: Lower bound of O(log N) for number of group operations.  o(log N) is only for the number of terms.
...
\begin{defn}
The \emph{ambiguous classes} are the classes $[\mathfrak a]$ such that ${[\mathfrak a]}^2$ is the identity class \cite{Schnorr1984}.  Notice that the identity ideal class $[\mathcal O_\Delta] \in Cl_\Delta$ is an ambiguous class.
\end{defn}
...
\end{align*}
TODO: describe how to use the $\gcd$ to compute the factor.
Suppose we wish to find a factor of an odd integer $N$. Since $\Delta = b^2 - 4ac$ we must have $\Delta \equiv 0, 1 \pmod 4$.  Therefore, for some square free integer $k$, let $\Delta = -kN$ when $-kN \equiv 1 \pmod 4$ and $\Delta = -4kN$ otherwise.  Now, to find a factor of $N$ we find a reduced ambiguous class representative with discriminant $\Delta$.  If we are lucky, we get a proper factor of $N$.


\subsection{SPAR Algorithm}

In Chapter \ref{chap:idealArithmetic}, we mentioned that for a negative discriminant $\Delta$, the ideal class group $Cl_\Delta$ has a finite number of elements (TODO: Mention this in Ch2!).  This means that for a random ideal class $\aclass$, there exists an integer $m$ such that $\aclass^m = \idclass$. We say that $m$ is the \emph{order} of the element $\aclass$ and denote this $m = \ord(\aclass)$.  When the order is even, then $\bclass = \aclass^{m/2}$ is an ambiguous ideal class.  This follows from the fact that $\bclass^2 = \idclass$.  Therefore, factoring an integer $N$ reduces to the problem of determining the order of a random ideal class $\aclass \in Cl_\Delta$ for $\Delta$ a square free multiple of $-N$ or $-4N$.
...
In the first stage of the algorithm, we exponentiate a random ideal class $\aclass \in Cl_\Delta$ to the product of primes $\prod_{i=2}^t {p_i}^{e_i}$ such that $p_t \approx N^{1/2r}$ where $r = \sqrt{\ln N / \ln \ln N}$.  Using binary exponentiation, this will take $O(p_t)$ group operations and for a random composite integer $m \in [0, N]$ will factor $m$ with probability $\ge r^{-r}$ \cite[p.290]{Schnorr1984}. Stage 2 performs a random walk of at most $O(p_t)$ group operations and with probability $\ge (r-2)^{-(r-2)}$ will factor $m$ \cite[p.290]{Schnorr1984}.  Their claim is that if Stage 1 is run on each integer $kN$ for $k \le r^r$, then every composite integer $N$ will be factored within $o\left(\exp \sqrt{ \ln N \ln\ln N } \right)$ bit operations.
...
\STATE maximize $P_w$ such that $P_w \le \sqrt{M}$
...
Stage 1 takes the first $t$ primes $p_1 = 2, p_2 = 3, ..., p_t \approx N^{1/2r}$ for $r = \sqrt{\ln N / \ln \ln N}$, sets $e_i = \max \{ v : {p_i}^v \le {p_t}^2 \}$, lets $E = \prod_{i=2}^t {p_i}^{e_i}$ be the product of the odd prime powers, and then computes
\[
	\bclass = \aclass^E,
\]
for a random ideal class $\aclass$.  Finally, it computes $\cclass = {\bclass}^{\left(2^k\right)}$ for $k=\floor{\log_2 \sqrt{|\Delta|}}$.  If $\bclass = \idclass$, then $\ord(\aclass)$ divides $E$ and is odd, and the algorithm tries again with a different random ideal class $\aclass$.  If the odd part of $\ord(\aclass)$ divides $E$ and the order of $\aclass$ is even, then there is some $\bclass^{\left( 2^j \right)}$ for $0 \le j < k$ that is an ambiguous ideal and we attempt to factor the discriminant $\Delta$.  Failing this, SuperSPAR continues with stage 2, and this is where SPAR and SuperSPAR differ.

Stage 2 of SPAR attempts to find the order of $\cclass$ by performing a random walk on the cyclic group it generates by using $O(p_t)$ group operations.  Schnorr and Lenstra \cite[p.294]{Schnorr1984} suggest a Pollard-Brent Recursion \cite{Brent1980} based on Pollard's rho method \cite{Pollard1975}, but also remark \cite[p.298]{Schnorr1984} that Shanks' baby-step giant-step method \cite{Shanks1971} could be used to deterministically find $\ord(\cclass)$.  They point out that this approach could be made faster by exploiting the fact that $\ord(\cclass)$ is likely to have no prime divisors $p_1, ..., p_t$, and this is precisely what SuperSPAR does.

Stage 2 of SuperSPAR attempts to find the order of $\cclass$ by taking baby steps coprime to some primorial, and then giant steps that are a multiple of that primorial.  Since the function $\phi(P_w)$ (Equation \ref{eq:phiPrimorial}) gives the number of integers coprime to the primorial $P_w$, we use this to determine the largest primorial to use for the coprime baby steps.  First maximize $\phi(P_w) \le p_t$, then let $m = \floor{p_t / \phi(P_w)}$ and $b = mP_w$.  Notice that $b$ is a multiple of the primorial $P_w$ and that the number of baby steps is $s = m \phi(P_w) \le p_t$.  We then take baby steps $\cclass^i$ for $1 \le i \le b$ with $i$ coprime to $P_w$, followed by giant steps $\cclass^j$ for $j=2b,-2b,4b,-4b,...,2sb,-2sb$\footnote{We use this sequence of giant steps since computing the inverse in the ideal class group is essentially free.}.  Taking an equal number of giant steps as baby steps means that this stage of SuperSPAR uses $O(p_t)$ group operations (not counting inversions).

If Stage 2 successfully computes $h' = \ord(\cclass)$ and assuming that $\ord(\bclass)$ is even, we compute an ambiguous ideal by repeated squaring of $\bclass^{h'}$ and then attempt to factor the discriminant.  If the order is odd, however, then the algorithm runs Stage 1 again with a different random ideal class $\aclass$.  If Stage 2 fails to compute the order of $\cclass$, the algorithm tries again with different square free multiplier $k$.
...
%TODO: something about the class number being $\sqrt{|\Delta|}$ as $\Delta \rightarrow \infty$ \cite[p.247]{Crandall2001}.
...
In practice, balancing both stages of SuperSPAR to use $O(N^{1/2r})$ group operations for $r = \sqrt{\ln N / \ln \ln N}$ is often not ideal.  This is partly because the actual cost of multiplication, squaring, and cubing differ, but also because the success of each stage depends on different properties of the class number.  The exponentiation stage is successful when the order of a random ideal class is smooth with respect to some primorial $E$, while the search stage is successful when the non-smooth part of the order is sufficiently small.  In this sense, the efficiency of the factoring algorithm depends both on the smoothness of a random ideal class $\aclass$ with respect to the primorial $E$, but also on the size of its non-smooth part with respect to the largest exponent reached in the search stage.  Selecting both the primorial $E$ for the exponentiation stage and the bound $b = mP_w$ for the search stage independently of $r$ will vary the time spent during each stage inversely to the probability of the success of that stage.  Section \ref{sec:superSparExponentiation} discusses our choice of the exponent $E$ for the exponentiation stage, and Section \ref{sec:expVsSearch} discusses our approach to generating a set of bounds for the search stage.

In both SPAR and SuperSPAR, the largest prime used by the exponentiation stage is $p_t \approx N^{1/2r}$.  This is chosen to theoretically minimize the total expected running time of the algorithm.  Using a larger value for $r$ means this stage is less successful, and the algorithm is more likely to use the search phase, and more likely to use additional multipliers for the discriminant.  On the other hand, using a smaller value for $r$ means this stage is more likely to be successful, and ultimately fewer multipliers are tried.  Since $r = \sqrt{\ln N / \ln \ln N}$ is chosen to minimize the expected running time in theory, in practice it may be more efficient to choose some other value for $r$, effectively trading off the number of ideal classes tried against the largest prime used in the primorial $E$.

Furthermore, Schnorr and Lenstra \cite[p.293]{Schnorr1984} advise the use of smaller exponents $e'_i = \max \{v : {p_i}^v \le p_t \}$ for the first $t$ primes $p_i \le N^{1/2r}$ when computing the exponent $E$ in practice.  Section \ref{sec:idealsToMultipliers} of this thesis studies sequential prime ideal representatives for fixed discriminants.  Let 
\[
	\mathcal P_{\Delta, n} = \left\{ [\mathfrak a_i] \in Cl_\Delta ~:~  \mathfrak a_i \in Cl_\Delta \textrm{ is the $\ith$ smallest prime ideal for } 1 \le i \le n \right\}
\]
be the set of the $n$ smallest prime ideal representatives for $Cl_\Delta$. Computing the factorization of the order of each $[\mathfrak a_i]$ shows that most prime factors of the order are unique, but also that for a given ideal class $Cl_\Delta$, that the order of ideal classes from the set above only differ by a small factor from each other.  If the exponent $E$ is chosen to include sufficiently many small factors, then if either stage of the algorithm discovers some $h$ such that $[\mathfrak a_i]^h = \idclass$, then it is likely that $[\mathfrak a_j]^h = \idclass$ for some other prime ideal $[\mathfrak a_j] \in Cl_\Delta$.  As such, once the algorithm discovers some $h$ that is a multiple of the order of a prime ideal, the algorithm then tries to find an ambiguous form and attempts to factor $N$.  Failing this, the algorithm simply chooses the next prime ideal representative and tries again with the same value of $h$.  Once this fails for several prime ideal representatives, the algorithm then starts over with a different multiplier $k$ for the discriminant.  Let $t$ be the number of ideal classes attempted before switching multipliers.

Assuming good parameters are known, the SuperSPAR factoring algorithm used in practice is given in listing \ref{alg:superSpar}.  Chapter \ref{chap:superSparExperiments} discusses the experiments and results that lead to parameters for this algorithm that perform well in practice.  Notice, that parameters can be selected based on the theoretical analysis of SuperSPAR from the previous section.

%TODO: Define $h_\Delta$ either in this section or in the section on complexity.
%TODO: Give an algorithm to test for an ambiguous form.
%TODO: Give an algorithm to attempt to factor $N$ from an ambiguous form.
...
\REQUIRE $N \in \ZZgez$ odd and composite, \\
  ...
  $t \in \ZZgtz$ the number of ideal classes to try before switching multipliers.
...
\label{sec:superSparExponentiation}
