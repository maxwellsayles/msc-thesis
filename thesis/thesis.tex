...
\begin{example}
Let $\OO = [1, \sqrt{\Delta}]$ for $\Delta = -20$ be an order in the field $\QQ(\sqrt{-5})$ and let 
\[
	\ideal a = \left\{7x + \frac{(8 + \sqrt{\Delta})y}{2} : x, y \in \OO \right\}
	= \left( 7, \frac{8 + \sqrt\Delta}{2} \right)
\]
be an ideal of $\OO$.  The inverse of $\ideal a$ is given as
\[
...
\begin{example}
Let $\OO = [1, \sqrt\Delta]$ for $\Delta = -20$ be an order in the field $\QQ(\sqrt{-5})$ and let $\ideal a = (3, (4+\sqrt\Delta)/2)$ and $\ideal b = (3 \sqrt\Delta, -10 + 2 \sqrt\Delta)$ be two ideals of $\OO$.  Ideals $\ideal a$ and $\ideal b$ are equivalent since
\[
...
\]
where $(\sqrt\Delta)$ and $(1)$ are principle ideals of $\OO$.
\end{example}
...
\begin{example}
Let $\ideal a = [7, (8 + \sqrt{\Delta})/2]$ with $\Delta = -20$.  Initially, $a = 7, b = 8, c = 3$.  Using Algorithm \ref{alg:reduce}, the steps to compute a reduced ideal equivalent to $\ideal a$ are as follows.
\begin{align*}
...
\begin{example}
Let $\OO = [1,\sqrt{\Delta}]$ for $\Delta = -20$ be an order of $\QQ(\sqrt{-5})$.  Now let $\ideal a = [3, (4+\sqrt{\Delta})/2]$ and $\ideal b = [6, (10 + \sqrt{\Delta})/2]$ be reduced ideals of $\OO$.  Multiplication using the above gives
\[
...
\break
\begin{proof}
By contradiction, suppose there exists $(m_i, w_i)$ and $(m_j, w_j)$ with $m_i \neq m_j$ or $w_i \neq w_j$ such that $\stepCount(m_i, w_i) = \stepCount(m_j, w_j)$ and $\lastStep(m_i, w_i) = \lastStep(m_j, w_j)$.  Let $w_i < w_j$. By Equation \ref{eq:stepCount}
\begin{equation}
\label{eq:fromAbove}
...
\end{equation}
Then by Equation \ref{eq:lastStep}
\[
2{m_i}^2\phi(P_{w_i})P_{w_i} = 2{m_j}^2\phi(P_{w_j})P_{w_j}.
\]
Substituting from Equation \ref{eq:fromAbove} above gives
\[
...
\end{equation}
Then by Equations \ref{eq:mimj1} and \ref{eq:mimj2}, we have
\[
