...
\end{equation}
The first step is to reduce $a$ by setting $\ideal a = [c, (-b + \sqrt\Delta)/2]$, if $a > c$.  Then $b$ is reduced since $b$ is unique modulo $2a$.  These steps are repeated while $\ideal a$ is not reduced. In the case that $a = c$, we use the absolute value of $b$, since by Equation \ref{eq:idealSwapAC} the ideals $[a, (b + \sqrt\Delta)/2]$ and $[c, (-b+\sqrt\Delta)/2]$ are equivalent.  Algorithm \ref{alg:reduce} gives Pseudo-code.
...
\mygraphXPNG{bits-72-3d.png}{fig:search72td}{Average time to factor 72-bit integers.}{64-bit Factoring Search Space.}
\mygraphXPNG{bits-80-3d.png}{fig:search80td}{Average time to factor 80-bit integers.}{64-bit Factoring Search Space.}
