...
TODO: Algorithm, incorporates cost of multiply, square, cube, cache efficiency
...
Since both functions require an exponential amount of computation in the length of the input, we were only able to compute chains for small exponents.  This is still useful when we consider methods that partition large exponents into smaller blocks using their binary representation, or when we consider multiple exponentiations by a list of prime powers, rather than a single exponentiation by the product of many primes.


\subsection{Incremental Searching}
\label{subsec:incrementalSearching}

For small enough inputs, the number of possible additive 2,3 chains is sufficiently few that we can compute all such chains in order to find the shortest such chain.  Here we consider a different approach to searching for representations.  Rather than searching for a representation for a specific integer, we instead iterate over a set of possible representations and compute each integer that is represented.  For each integer represented in the set, we record the cheapest representation encountered.
...
In our experiments, we wanted to search for representations for all 16-bit integers, so we started our search with $A=\ceil{\log_2\left(2^{16}\right)} = 16$, $B=\ceil{\log_3 \left(2^{16}\right)} = 11$, and we iterated the number of terms $k$ from 1 through 5. For representations of $k=6$ terms, our implementation took several days to complete.  We found that for our 64-bit implementation of ideal arithmetic, that no integer was associated with a minimal cost representation of more than 5 terms.  We then ran our search again using $A=18$ and $B=12$ and found that none of our minimal representations changed.  For our 128-bit implementation of ideal arithmetic, we found that (TODO) number of integers had better minimal representations with 6 terms.  We did not extend the search to representations with 7 terms, however.

\subsection{16-bit Blocked Exponentiation}

The techniques of the previous two Subsections are computationally expensive and as such, we limited our search to representations of 16-bit integers.  We mentioned previously that this is still useful for larger exponents, since we can represent the exponent $n$ in 16-bit blocks as
\[
...
\subsection{Exponentiation by the Prime Factorization}

\subsection{Experimental Results}

One reason to improve the performance of exponentiation in the ideal class group is to improve the performance of order finding in this group.  In particular, we consider the approach where we exponentiate a random ideal class to the product of several small prime powers.  For a group element $g$, and the odd primorial $P_n = \prod_{i=2}^n p_i$ where $p_i$ is the $i^{\textrm{th}}$ prime number, we want to compute $g^{P_n}$. We first show empirical results for computing $g^{p_2} \cdot g^{p_3} \cdots g^{p_n}$ as a series of $n-1$ small exponentiations and $n-2$ multiplications.  We then show results for computing $g^{P_n}$ Then we show our results of computing $g^{P_n}$ as one large exponentiation.  Finally, we compare the two different approaches in order to determine which is faster in practice.
...
In Figure \ref{fig:memoChains} we show the average time to exponentiate ideals with 59-bit and 118-bit discriminants for exponents 1 through 65535.  The graph shows every $512^{\textrm{th}}$ point where the two approaches give different timings\footnote{In the case of the 59-bit discriminant, the timings were the same for 21,816 exponents, and for the 118-bit discriminant, the timings were the same for 16,481 exponents.}.  For every exponent tested, $\pm 2, 3$ chains were on average as fast or faster than additive only chains.
\begin{figure}[H]
\centering
...
Similar to before, Figure \ref{fig:preVsMemo} shows the average time to exponentiate ideals with 59-bit and 118-bit discriminants for exponents 1 through 65535. Again, the graph shows every $512^{\textrm{th}}$ point where the two approaches give different timings\footnote{For the 59-bit discriminant, the timings were the same for 37,732 exponents and the incremental search always yielded a lower average cost. For the 118-bit discriminant, the timings were the same for 44,291 exponents. For 19,390 exponents, the incremental search produced lower average costs, but for 1854 exponents, the cost was actually higher.} For the 64-bit implementation, the incremental search was as fast or faster on average for all exponents tested, while the 128-bit implementation was slower on average for 2.8\% of the exponents tested.
\begin{figure}[H]
\centering
\includegraphics{pre_vs_memo-64}
\includegraphics{pre_vs_memo-128}
