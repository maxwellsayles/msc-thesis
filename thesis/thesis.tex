...
\STATE lazily compute prime ideal classes $[\mathfrak a_1], [\mathfrak a_2], ..., [\mathfrak a_c]$ in $Cl_\Delta$, only as needed
...
\STATE $h \gets 1$
\STATE \{-- attempt to factor $N$ by finding an ambiguous ideal class --\}
\FOR{$1 \le k \le \floor{\log_2 h_\Delta}$}
\label{goto:tryAmbiguous}
...
\FOR{$1 \le i \le s$ where $\gcd(i,P_w)=1$}
	...
	\IF{$\bclass^i = \idclass$}
		\STATE $h \gets iE$, $\bclass \gets \idealclass{b'}^i$, then go to line \ref{goto:tryAmbiguous}
	\ENDIF
\ENDFOR
...
\FOR{$2 \le y \le c$}
	\STATE $\bclass \gets {[\mathfrak a_y]}^h$
