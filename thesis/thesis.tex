...
TODO: Fill this out. \cite{Imbert2010b}.

 (incorporates cost)

This section should probably go into the chapter on exponentiation since it's been published.
...
When the exponent is not too large, Imbert and Philippe \cite{Imbert2010b} give a method to compute the shortest additive strictly chained 2,3 partitions.  These partitions do not permit subtraction and every term is strictly less than and divides all subsequent terms. These partitions take the form
\[
	n = \sum_{i=0}^k 2^{a_i}3^{b_i}
\]
for $a_i, b_i \in \ZZgez$ where $2^{a_i}3^{b_i}$ divides $2^{a_j}3^{b_j}$ and $a_i < a_j$ and $b_i < b_j$ for all $i < j$.

We discussed this technique in detail in Subsection \ref{subsec:shortAddChains}.  For our purposes, we were able to modify this algorithm to incorporate the average cost of arithmetic in the ideal class group, so that the resulting additive 2,3 chains give a better average time for exponentiation.  Since their algorithm uses an exponential amount of time in the length of the input, we were only able to compute optimal additive chains for small exponents.  This is useful, however, when we consider methods that partition larger exponents into smaller blocks using their binary representation, or when we consider exponentiating by a list of prime powers, rather than by their large product.
...
f(n) = \begin{cases}
	...
	\min(M + f(n-1), M + f(n+1)) & \textrm{when } n \equiv 1 \pmod 6 \\
	\min(S + f(n/2), M + C + f((n+1)/3) & \textrm{when } n \equiv 2 \pmod 6 \\
	\min(C + f(n/3), M + S + f((n-1)/2) & \textrm{when } n \equiv 3 \pmod 6 \\
	\min(S + f(n/2), M + f(n-1)) & \textrm{when } n \equiv 4 \pmod 6 \\
	\min(M + f(n-1), M + f(n+1)) & \textrm{when } n \equiv 5 \pmod 6
\end{cases}
\end{equation*}
where $f(1) = 0$, $f(2) = S$, and $f(3) = C$ are the base cases.

\begin{figure}[H]
\centering
\includegraphics{add_chains-64}
\caption{a}
\label{fig:addChains64}
\end{figure}


\subsection{Searching For 2,3 Representations}

The approach to computing optimal additive 2,3 chains constrains the search space sufficiently, that we can then exhaustively compute all additive 2,3 strictly chained partitions in order to find the one with the cheapest average cost to exponentiate.  Here we consider a different approach to searching for representations.
