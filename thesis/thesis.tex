...
The SuperSPAR Algorithm \ref{alg:superSpar} (and more generally, the bounded primorial steps algorithm \ref{alg:primorialSteps}) requires a lookup table.  Baby steps for group elements $\bclass^i$ for $i$ coprime to some primorial $P_w$ are stored in the lookup table, while giant steps for group elements $\bclass^{2bj}$ and $\bclass^{-2bj}$ are retrieved from the lookup table.  As such, the performance of the algorithm directly corresponds to the performance of the lookup table.  This Section describes the approach used to store and retrieve group elements from a lookup table in our implementation of SuperSPAR.

Our implementation of idea class arithmetic represents an ideal class $\aclass \in Cl_\Delta$ with the triple $(a, b, c)$ where $\ideal a = [a, (b + \sqrt\Delta)/2]$ is a reduced representative ideal for the ideal class and $c = (b^2 - \Delta)/4a$ (see Subsection \ref{subsec:implementationOfIdealArithmetic}).  Since inversion of ideal classes is virtually free, SuperSPAR computes giant steps for ideal classes $\bclass^{2bj}$ and $\bclass^{-2bj}$, but rather than access the table twice, we take advantage of how the inverse of an ideal class is computed.  Recall form Subsection \ref{subsec:idealClassInverse} that the inverse of an ideal class for a representative $[a, (b + \sqrt\Delta)/2]$ is given by the representative $[a, (-b + \sqrt\Delta)]$.  Also notice that given $a$, $c$, and $\Delta$, that $|b| = +\sqrt{\Delta + 4ac}$.  As such, by using the pair $(a, c)$ to index the table instead, our implementation is able to look up both $\bclass^{2bj}$ and $\bclass^{-2bj}$ using a single table access.


Collision resolution with chaining \cite[Subsection~11.2]{Cormen2001} where the first record is stored in the slot array itself.  The idea is to reduce memory and improve cache efficiency, since a node in the list is only traversed when there's a collision, which we are trying to minimize anyways.

Open Addressing \cite{Kuchling2008} using the addressing scheme from Python.
