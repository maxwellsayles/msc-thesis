...
\newcommand{\Pari}{Pari 2.5.1}
\newcommand{\PariGP}{Pari/GP 2.5.1}
...
We specialized the implementation of each of the extended GCD algorithms for 32, 64, and 128-bit inputs.  For each $k$ from 1 to 127, we generated 1,000,000 pairs of random positive integers of $k$ bits each.  For the 32-bit implementations, we measured the time to compute the extended GCD of each pair for $1 \le k \le 31$, then for the 64-bit implementations, we measure the time for each pair for $1 \le k \le 63$, and finally for the 128-bit implementations, we measure the time for each pair for $1 \le k \le 127$.  Only positive integers were considered since the prologue and epilogue code to handle negative inputs is the same in each implementation, namely the sign of the inputs is recorded and the inputs are made positive, the extended GCD is computed, and then the sign of the output coefficients is corrected.  The source for these experiments was developed using the \compiler{} on Ubuntu 12.10.  The experiments were performed on a \cpu{} with 8Gb of memory.  The CPU has four cores, only one of which was used during testing.

To verify that this work has lead to practical performance improvements, we perform the same timing experiments using \Pari{}, \MPIR{}, and \GMP{} as reference implementations.  We present this data first.  We then demonstrate that for each of the extended GCD techniques presented in this chapter, that our 32-bit implementation is no slower than our 64-bit implementation, which is no slower than our 128-bit implementation.  Finally, we show that for input integers no larger than 31-bits, that our implementation of the standard extended Euclidean algorithm performs best.  For input integers larger than 31-bits but no larger than 63-bits, our implementation of a simplified left-to-right binary extended GCD is the fastest.  Then for input integers larger than 63-bits but no larger than 127-bits, our implementation of Lehmer's extended GCD using our simplified left-to-right binary extended GCD for 64-bit machine words is the fastest of the implementations tested.  For input integers larger than 127-bits, we defer to one of the reference implementations.

Ideal class arithmetic uses a variant of the extended GCD where only $s$ and $U$ are computed for the solution $s = Ua + Vb$ for input integers $a$ and $b$.   This is referred to as a \emph{left-only} extended GCD.  Ideal class arithmetic also uses the partial extended GCD, which is similar to the left-only extended GCD, but also that the algorithm is terminated when the next quotient is less than a specified bound.  Future work (see Section \ref{sec:futureIdealArithmetic}) would test these implementations specifically.  In Chapter \ref{chap:idealExperiments}, we adapt the best performing extended GCD for each input size for use with the left-only extended GCD and the partial extended GCD.

\subsection{Reference Implementations}

As reference implementations of the extended GCD, we use \Pari{}, \MPIR{}, and \GMP{}.  In the case of both GMP and MPIR, we use instances of \texttt{mpz\_t} modified to use at most two 64-bit words of locally allocated stack space for limbs.  The only operations timed are calls to \texttt{mpz\_xgcd}.  Since MPIR is a fork of GMP, we simply change the \texttt{\#include} directive, recompile, and then link against the appropriate library.  In the case of Pari, we interface directly with the Pari library from C.  For 64-bit integers, Pari \texttt{GEN} objects are created using \texttt{stoi}, while for 128-bit integers we use \texttt{mkintn(4, ...)}.  All pairs of inputs integers are first converted to Pari \texttt{GEN} objects.  Pari timings only include calls to \texttt{bezout}.  Garbage collection in Pari is performed by recording Pari's stack pointer (\texttt{avma}) and then restoring it at the end of each batch timing.  In each case, we statically linked with the corresponding library.  Figure \ref{fig:gcdRefImpl} shows the results.

\mygraph{reference-impl}{fig:gcdRefImpl}{Reference Implementation of Extended GCD.}

Pari is faster than GMP and MPIR for input integers no larger than 66-bits, at which point GMP is faster.  Pari's timings for input integers larger than 66-bits is very similar to GMP's timings.  This is likely because Pari was compiled to take advantage of GMP and only specializes the GCD implementation for input integers that fit within a single machine word, i.e.\ 64-bits.

\subsection{Extended Euclidean GCD}

Figures \ref{fig:divrem32v64} and \ref{fig:divrem64v128} show the performance of our extended Euclidean GCD.

\mygraph{divrem-32v64}{fig:divrem32v64}{Extended Euclidean Algorithm (32-bit vs 64-bit).}
\mygraph{divrem-64v128}{fig:divrem64v128}{Extended Euclidean Algorithm (64-bit vs 128-bit).}

\clearpage


\subsection{Right-to-Left Binary Extended GCD}

\subsection{Left-to-Right Binary Extended GCD}

\subsection{Lehmer's Extended GCD}

\subsection{32-bit Implementations}

\subsection{64-bit Implementations}

\subsection{128-bit Implementations}
...
The software used to generate timing information was developed using \compiler{} on Ubuntu 12.10 and a \cpu{} with 8Gb of RAM.  Never more than one of the four cores was used for experiments.
