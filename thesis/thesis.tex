...
% NOTE: 250,000 was as big as could fit in memory and be processed in a reasonable amount of time.
...
This is evidence that the exponent $E$ for the exponentiation stage can be chosen with $e_i = 1$ for $i \ge 5$ and $e_2 = 4$, $e_3 = 2$, and $e_4 = 2$, and that with high probability, the order of the resulting ideal, after exponentiation, will be coprime to $E$.

To provide additional evidence of this position, Table \ref{tab:primePowerBound} represents the probability that an ideal class $\aclass$ associated with an integer $N$ of $n$ bits, with order $\ord(\aclass) = \prod {p_i}^{e_i}$ for primes $p_i$ is such that $e_i \le \floor{\log_{p_i} B}$ for primes $3 \le p_i \le 11$ and $e_i = 1$ for all $p_i > 11$\footnote{For the prime 2, the exponent $e_1 \le \floor{\log_2 h_\Delta}$ is bound by the class number since this is handle separately when searching for ambiguous ideal class.}.  In other words, this table represents the probability that choosing some exponent $E$ with the above constraints implies that the ideal class $\bclass = \aclass ^E$ will have order $\ord(\bclass)$ coprime to the exponent $E$.
...
Since the goal is to improve the performance of SuperSPAR in practice, bounds on the exponents of the prime powers in the 

Since in practice, the largest primorial used for the search phase of SuperSPAR is $P_5 = 2 \times 3 \times 5 \times 7 \times 11$ (see Section \ref{sec:ssparEmpirical}), Table \ref{tab:primePowerBound} uses prime power bounds for primes up to 11.  This table provides evidence that even smaller exponents than those suggested by Schnorr and Lenstra will be sufficient with high probability.

Primorial 45, Steps 40. 
\begin{table}[h]
\centering
\begin{tabular}{| r | r |}
	\hline
	$E'$ & Milliseconds \\
	\hline
      $3^3 5^2 7^2 11^1$ & 1.86944 \\
      $3^4 5^2 7^2 11^1$ & 1.83744 \\
      $3^4 5^2 7^2 11^2$ & 1.83737 \\
      $3^4 5^3 7^2 11^2$ & 1.80073 \\
      $3^5 5^3 7^3 11^2$ & 1.77602 \\
      $3^5 5^4 7^3 11^2$ & 1.78133 \\
      $3^6 5^4 7^3 11^2$ & 1.78861 \\
      $3^7 5^4 7^3 11^2$ & 1.79033 \\
      $3^7 5^4 7^4 11^2$ & 1.79266 \\
      $\cdots$ & $\cdots$ \\
      $3^{59} 5^{40} 7^{33} 11^{27}$ & 2.54599 \\
	\hline
\end{tabular}
\caption{Average time to factor a semiprime integer $2^{63} \le N < 2^{64}$ using the exponent $E = E'\prod_{6 \le i \le 41} p_i$ for the exponentiation stage, and the primorial $2 \times 3 \times 5 \times 7$ with a multiplier of 6 for the search stage.}
\label{tab:actualPrimePowerBound}
\end{table}

% TODO: Roughly 99.2\% of all factorizations of the order have $11^{e_5}$ with $e_5 \le 1$.  Do this for 3, 5, and 7?

% TODO: Section \ref{sec:ssparEmpirical} shows that selecting exponents $E = \prod {p_i}^{e_i}$ such that $e_i = \max \{1, \floor{\log_{p_i} 49} \}$ performs slightly better in practice.

\subsection{Difference between the Order of Two Ideal Classes}
\label{subsec:ssparDiffBtwOrder}

% TODO: Show how repeatable the order is if the exponent $E$ is chosen to remove all factors $\le B$ for some bound $B$.

\newcommand{\idelta}{\mathcal I_\Delta}

This Subsection assumes that an exponent $E$ is chosen such that for each ideal class in our data set, the order of the ideal class $\aclass^E$ is coprime to $3 \times 5 \times 7 \times 11$.  For each bit size $n$, fix the integer to be factored $N$ and the multiplier $k$. Let $\idelta$ be the subset of ideal classes associated with the discriminant $\Delta = -kN$ or $\Delta = -4kN$ from our data set.  Then for each pair of distinct ideal classes $\aclass, \bclass \in \idelta$, there exists some set $\mathcal P = \{ p ~:~ p \textrm{ is prime}, p > 11 \}$ such that $\ord(\bclass^E)$ divides $\ord(\aclass^E) \cdot \prod_{p \in \mathcal P} p$.  Let $x = |\mathcal P|$.  For each $x \ge 0$, sum the number of pairs such that $\ord(\bclass^E)$ divides $\ord(\aclass^E) \cdot \prod_{p \in \mathcal P} p$ and $|\mathcal P| = x$.  This is done for each multiplier $k$ and each integer $N$, and the sum is divided by the total number of ideal class pairs.  The result is the probability that if $h'$, a multiple of the order of an ideal class $\aclass \in \idelta$ is known, that the order $h''$ of the ideal class $\bclass^{Eh'}$ will have exactly $x$ prime factors greater than 11.  In some sense, this is a measurement of the utility that a multiple of the order of one ideal class has towards finding the order of some other ideal class in the same group.  Table \ref{tab:diffOfOrderOfTwoIdeals} shows the results of this calculation.
...
TODO: Table showing that if we assume an exponent is known that removes all factors $\le B$, then the order of some ideal class is a multiple of the order of some other ideal class with what probability?
