...
This chapter discussed methods that lead to practical performance improvements of arithmetic in the ideal class group, including that of computing the extended greatest common divisor.  Having a practical implementation of arithmetic in the ideal class group, the next Chapter, discusses several methods for exponentiating that we have not discussed previously -- many of which take the average time to multiply, square, and cube into consideration.
...
One motivation for our implementation of ideal class group arithmetic is to improve the performance of exponentiation within this group.  In Chapter \ref{chap:powExperiments}, we compare methods to exponentiate ideal representatives to large fixed exponents that are the product of many primes, including several novel approaches to computing double-base representations.


Improvements to ideal arithmetic also lead to improvements in ideal exponentiation, and so we start there. 
