...
\begin{algorithm}[h]
\caption{Try to Find an Ambiguous Ideal Class.}
\begin{algorithmic}[1]
\Procedure{TryAmbiguous}{$\bclass, \ell$}
\For{$i$ from 1 to $\ell$ while $\bclass \neq \idclass$}
	\If{$\bclass$ is an ambiguous ideal}  \Comment{See Subsection \ref{subsec:forms}.}
...
% -- Try Several Ideal Classes -- %
\begin{algorithm}[h]
\caption{Try Several Ideal Classes.}
\begin{algorithmic}[1]
\Procedure{TrySeveralIdealClasses}{$\Delta$, $h$, $\ell$, $c$}
\For{$2 \le i \le c$}
	\State let $\idealclass{p_i}$ be the $i^{\textrm{th}}$ smallest prime ideal class in $Cl_\Delta$
	\State $(\_, d) \gets$ \Call{TryAmbiguous}{$\idealclass{p_i}^h$, $\ell$}
	\If{$d \neq 1$}
		\State \Return d
	\EndIf
\EndFor
...
\If{$\Delta \not\equiv 0,1 \pmod 4$}
	$\Delta \gets 4\Delta$
\EndIf
%\State lazily compute prime ideal classes $\idealclass{a_1}, \idealclass{a_2}, ..., \idealclass{a_c} \in Cl_\Delta$ only as needed
...
\State $(\idealclass{b}, d) \gets$ \Call{TryAmbiguous}{$\idealclass{b'}$, $\ell$}
\If{$d \neq 1$}
	\Return d
\ElsIf{$\bclass = \idclass$}
	\State $d \gets$ \Call{TrySeveralIdealClasses}{$\Delta$, $E$, $\ell$, $c$}
	\If{$d \neq 1$}
		\Return d
	\Else
		~start over at line \ref{goto:newK}
	\EndIf
\EndIf
...
\State $(\_, d) \gets$ \Call{TryAmbiguous}{$\idealclass{b'}^h$, $\ell$}
\label{goto:foundOrder}
\If{$d \neq 1$} \Return $d$	\EndIf	
\State $d \gets$ \Call{TrySeveralIdealClasses}{$\Delta$, $E \cdot h$, $\ell$, $c$}
\If{$d \neq 1$} \Return $d$ \EndIf
