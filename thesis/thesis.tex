...
\caption{The construction of $2^{15} 3^6 - 2^8 3^5 - 2^4 3^6 + 2^3 3^3$.}
\label{fig:yao1}
\end{figure}
...
\mygraph{search-48-bits-primorial-x}{fig:search48PrimorialX}{The average time to split 48-bit semiprime integers when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated for a fixed search bound.}
\mygraph{search-56-bits-primorial-x}{fig:search56PrimorialX}{Average time to split 56-bit semiprime integers when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated for a fixed search bound.}
\mygraph{search-64-bits-primorial-x}{fig:search64PrimorialX}{Average time to split 64-bit semiprime integers when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated for a fixed search bound.}

\mygraph{search-48-bits-steps-x}{fig:search48StepX}{Average time to split 48-bit semiprime integers when the search bound index is iterated for fixed exponent.}
\mygraph{search-56-bits-steps-x}{fig:search56StepX}{Average time to split 56-bit semiprime integers when the search bound index is iterated for fixed exponent.}
\mygraph{search-64-bits-steps-x}{fig:search64StepX}{Average time to split 64-bit semiprime integers when the search bound index is iterated for fixed exponent.}

\mygraph{search-primorial-x-steps-min}{fig:searchStepsMin}{The best average time of all search bounds tested when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated.}
\mygraph{search-primorial-min-steps-x}{fig:searchPrimorialMin}{The best average time of all exponents tested when the search bound is iterated.}
...
To find values for the exponentiation stage and search stage that work well in practice, one could iterate all suitable exponents $E$ and multiples of primorials $mP_w$, but that might conceivably take a long time for even a small sample set of integers.  The Figures in the previous section suggest a more efficient approach, given by Algorithm \ref{alg:noisyCurveSearch}.

\begin{algorithm}[h]
\caption{Determine some $x$ such that $f(x)$ is suitably small where $f(x)$ represents a noisy curve.}
...
Table \ref{tab:ssparParams} shows the values used for the exponentiation and search stage in practice for various sized integers.  The range of exponents for the exponentiation stage was from a largest prime of 11 in the exponent to a largest prime of 3673.  The range for search bounds was given by the set $S_{16384}$.  Algorithm \ref{alg:noisyCurveSearch} was configured to switch to a linear search when the size of the range was $\le 8$.  Notice that the largest primorial used for search bounds is $P_5$.  This justifies our approach to bounding the values $e_i$ in the exponent $E = \prod {p_i}^{e_i}$ for values of $i \le 5$.
...
\begin{tabular}{| r | r | r |}
	\hline
Bit Size & Largest Prime in $E$ & Search Bound \\
	\hline
	\hline
\end{tabular}
\caption{Values for the largest prime used in the exponent $E = \prod {p_i}^{e_i}$ for the exponentiation phase, and the baby step bound $mP_w$ used for the search stage.}
...
\item Pari/GP
\item Custom SQUFOF
\item GNU MSieve (Crashes for 86-bits and up)
\item GNU-ECM  (Fails about 26\% of the time)
\item YAFU
\item Flint
\item Maple
\end{itemize}
...
The work of this thesis was to improve exponentiation in the ideal class group of imaginary quadratic number fields, with an application to integer factoring.  This lead to practical improvements to extended GCD computations for bounded integers (Section \ref{sec:eea}) as well as improvements to ideal class arithmetic for bounded discriminants (Section \ref{sec:idealArithmetic}).  Chapter \ref{chap:powExperiments} lead to practical performance improvements when exponentiating an ideal class with bounded discriminant to a large primorial.  Finally, this thesis revisits the SPAR factoring algorithm to bring together all the practical improvements as well as the asymptotically faster primorial steps algorithm.  Section \ref{sec:ssparComparison} compares our implementation of SuperSPAR to several integer factoring libraries available and shows that the performance improvements of this thesis lead to SuperSPAR being the fastest implementation for integers in the range of 54-bits to 64-bits (TODO).

For the work of this thesis to be most useful to the future work of others, the source code for the libraries used throughout is available online, and the hope is for eventual inclusion into common mathematical packages such as Pari/GP, Sage, and Dr. Michael J. Jacobson, Jr.'s personal algorithmic number theory library.
...
The practical improvements to arithmetic in the ideal class group were based on the Algorithms for computing reduced (or almost reduced) representatives for multiplication (Subsection \ref{subsec:nucomp}), squaring (Subsection \ref{subsec:nudupl}), and cubing (Subsection \ref{subsec:nucube}).  While these Algorithms are certainly faster as the size of the discriminant grows, future work could include a comparison of the implementation in this thesis with an implementation of ideal class arithmetic that does not directly compute reduced (or almost reduced) representatives.  For small discriminants, the simplified implementation might prove faster.  Also relevant would be a performance comparison of the implementation presented here with that in Pari/GP and Sage.

For an implementation of ideal class arithmetic based on the non-reduced product of Subsection \ref{subsec:idealMultiply}, the idea would be an optimized implementation as well as improvements to the ideal reduction algorithm from Subsection \ref{subsec:reduction}.  Ideal reduction is similar to the standard Euclidean Algorithm, as such, future work might apply several of the approaches used for computing the extended GCD to that of ideal reduction.  Furthermore, additional techniques for computing the extended GCD can be explored.  A relatively straightforward approach is to apply windowing to the left-to-right binary GCD that performed well for 32-bit to 64-bit integers.  Binary GCD computations favour bit shifting over multiplication and division since bit shifting is faster.  In groups where squaring and cubing are fast, an interesting combination to explore would be a right-to-left 2,3 GCD, similar perhaps to the right-to-left 2,3 chain of Section \ref{sec:rtolChains}.

Finally, additional exploration is possible for other types of ideal arithmetic, such as the ideal ring of algebraic integers of real quadratic fields, or function fields.
...
Number systems with three, four, or more bases.
...
Histogram of the number of multipliers used.

Rigorous Analysis of the Factorization of the Order of Ideal Classes
