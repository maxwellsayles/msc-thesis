...
A windowed right-to-left binary GCD algorithm trades off precomputation of $2^k$ multiples of $a$ and $b$ with the likelihood that $t$ will be divisible by some power of two. In practice, we chose a value of $k=3$.  Figure \ref{fig:blockbinary} shows the timings for values of $k$ between 1 and 5 inclusive.  Notice that a 3 bit window performs best in practice when each input argument is 14 bits or larger.
