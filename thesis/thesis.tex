...
Recall how to test for an ambiguous ideal, recall how to split $N$ with an ambiguous ideal, recall how to hash an ideal and its inverse.  Talk about building up the $\idealclass{d_{2i}}$.  Talk about how the last baby step is $s+1$.
...
\algnewline $s = mP_w$ a multiple of a primorial for the search phase, and $\phi(P_w)$,
\algnewline $c \in \ZZgtz$ the number of ideal classes to try before switching multipliers,
\algnewline a coprime delta list $\coprimelist_w = \delta_1, \delta_2, ..., \delta_{\phi(P_w)}$ for the primorial $P_w$ (see Subsection \ref{subsec:coprimeWheel}),
\algnewline $\delta_{\textrm{max}} = \max \coprimelist_w$.
