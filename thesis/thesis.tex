...
In Stage 1 of SPAR, we take the first $t$ primes $p_i \le N^{1/2r}$ for $r = \sqrt{\ln N / \ln \ln N}$, and let $e_i = \max \{ v : {p_i}^v \le {p_t}^2 \}$ (TODO: Should this be $N$ or $\Delta$?).  We then compute
\[
	\bclass = \aclass^{\prod_{i=2}^t {p_i}^{e_i}},
\]
for a random ideal class $\aclass$ and $\cclass = {\bclass}^{\left(2^k\right)}$ for $k=\floor{\log_2 {h_\Delta}}$ (where $h_\Delta$ is given by Equation \ref{eq:hDelta}).  The purpose of Stage 1 is to find an ideal class $\bclass$ whose order is likely not have any of the odd primes $p_2, ..., p_t$ as divisors.  If the odd part of $\ord(\bclass)$ divides $\prod_{i=2}^t {p_i}^{e_i}$ and the order of $\bclass$ is even, then computing $\cclass$ produces an ambiguous ideal and we can attempt to factor the discriminant $\Delta$.

Failing this, Stage 2 attempts to find the order of $\cclass$ by performing a random walk on the cyclic group that it generates.  Schnorr and Lenstra \cite[p.294]{Schnorr1984} suggest a Pollard-Brent Recursion \cite{Brent1980} based on Pollard's rho method \cite{Pollard1975}.  This approach will yield $\ord(\cclass)$ within $\sqrt{\pi/2}p_t$ steps if $\ord(\cclass) \le {p_t}^2$ and requires only a constant number of group elements.  They also remark \cite[p.298]{Schnorr1984} that Shanks' baby-step giant-step method \cite{Shanks1971} could be used to deterministically find $\ord(\cclass)$ within $\sqrt{2}p_t$ group operations when $\ord(\cclass) \le {p_t}^2$, but requiring $O(p_t)$ storage.  They point out that this approach can be made faster by exploiting the fact that $\ord(\cclass)$ is likely to have no prime divisors $p_1, ..., p_t$, and this leads us to the SuperSPAR factoring algorithm.

\subsection{SuperSPAR Algorithm}

SuperSPAR works by finding an ambiguous ideal that leads to the factorization of its discriminant, and we find an ambiguous ideal by applying the relevant parts of the primorial steps algorithm.  Let $N$ be the odd integer we wish to factor.  We start with a square free integer $k$ and choose a discriminant $\Delta = -kN$ or $\Delta = -4kN$ such that $\Delta \equiv 0, 1 \pmod 4$.  We then find a random prime ideal class representative $\aclass \in Cl_\Delta$.  If we were to compute $h = \ord(\aclass)$ and $h$ was even, we could compute an ambiguous ideal $\aclass^{h/2}$, but this is more work than necessary -- all we need is the ambiguous ideal and not the order.

To use the bounded primorial steps algorithm, we need some bound $M$ on the order of the element.  Cohen \cite[p.295]{Cohen1993} states a bound on the number of elements in the class group $Cl_\Delta$ as
\begin{equation}
\label{eq:hDelta}
	h_\Delta < \frac{1}{\pi} \sqrt{|\Delta|}\log{|\Delta|} \textrm{ when } \Delta < -4.
\end{equation}
Let this be our bound $M$ on the order of $\aclass$.  Following the primorial steps algorithm, we compute the largest primorial $P_n$ such that $P_n \le \sqrt M$.  Next, we maximize $m$ such that $m^2P_n\phi(P_n) \le M$, and set $b = mP_n$.

If we wanted to determine the order of $\aclass$, following the primorial steps algorithm we would compute $E = \prod_{i=1}^n {p_i}^{\floor{\log_{p_i} M}}$ and $\bclass = \aclass^E$.  However, since we only want to find an ambiguous ideal, we instead only use the \emph{odd} primes $p_i \le p_n$ when computing $E$ and $\bclass = \aclass^E$.

Similar to the original SPAR algorithm, we then compute $\cclass = \bclass^{\left(2^k\right)}$.  In this case, we use the bound on the class group $h_\Delta$ from Equation \ref{eq:hDelta} such that $k = \floor{\log_2 h_\Delta}$.  Now we have an ideal class $\cclass$ whose order is known not to have any of the primes $p_i \le p_n$ as divisors.  We compute the order of $\cclass$ by taking baby steps $\cclass^i$ for $1 \le i \le b$ where $i$ is coprime to $P_n$, and giant steps $\cclass^j$ for $j=2b, -2b, 4b, -4b ...$\footnote{We use this sequence of giant steps since computing the inverse in the ideal class group is essentially free.}.  Having computed the order $h' = \ord(\cclass)$, we compute an ambiguous ideal by repeated squaring of $\dclass = \bclass^{h'}$ assuming that $\ord(\bclass)$ is even.  This works since $\bclass = \aclass^E$ and $E$ has no factors of 2.  We have 
\[
