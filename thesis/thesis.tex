...
\]
for some quadratic irrational $\alpha \in \CC$.  Any quadratic field can be written as $\QQ(\sqrt D)$ for some square-free integer $D$ \cite[p.188]{Ireland1990}, and $D$ is called the \emph{radicand} of the quadratic field $\QQ(\sqrt D)$ \cite[p.10]{Ramachandran2006}.  When $D < 0$, then $\QQ(\sqrt D)$ is a subset of the complex numbers and we call $\QQ(\sqrt D)$ an \emph{imaginary} quadratic number field.  When $D > 0$, then $\QQ(\sqrt D)$ is a subset of the real numbers and we call $\QQ(\sqrt D)$ a \emph{real} quadratic number field.  The assumption throughout this thesis is that $D$ is negative.
...
\[
	\omega_1 \ZZ + \omega_2 \ZZ = \{ x\omega_1 + y\omega_2 : x, y \in \ZZ \}.
\]
...
Then by Ramachandran \cite[p.13]{Ramachandran2006}, the discriminant $\Delta$ of the maximal order $\OO_\KK = [1, \omega]$ can be written as
\[
\Delta = \begin{cases}
...
\end{cases}
\]
and is an invariant of the quadratic field $\QQ(\sqrt D)$.  A discriminant of a maximal order is called a \emph{fundamental discriminant}.
