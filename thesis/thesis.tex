...
\]
Now the coefficients of the partial extended GCD represent solutions to the equation
\[
	R_i = -C_i a + D_i b.
\]
...
Each of the operations performed on the matrix
\[
\matrixtt{R_{i-1}}{C_{i_1}}{R_i}{C_i}
\]
during the partial extended GCD must be representable by an invertible $2 \times 2$ matrix with determinant $\pm 1$ (known as a \emph{unimodular} matrix).  This is because the resulting coefficients are applied to an ideal in order to (partially) reduce it, and so it must have determinant $\pm 1$ in order to stay in the same equivalence class.

This is critical since only extended GCD computations that are restricted to operations representable using invertible $2 \times 2$ matrices can be used for the partial extended GCD.

% TODO: It is due to the defn of equivalence of forms (and thus ideals).  Recall that two forms are equivalent if one can be obtained from another via a change of variables represented by a 2x2 matrix of det +1 or -1.  All the matrix stuff in NUCOMP is intended to be applied to a form/ideal in order to reduce it, so it has to have det +1 or -1 in order to stay in the same equivalence class.
