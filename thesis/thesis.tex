...
% TODO: Justify why we can drop the first term below.

Equations \ref{eq:idealProductS}, \ref{eq:idealProductA}, and \ref{eq:idealProductB} from the previous subsection give a solution to the ideal product $\mathfrak a \mathfrak b = s[a, (b+\sqrt\Delta)/2]$.  We begin with the observation of Jacobson and Williams (\cite[p.119]{Jacobson2009}) that the fraction $(b/2)/a$ is roughly equal to the fraction $sU / a_1$ where $U$ is given by Equation \ref{eq:idealProductU}:
\[
...
\begin{align}
	q_i &= \floor{R_{i-2} ~/~ R_{i-1}} \nonumber \\
	R_i &= R_{i-2} - q_i R_{i-1} \nonumber \\
	C_i &= C_{i-2} - q_i C_{i-1} \label{eq:cNUCOMP}
\end{align}
...
\]
Finally, we compute the representative $\mathfrak a^3 = [a, (b + \sqrt\Delta)/2]$ using the equations
\begin{align*}
