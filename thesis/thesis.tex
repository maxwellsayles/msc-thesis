...
In practice, the efficiency of the SuperSPAR factoring algorithm depends on the smoothness of the order of the ideal class $\aclass$ with respect to the primorial $E$ chosen.  The larger the divisor that $\ord(\aclass)$ and $E$ have in common, the smaller the order of $\cclass = \aclass^{\left(2^k\right)E}$.  Since the order of each ideal class in $Cl_\Delta$ depends on the class number of $Cl_\Delta$, changing square free multipliers $k$ for the discriminant $\delta$ also changes the order of random elements in the new group.  As such, we can trade off the number of ideal classes or multipliers $k$ against the size of the bound $M \le h_\Delta$ that we assume.


Schnorr and Lenstra \cite[p.293]{Schnorr1984} advise smaller exponents $e'_i = \max \{v : {p_i}^v \le p_t \}$ for the first $t$ primes $p_i \le N^{1/2r}$.

TODO: describe all the steps of the implemented algorithm, without saying the exact values of things, just that we determine them empirically in Chapter \ref{chap:superSparExperiments}
