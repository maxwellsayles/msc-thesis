...
Suppose the search stage of SuperSPAR determines $h'$, a multiple of the order of an ideal class $\bclass = \aclass^{2^\ell E}$ for $\ell = \floor{\log_2 h_\Delta}$.  Then for some other ideal class $\idealclass{a'}$ in the same group, there exists a set of primes $\mathcal P$ such that the order of $\idealclass{a'}^{2^\ell E}$ divides $h' \prod_{p \in \mathcal P} p$, where the product of the empty set is taken to be 1.  Also suppose that an exponent $E$ was chosen for the exponentiation phase such that $\ord(\aclass^E)$ is coprime to $E$.  Since in practice our implementation of SuperSPAR does not step coprime to a primorial larger than $2 \times 3 \times 5 \times 7 \times 11$ (see Section \ref{sec:ssparEmpirical}), this Subsection assumes that the exponent $E$ is chosen such that for each ideal class in our data set, the order of the ideal class $\aclass^{2^\ell E}$ is coprime to $2 \times 3 \times 5 \times 7 \times 11$.  Assuming the above constraints on our data set, for two ideal classes from the same group $\idealclass u, \idealclass v \in Cl_\Delta$, Table \ref{tab:diffOfOrderOfTwoIdeals} shows the probability that the smallest set of primes $\mathcal P = \{ p ~:~ p \textrm{ is prime}, p > 11 \}$ with $\ord({\idealclass v}^{2^\ell E})$ dividing $\ord({\idealclass u}^{2^\ell E}) \cdot \prod_{p \in \mathcal P} p$ is of a given size.
...
80 & 0.97781 & 0.02191 & 0.00028 & 0.00000 \\
	\hline
\end{tabular}
\caption{If $h'$, a multiple of the order of an ideal class $\idealclass u$ is known, this table shows the probability that the order of the ideal class ${\idealclass v}^{h'}$, for some other ideal class $\idealclass v$ in the same group, has exactly $|\mathcal P|$ prime factors larger than 11.}
