...
\end{array}
\end{equation*}
where $\floor{\log_2x+1}$ is the number of bits in the binary representation of $x$.  Using this approximation, we compute the integer square root of $x$ as roughly $\floor{x / 2^{\floor{\log_2x+1}/2}}$.  This is simply a bit shift right by half the number of bits in $x$.
...
q   &= q' - (d \band q_m) & \textrm{\{adjust $q$ with respect to $r$\}} \\
b'  &= r
\end{align*}
as an equivalent step to computing $b'$ in the ideal reduction algorithm. We note here that instead of computing a new value for $c = (b^2 - \Delta)/4a$, we can compute the new value from $q$ and $r$ above as $c - (b + r)q/2$.

For GMP, we preallocate the memory needed to save time on memory allocation/releases.
...
Precompute square roots modulo a prime.  Useful for finding prime ideals. For a given prime $p$, let $a = p$, then $b = \pm \sqrt\Delta \pmod p$ if such a $b$ exists.   We want $b^2 - 4ac = \Delta$.  We have $b^2 - 4pc = \Delta$.  Take everything in the residue class modulo $p$ and we have $b^2 = \Delta \pmod p$.  That where it comes from.
