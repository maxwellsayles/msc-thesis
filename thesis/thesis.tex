...
Section \ref{sec:gcdEEA} discuses the standard extended Euclidean Algorithm.  The core of the extended Euclidean Algorithm is used in Lehmer's extended GCD in Section \ref{sec:gcdLehmer}.  Both the extended Euclidean algorithm and Lehmer's extended GCD use division and multiplication, which can be costly.  Binary extended GCD computations emphasize bit shifting in place of multiplication and division.  Section \ref{sec:gcdR2LBin} discusses an extended GCD algorithm that works from low-order to high-order, referred to as \emph{right-to-left} since this is the order in which bits are operated on in the binary representation. Section \ref{sec:gcdR2LWindowed} discusses the windowed variant of this algorithm.  An extended GCD can also be computed in binary from high-order to low-order, also referred to as \emph{left-to-right}, and is discussed in Section \ref{sec:gcdL2R}.
