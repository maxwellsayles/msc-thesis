...
\]
and perform the extended Euclidean algorithm on the left hand side.  Invariants of the partial extended GCD are that $R_{i-1} \ge R_i$ and from \cite[p.5]{Imbert2010} that
\[
	C_i = (-1)^{i+1}B_i.
\]
Since the coefficients $B_i$ are never used, our implementation of the partial extended GCD does not compute them.  This is similar to the case of computing the extended GCD in Equation \ref{eq:idealProductS}, since the coefficient $Y$ is never used.
