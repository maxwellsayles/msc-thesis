...
Subsections \ref{subsec:nucomp}, \ref{subsec:nudupl}, and \ref{subsec:nucube} describe algorithms to compute reduced or almost reduced ideal class multiplication, squaring, and cubing respectively.  Each of these techniques involve computing a simple continued fraction, which is done using an extended GCD computation (see \cite[\S 3.2]{Jacobson2009}).  Unlike the extended GCD algorithms previously described in this chapter, the extended GCD computation representing the simple continued fraction expansion used in ideal multiplication terminates once the next remainder computed is within a given bound.  As such, we refer to this computation as the \emph{partial extended GCD}.

In the description of NUCOMP, NUDUPL, and NUCUBE, we use the variables $R_i$ and $C_i$.  

the partially reduced coefficients of ideal representatives for multiplication, squaring, and cubing respectively.  In each case, a continued fraction expansion is computed using the recurrences
\begin{align*}
