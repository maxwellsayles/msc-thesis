...
Two parameters remain to be determined for our implementation of SuperSPAR.  The first is the largest prime used in the exponent $E = \prod {p_i}^{e_i}$ for the exponentiation stage, and the second is a multiple of a primorial for use with the search stage.  This Section begins by graphically displaying the timing trends to split random semiprime integers of three different bit sizes when the largest prime in the exponent $E$ or the bound on the search phase is iterated.  Based on these trends, an algorithm is then proposed to find the largest prime in the exponent $E$ and the bound on the search stage that works well in practice for random semiprime integers of a fixed bit size.

In selecting the exponent $E$ for the exponentiation stage, our implementation uses the results of Chapter \ref{chap:exponentiation}.  Section \ref{sec:expBestApproximations} introduces the $L$ best approximations technique for generating 2,3 representations of an integer, for which Section \ref{sec:expResults} shows that this method performs best in practice when exponentiating an ideal class to the product of many prime numbers.  This is the method used in our implementation of SuperSPAR.  In practice, for a given exponent, the bound $L$ was chosen to be $L=2^k$ for some value $k$.  The bound was repeatedly doubled and an $L$ best approximation was computed for a given exponent until the cost of exponentiating an ideal class to that particular exponent remained fixed between iterations.  For the exponents tested in this section, the bound was $L \le 2048$.

When selecting a multiple of a primorial for the search stage, candidates are selected from the list generated by Algorithm \ref{alg:stepBounds}.  For the purposes here, the candidate list was bound such that no candidate used more than 16384 total steps for the search stage.

To indicate the effect of iterating the largest prime in the exponent $E$ or the index of the search bound, three sets of 10,000 random semiprime integers $N = p \cdot q$ were generated for $p$ and $q$ prime and half the size of $N$.  The three sets consist of integers of 48, 56, and 64 bits.  The largest prime in the exponent was iterated from 13 to 947, and the search bound index was iterated from the first index up to the $\textrm{120}^{\textrm{th}}$ index.  Figures \ref{fig:search48PrimorialX}, \ref{fig:search56PrimorialX}, and \ref{fig:search64PrimorialX} show the average time to split integers as the largest prime in the exponent is iterated.

\mygraph{search-48-bits-primorial-x}{fig:search48PrimorialX}{The average time to split 48-bit semiprime integers when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated for fixed search bound.}
\mygraph{search-56-bits-primorial-x}{fig:search56PrimorialX}{Average time to split 56-bit semiprime integers when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated for fixed search bound.}
\mygraph{search-64-bits-primorial-x}{fig:search64PrimorialX}{Average time to split 64-bit semiprime integers when the largest prime in the exponent $E = \prod {p_i}^{e_i}$ is iterated for fixed search bound.}

Figures \ref{fig:search48StepX}, \ref{fig:search56StepX}, and \ref{fig:search64StepX} show the average time to split integers as the search bound index is iterated.
...
Finally, figure \ref{fig:searchStepsMin} shows the best average time of all search bounds tested when the largest prime in the exponent is iterated, and figure \ref{fig:searchPrimorialMin}  shows the best average time of all exponents tested when the search bound index is iterated.
...
\begin{algorithmic}[1]
\REQUIRE A function $f(x)$ corresponding to the average time to split an integer, a search range $[\xmin, \xmax]$, and a bound $k$ for linear searching.
\ENSURE Some value of $x \in [\xmin, \xmax]$ such that $f(x)$ is suitably small.
\STATE $(\xlo, \xhi) \gets (\xmin, \xmax)$
\WHILE{$\xhi - \xlo > k$}
	\STATE $\xonethird \gets \floor{(\xhi - \xlo) / 3}$
	\STATE $\xtwothird \gets \floor{2(\xhi - \xlo) / 3}$
	\STATE $\yonethird \gets f(\xonethird)$
	\STATE $\ytwothird \gets f(\xtwothird)$
\ENDWHILE
\RETURN $x$
\end{algorithmic}
\end{algorithm}
