...
% TODO: Is the actual running time $O(\sqrt{M / \log \log M})$ ?
...
The bounded primorial steps algorithm from Section \ref{sec:primorial} and used by the SuperSPAR factoring algorithm (Section \ref{sec:superSpar}) computes baby-steps $\aclass^i$ for $1 \le i \le b$ where $i$ is coprime to some primorial $P_w$.  This section discusses three methods used in practice: sieving, wheeling, and precomputed deltas.  Our implementation of the SuperSPAR factoring algorithm uses precomputed deltas to compute values of $i$ for the coprime baby steps.
...
$\phi(P_w)$ deltas $d_1, d_2, ...$ such that $d_i = c_{i+1} - c_i$ for consecutive values $c_i$ and $c_{i+1}$ coprime to $P_w$ where $c_1 = 1$, \\
and the next prime $p_{w+1}$. \\
Let $P_{w+1} = p_{w+1} P_w$ and $\phi(P_{w+1}) = (p_{w+1}-1)\phi(P_w)$.
\ENSURE $\phi(P_{w+1})$ deltas $d'_1, d'_2, ...$ such that $d_j' = c'_{j+1} - c'_j$ for consecutive values $c'_j$ and $c'_{j+1}$ coprime to $P_{w+1}$ where $c'_1 = 1$.
...
\subsection{Precomputed Coprime Deltas}
