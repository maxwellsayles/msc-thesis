...
One fast order finding algorithm $\mathcal A(\alpha^{h'}, E)$ uses the factorization of $E=\prod {p_i}^{e_i}$.  Notice that ${\alpha^{h'}}^E = \beta^{h'} = 1_G$.  The idea is to iterate on the factors of $E$, removing each factor $p$ before computing $\gamma = {\alpha^{h'}}^{E'}$ for the product $E' = E/p$. If $\gamma = 1_G$, then $\ord(\alpha) ~|~ h'E'$ and we continue with the next factor in the product $E'$.  Otherwise, $p$ is a factor of $\ord(\alpha)$ and we continue with the next factor of $E$.  There are asymptotically faster algorithms to compute this (see Chapter 7 from \cite{Sutherland2007}), however, our interest is primarily in finding an ambiguous ideal that leads to the factorization of the discriminant.

\subsection{Primorial Steps for a Set of Elements}

Suppose the success of a computation is not limited to computing the order of a single element, but that the order of any element from a set of groups will work.  Let $\{ \alpha_i \in G_i \}$ be a set of elements from different groups such that the order of $\alpha_i$ is distributed uniformly at random on the interval $[1, U]$.  When the order of any element $\alpha_i$ from the set will suffice, Sutherland gives an algorithm with subexponential complexity.

Recall that an integer $x$ is $y$-smooth if none of its prime factors are larger than $y$.  By \cite[p.81]{Sutherland2007}, the probability that a random integer $x$ is $x^{1/u}$ smooth is $u^{-u+o(1)}$.  As such, the idea is to pick some $u$ and attempt to compute $\ord(\alpha_i)$ using at most $O(M^{1/u})$ group operations (where $M$ is an estimate of the group order), and then to try again for the next $\alpha_{i+1}$ in the set.  Using this approach, the expected running time by \cite[p.81, 82]{Sutherland2007} is approximately
\[
