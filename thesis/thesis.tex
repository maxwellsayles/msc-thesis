...
There are many interesting problems related to the study of algebraic number fields, and since quadratic number fields are the simplest of algebraic number fields \cite[p.77]{Jacobson2009}, they provide a good starting point for the study of ideals and class groups.  This thesis discusses practical improvements to exponentiation in the ideal class group of imaginary quadratic number fields.  Quadratic number fields are a degree 2 extension over the rational numbers, with the form $\KK = \QQ(\sqrt\Delta)$.  When the discriminant $\Delta$ is negative, then $\KK$ is a subset of the complex numbers and we call $\KK$ an \emph{imaginary} quadratic number field.  When $\Delta$ is positive, then $\KK$ is a subset of the real numbers and we call $\KK$ a \emph{real} quadratic number field.  
...
\]
for some quadratic irrational $\alpha \in \CC$.  The element $\alpha$ must be a root of a quadratic polynomial $ax^2 + bx + c$ with integer coefficients.  Following \cite[p.188]{Ireland1990}, let $A = b^2-4ac$, then $\QQ(\sqrt A) = \QQ(\alpha)$, and when $A = f^2 D$ for integers $f$ and $D$ with $D$ square-free, then $\QQ(\sqrt D) = \QQ(\alpha)$.  When $D < 0$, then $\QQ(\sqrt D)$ is a subset of the complex numbers and we call $\QQ(\sqrt D)$ an \emph{imaginary} quadratic number field.  When $D > 0$, then $\QQ(\sqrt D)$ is a subset of the real numbers and we call $\QQ(\sqrt D)$ a \emph{real} quadratic number field.  The assumption throughout this thesis is that $D$ is negative.
...
\xi = \begin{cases}
	...
	\end{cases}
\end{equation*}
for integers $x$ and $y$.  See \cite[p.189]{Ireland1990} for a proof.
\end{prop}
...
\]
where $\overline{\xi}$ denotes complex conjugation.
\end{defn}
...
All fractional ideals in $\OO_\Delta$ are invertible, and the set of invertible ideals forms a group under ideal multiplication.  We can now define an equivalence relation on the invertible ideals of $\OO_\Delta$.

\begin{defn}
\cite[p.88]{Jacobson2009} Two invertible ideals $\ideal a$ and $\ideal b$ are \emph{equivalent} if there exists $\alpha, \beta \in \OO_\Delta$ such that $\alpha \beta \neq 0$ and $(\alpha)\ideal a = (\beta) \ideal b$.
\end{defn}
...
\end{equation}
where $\mathcal N(\ideal a) = q^2a$ is the norm of $\ideal a$ and is multiplicative.  For a primitive invertible ideal $(a, b)$, the inverse is given by $\frac{1}{a}(a, -b)$.  Let $\aclass$ be an ideal class represented by the primitive ideal $\ideal a$.  The inverse $\idealclass{a^{-1}}$ is then given simply by $(a, -b)$ \cite[p.20]{Ramachandran2006}.  As such, computing the inverse of an ideal class is virtually free. 
