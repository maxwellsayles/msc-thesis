...
\begin{example}
Let $\OO = [1,\sqrt{-1}]$ be the maximal order of $\QQ(\sqrt{-1})$.  By Equation \ref{eq:radicandToDiscriminant},  let $\Delta = -4$.  Now let $\ideal a = [2, (2+\sqrt{\Delta})/2]$ and $\ideal b = [5, (6 + \sqrt{\Delta})/2]$ be reduced ideals of $\OO$.  Multiplication using the above gives
\[
	\ideal a \ideal b = \left[10, \frac{6 + \sqrt{\Delta}}{2}\right].
\]
\end{example}
