...
\]
The multiplier 2654435761 was chosen because it is the largest prime less than $2^{32} \times (\sqrt 5 - 1)/2$, and has been shown by Knuth \cite[Section~6.4]{Knuth1998} to have good scattering properties\footnote{Other multipliers were also tried but had little impact on performance.}.  Since the number of baby steps is a function of the size of the integer to be factored (see Section \ref{sec:ssparEmpirical}), the maximum number of entries in the hash table is known at initialization.  We found that a table of $2^k$ slots for the smallest $k$ such that $2^k \ge 3m\phi(P_2)$, where $m\phi(P_w)$ is the number of baby steps, worked well in practice\footnote{Different multiples of the number of baby steps were tried, as well as setting the number of slots to a prime near some multiple of the number of baby steps. There was little difference in performance, with the exception of a table having too few slots (resulting in an insertion failing), or a table having an excessive number of slots (and table initialization dominated).}.  Using a power of 2 for the number of slots has the added benefit that once a hash key is computed, the corresponding slot is found by using a binary $\band$ operation.

Regardless of the hash function used, multiple elements may hash to the same slot in the table.  Two methods of collision resolution were considered.  The first method is for each slot to consist of the 32-bit value for the specified key, as well as a 32-bit index into an overflow table.  The entry indexed in the overflow table also consists of a 32-bit value for the same key, and another index into the overflow table for the next value of the same key.  This is essentially a linked list, where the head of the list is stored directly in the slot of the hash table (see \cite[Subsection~11.2]{Cormen2001}).  A special constant (0x00000000) is used to indicate that a slot is empty, or that the index into the overflow table should not be followed.

The second method considered is open addressing, and in particular, we implemented the same probing function as in CPython \cite{Kuchling2008}.  In this method, each slot consists of a 32-bit hash key, as well as the 32-bit value associated with that hash key.  Any hash key can occupy any slot, and a special pair of constants (0xAAAAAAAA, 0x55555555) are used to indicate that a slot is unoccupied.  Let the table consist of $2^k$ slots.  When an element is hashed, the index of a slot is computed as
\[
	\textrm{index} = \hash(a, b) \band (2^k-1).
\]

  When two elements hash to the same key, the second element will 
