...
\[
	\QQ(b^2-4ac) = \QQ(\alpha).
\]
Let $f^2 D = b^2-4ac$ for $f, D \in \ZZ$ and $D$ square-free, then $\QQ(\sqrt D) = \QQ(\alpha)$ also.  When $D < 0$, then we call $\QQ(\sqrt D)$ an \emph{imaginary} quadratic number field, and when $D > 0$, we call $\QQ(\sqrt D)$ a \emph{real} quadratic number field.  The assumption throughout this thesis is that $D$ is negative.
...
% TODO: Maybe come up with an example?
...
% TODO: How do we test correctness.  Also what do we mean by "correct"?
...
Figures \ref{fig:expBlockList64} and \ref{fig:expBlockList128} in Section \ref{sec:expResults} show that repeated exponentiation by a series of prime powers is faster on average, at least when exponentiating by power primorials, than exponentiating by the product using 16 bit blocks.
