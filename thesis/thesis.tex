...
\graphicspath{{gcd-eps/}{ideal-eps/}{exp-eps/}{sspar-eps/}{sspar-evolve-eps/}}
...
% TODO(bauer, jacobson): An example for every definition?
...
%\begin{example}
%Let $\OO = [1, 3i]$ for $i^2 = -1$ be a non-maximal order and $\ideal a = \{3x + y i : x, y \in \OO\}$ be an additive subgroup of $\OO$.  Then by Proposition \ref{prop:quadInts}, for some $\xi \in \OO$ we have $\xi = u + 3v i$ for $u,v \in \ZZ$ and for some $\alpha \in \ideal a$ we have $\alpha = 3 \omega_1 + \omega_2 i$ for $\omega_1, \omega_2 \in \OO$.  Since $\omega_1, \omega_2 \in \OO$, there exists $x_1, y_1, x_2, y_2 \in \ZZ$ with $\omega_1 = x_1 + 3 y_1 i$ and $\omega_2 = x_2 + 3 y_2 i$.  Now it follows that
%\begin{align*}
%\xi \alpha &= (u + 3 v i)(3 \omega_1 + \omega_2 i) \\
%&= (u + 3 v i)(3 (x_1 + 3 y_1 i) + (x_2 + 3 y_2 i) i) \\
%&= (u + 3 v i)(3 x_1 + 9 y_1 i + x_2 i - 3 y_2) \\
%&= 3 x_1 u + 9 y_1 u i + x_2 u i - 3 y_2 u
% + 9 x_1 v i - 27 y_1 v - 3 x_2 v - 9 y_2 v i \\
%&= 3 ((x_1 u - 9 y_1 v) + 3(3 y_1 u + 3 x_1 v) i) 
% +   ((x_2 u - 9 y_2 v) + 3(y_2 u - x_2 v) i) i.
%\end{align*}
%\end{example}

%\begin{example}
%Let $\OO = [1, 3i]$ for $i^2 = -1$ be a non-maximal order and $\ideal a = \{2x + 3y i : x, y \in \OO\}$ be an additive subgroup of $\OO$.  Then by Proposition \ref{prop:quadInts}, for some $\xi \in \OO$ we have $\xi = u + 3v i$ for $u,v \in \ZZ$ and for some $\alpha \in \ideal a$ we have $\alpha = 2 \omega_1 + 3 \omega_2 i$ for $\omega_1, \omega_2 \in \OO$.  Since $\omega_1, \omega_2 \in \OO$, there exists $x_1, y_1, x_2, y_2 \in \ZZ$ with $\omega_1 = x_1 + 3 y_1 i$ and $\omega_2 = x_2 + 3 y_2 i$.  Now it follows that
%\begin{align*}
%\xi \alpha &= (u + 3 v i)(2 \omega_1 + 3 \omega_2 i) \\
%&= (u + 3 v i)(2 (x_1 + 3 y_1 i) + 3 (x_2 + 3 y_2 i) i) \\
%&= (u + 3 v i)(2 x_1 + 6 y_1 i + 3 x_2 i - 9 y_2) \\
%&= 2 x_1 u + 6 y_1 u i + 3 x_2 u i - 9 y_2 u
% + 6 x_1 v i - 18 y_1 v - 9 x_2 v - 27 y_2 v i \\
%&= 2 ((x_1 u - 9 y_1 v) + 3(2 y_1 u + 2 x_1 v) i) 
% + 3 ((x_2 u - 9 y_2 v) + 3(y_2 u - x_2 v) i) i.
%\end{align*}
%\end{example}
...
% TODO: Use Pari/GP.
% a = qfbprimeform(-131, 3)
% b = qfbprimeform(-131, 7)
% These give nice ideals and 7 is non-reduced so we can show raw multiplication.
%
% Maybe \Delta = -131 will work better since that is \equiv 3 \pmod 4.
...
\EndWhile
\State $M_1 \gets (R_i a_2 + C_i(b_1-b_2)/2)/a_1$ \Comment{$(b_1 - b_2)/2$ is reused from above}
\State $M_2 \gets (R_i (b_1+b_2)/2 -sC_i c_2)/a_1$
\State $a \gets (-1)^{i+1}(R_i M_1 - C_i M_2)$
\State $b \gets ((2(R_i a_2 - C_{i-1} a)/C_i) - b_2) \bmod{2a}$ \Comment{$R_i a_2$ is reused from above}
\State \Return $[a, (b+\sqrt\Delta)/2]$ \Comment{optionally return $c = (b^2-\Delta)/4a$}
\end{algorithmic}
\end{algorithm}
...
\Ensure $(s_1, a_1, b_1), ..., (s_k, a_k, b_k)$ satisfying $n = \sum_{i=1}^k s_i 2^{a_i} 3^{b_i}$, $s_i \in \{1, -1\}$, $a_i, b_i \in \ZZgez$.
...
This chapter uses big and little O notation, which are defined as
\begin{align*}
...
\EndFor
\For{$j=2s, 3s, ...$}
	\If{$(\beta^j, i)$ is in the table for some $i$} \Comment{giant steps}
		\State $h' = j - i$
...
% TODO: If possible, adjust the line spacing.
...
%TODO(max): Introduce Brent \cite{Brent1976}.
...
% TODO(jacobson): Mike says we should continue with the previous values.   It's not obvious from the paper what to do.  I timed several variations of this, and the version presented here, without continuing on the previous values, is faster.  Future work to explain why?
...
\While{$t \neq 0$}
	...
	\While{$t' \neq 0$}
		...
		\If{$i$ is even}
			\If{$(t' < -D$ or $s' - t' < C - A)$} use previous $A, B, C, D$ and \textbf{break} \EndIf
		\Else \If{$(t' < -C$ or $s' - t' < D - B)$} use previous $A, B, C, D$ and \textbf{break} \EndIf
		\EndIf
		\State $i \gets i + 1$
	\EndWhile
	\If{$B = 0$}
...
Each of the implementations tested here have been verified to produce correct results for inputs up to 31 bits, with the exception of our 32-bit implementations of Stein's right-to-left binary extended GCD, which fails verification for inputs larger than 29 bits -- this appears to be due to arithmetic overflow.  The output for each extended GCD algorithm is a solution to the equation $s = Ua + Vb$.  We verify that the equation is satisfied by the output values given the inputs, and that $s$ is the greatest common divisor of both $a$ and $b$.
...
Figures \ref{fig:gcdRef64} through \ref{fig:gcdBest64} show the average time to compute the extended GCD for a pair of pseudorandom positive integers of $k$ bits for $k$ from 1 through 63 using each of our 64-bit and 128-bit implementations of the extended GCD, as well as for the reference implementations.  For each bit size, $k$, 1,000,000 pairs of pseudorandom positive integers were generated and used as inputs for each implementation of the extended GCD.  In each case, our 64-bit implementation is faster than the corresponding 128-bit implementation.  Figure \ref{fig:gcdRef64} shows that for inputs of this size, Pari performs the fastest of the reference implementations.

Figure \ref{fig:gcdSteins64} shows each of our 64-bit implementations of Stein's right-to-left binary extended GCD for window sizes from 1 bit to 5 bits.  Of these implementations, the implementation using a 4 bit window is consistently faster than our other windowed implementations.  Future would should include a theoretical analysis of optimal window sizes.

Finally, Figure \ref{fig:gcdBest64} shows each of our 64-bit implementations, as well as the fastest reference implementation, Pari.   This figure only includes timings for our 4 bit windowed implementation of Stein's right-to-left binary extended GCD.  This figure shows that for inputs larger than 31 bits but less than 64 bits, our simplified left-to-right binary extended GCD is the fastest and takes roughly 77\% of the time of Pari.   Each of the implementations tested in this subsection produce correct results up to 63 bits, with the exception of our implementations of Stein's right-to-left binary extended GCD, which only produce correct results for inputs up to 60 bits.
...
% TODO(max): Try cancelling an $a_1$ and insert graphs showing reduction after each step, etc.
...
\[
\left(\left(\left(g^{{p_1}^{e_i}}\right)
    ^{{p_2}^{e_2}}\right)^{\cdots}\right)^{{p_w}^{e_w}}
\]
...
% TODO(max): Transpose this table if we get the chance.
\begin{table}[htb]
\centering
...
Thus, the set $\stepBounds$ consists of pairs $(m, w)$ such that for all $(m', w')$ either
\[
