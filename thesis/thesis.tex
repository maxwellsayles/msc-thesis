...
In practice, balancing both stages of SuperSPAR to use $O(N^{1/2r})$ group operations for $r = \sqrt{\ln N / \ln \ln N}$ is often not ideal.  This is partly because the actual cost of multiplication, squaring, and cubing differ, but also because the success of the different stages depends on different properties of the class number.  The exponentiation stage is successful when the order of a random ideal class is smooth with respect to some primorial $E$, while the search stage is successful when the non-smooth part of the order is sufficiently small.  In this sense, the efficiency of the factoring algorithm depends both on the smoothness of a random ideal class $\aclass$ with respect to the primorial $E$, but also on the size of its non-smooth part with respect to the size of the exponent reached in the search stage.
...
In addition, since balancing the number of operations between both stages is often not ideal, we choose the bound $b = m P_w$ for the baby steps separately.  In section \ref{sec:expVsSearch} we discuss our approach to generating a set of bounds for the search stage.

Section \ref{sec:idealsToMultipliers} studies sequential prime ideal representatives for fixed discriminants.  Let 
\[
	\left\{ [\mathfrak a_i] \in Cl_\Delta ~:~  \mathfrak a_i \in Cl_\Delta \textrm{ is the $\ith$ smallest prime ideal for } 1 \le i \le n \right\}
\]
be a set of the $n$ smallest prime ideal representatives for $Cl_\Delta$. Computing the order of each $[\mathfrak a_i]$ shows that typically the orders only differ by a small factor.  If the exponent $E$ is chosen to include such factors, then if either stage of SuperSPAR discovers some $h$ that is a multiple of $\ord([\mathfrak a_i])$, it is likely that $h$ is also a multiple of $\ord([\mathfrak a_j])$ for some other prime ideal $[\mathfrak a_j]$.  As such, once the algorithm discovers some multiple of the order of a random prime ideal, if the algorithm then fails to find an ambiguous form or find an ambiguous form that fails to lead to a factorization of $N$, the algorithm simply chooses another random prime ideal representative and tries again with the same value of $h$.  The exponentiation and search phases are skipped, since with high probability $h$ is a multiple of the order of several random prime ideal representatives.  If several random prime ideal representatives fail to lead to a factorization of $N$, the algorithm then starts over with a different multiplier $k$ for the discriminant.  As such, the final parameter, $t$, is the number of ideal classes to attempt before switching multipliers.

Assuming good parameters are known, the complete SuperSPAR factoring algorithm is given in listing \ref{alg:superSpar}.  Chapter \ref{chap:superSparExperiments} discusses the experiments and results that lead to parameters that perform well in practice.

TODO: Introduce $h_\Delta$.
...
%\STATE attempt to factor $N$ using Algorithm \ref{alg:findAmbiguous} \label{goto:tryAmbiguous}
%\STATE if successful, return a factor of $N$, otherwise start over at line \ref{goto:newK}
\end{algorithmic}
\end{algorithm}
