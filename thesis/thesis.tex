...
\begin{algorithm}[h]
\caption{SuperSPAR - Exponentiation stage.}
\label{alg:ssparExponentiation}
\begin{algorithmic}[1]
\Require $N \in \ZZgez$ odd and composite, \\
  $E = \prod_{i=2}^t {p_i}^{e_i}$ for the exponentiation phase, \\
  $s = mP_w$ a multiple of a primorial for the search phase, \\
  $c \in \ZZgtz$ the number of ideal classes to try before switching multipliers.

\State $\bclass \gets {\idealclass{a_1}}^E$ for the smallest prime ideal class $\idealclass{a_1}$ \Comment{$\bclass = \idclass$ is handled by line \ref{line:firstAmbiguous}}
\State $\idealclass{b'} \gets \bclass$ \Comment{backup $\bclass$}
\For{$1 \le k \le \floor{\log_2 h_\Delta}$}
	\If {$\bclass$ is an ambiguous ideal class} \label{line:firstAmbiguous}
		\If{$\bclass$ leads to a factor of $N$}
			\State \Return a factor of $N$
		\Else
			\State go to line \ref{goto:tryIdeals}
		\EndIf
	\EndIf
...
\begin{algorithm}[h]
\caption{SuperSPAR - Try several ideals.}
\label{alg:ssparTryIdeals}
\begin{algorithmic}[1]
\Require SuperSPAR($h$, $c$, $\Delta$) \\
$N \in \ZZgez$ odd and composite, \\
  $E = \prod_{i=2}^t {p_i}^{e_i}$ for the exponentiation phase, \\
  $s = mP_w$ a multiple of a primorial for the search phase, \\
  $c \in \ZZgtz$ the number of ideal classes to try before switching multipliers.
\State \{-- try several ideal classes --\}
\State $h \gets h \cdot E$
\label{goto:tryIdeals}
\For{$2 \le y \le c$}
	\State $\bclass \gets {\idealclass{a_y}}^h$ where $\idealclass{a_y}$ is the $y^{\textrm{th}}$ smallest prime ideal class
...
\State $\Delta \gets -kN$
\State \textbf{if} $\Delta \not\equiv 0,1 \pmod 4$ \textbf{then} $\Delta \gets 4\Delta$ \textbf{endif}
...
\For{$1 \le i \le s$ where $\gcd(i,P_w)=1$}
	...
	\If{$\bclass^i = \idclass$}
		\State $h \gets i$, $\bclass \gets \idealclass{b'}^i$, then go to line \ref{goto:tryAmbiguous}
	\EndIf
\EndFor
