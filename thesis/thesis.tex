...
In the following figures we sample the average time to exponentiate ideals with 59-bit and 118-bit discriminants for exponents 1 through 65535.  The graphs only show points where the timings differ. Figure \ref{fig:memoChains} show the average cost to exponentiate using the additive chains and $\pm 1$ chains from Subsection \ref{subsec:addChains}. For every exponent tested, $\pm 2, 3$ chains were on average as fast or faster than additive only chains, which is to be expected since the one function computes a subset of the representations of the other function.
\begin{figure}[H]
\centering
\mbox{
...
\caption{Time to exponentiate 16-bit integers using cost based additive chains and $\pm 1$ chains.}
\label{fig:memoChains}
\end{figure}

Similarly, Figure \ref{fig:preVsMemo} compares the average time for representations found using the incremental search of Subsection \ref{subsec:incrementalSearching} against cost based $\pm 1$ chains from the previous graph. For all exponents tested, representations found using the incremental search were as fast or faster on average.
\begin{figure}[H]
\centering
\mbox{
