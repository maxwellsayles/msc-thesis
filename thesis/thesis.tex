...
This thesis compares our implementations of ideal class arithmetic to that our Pari.  With future improvements to ideal class arithmetic, comparisons with additional implementations would be useful, such as with Sage \cite{Sage} and Magma \cite{Magma}.  Finally, one could study practical improvements to other types of ideal arithmetic, such as the class group of real quadratic fields and hyperelliptic curves.
...
Finally, future improvements to SuperSPAR should involve comparisons with the factoring implementations used in this thesis and others that are available.  Our recommendation for future comparisons is to interface directly with the factoring implementation when possible so that timing operations are limited to only the integer factoring operation.  In this thesis, we were not able to do this for GMP-ECM \cite{Ecm}, MSieve \cite{Msieve}, YAFU \cite{Yafu}, and Maple \cite{Maple}, however, each of these projects (except Maple) are open source and it should be possible to modify these sources in order to do so.  Lastly, future work would inspect the source of these factoring implementations in order to determine the factoring algorithm being used for integers of the sizes studied in this thesis.
