...
\begin{example}
Let $i^2 = -1$, then $[1, 2i]$ is an order of $\QQ(i)$ that is \emph{not} maximal since it is contained in the order $[1, i]$.
\end{example}
...
The discriminant $\Delta'$ of the non-maximal order $[1,2i]$ is
\[
...
\]
is an integral ideal in the order $[1, \sqrt{-5}]$.
\end{example}
...
% TODO: Use Pari/GP.
% a = qfbprimeform(-143, 2)
% b = qfbprimeform(-143, 7)
...
\begin{example}
Let $\ideal = [72, (130 + \sqrt{\Delta})/2]$ with $\Delta = -1536$.  Initially, we have $a = 72, b = 130, c = 64$.  Using Algorithm \ref{alg:reduce}, the steps to compute a reduced ideal equivalent to $\ideal a$ are as follows.
\begin{align*}
1. && a &= 72 &b &= 130 &c &= 64 & \textrm{\{initial values\}} \\
2. && a &= 64 &b &= -130 &c &= 72 & \textrm{\{swap $a$ and $c$ and negate $b$\}} \\
3. && a &= 64 &b &= -2 &c &= 6 & \textrm{\{decrease $b \bmod 2a$, recalculate $c$\}} \\
4. && a &= 6  &b &= 2  &c &= 64 & \textrm{\{swap $a$ and $c$ and negate $b$\}}
\end{align*}
The final ideal $\ideal{a'} = [6, (2 + \sqrt \Delta)/2]$ is reduced as it satisfies Definition \ref{defn:reducedIdeal}.
\end{example}
...
% TODO(jacobson): Emphasis that 128 bit NUCUBE has many branches for different sized intermediates and that the growth is explained by GMP.   Possibly insert more graphs, but not necessarily.
