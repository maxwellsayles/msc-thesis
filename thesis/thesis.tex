...
Subsection \ref{subsec:ssparBestMultipliers} indicates that each multiplier is associated with a class group that has a distinct probability that knowing a multiple of the order of an ideal class in the group will lead to a successful splitting of the integer $N$ associated with the discriminant of the class group.  The Subsection also indicates that the expected number of ideas to try is often less than 2.  However, once a multiple of the order of an ideal class is known, our implementation of SuperSPAR is more efficient in practice if many ideal classes are tried before switching multipliers and class groups.  This is likely due to the associated cost of the search phase of the algorithm (since the exponentiation stage is executed for each ideal class tried).  Table \ref{tab:attemptedIdealClasses} shows the average time to split an integer of the associated bit size with an upper limit on the number of ideal classes tried before switching multipliers.  The best time for each bit range is underlined.

% TODO: This table data is make believe.
\begin{table}[h]
\centering
\begin{tabular}{| r | r | r | r | r |}
	\hline
...
80 & 0.97781 & 0.02191 & 0.00028 & 0.00000 \\
	\hline
\end{tabular}
\caption{The average time (in milliseconds) to split an integer $N$ using no more than a fixed number of ideal classes for each class group.}
...
% TODO: This might make more sense to do experimentally.
