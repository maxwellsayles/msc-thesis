...
In this section, we discuss several approaches to exponentiating an ideal class representative to a large exponent.  In particular, we consider large exponents that are the product of many small prime powers where the exponent is known in advance.  This allows us to precompute the sequence of operations in the exponentiation, so that the exponentiation may be as fast as possible without incurring the cost of computing the sequence of operations themselves.

We begin by recalling exponentiation techniques from Chapter \ref{chap:exponentiation}.  These include techniques that use signed and unsigned base two representations, greedy right-to-left and left-to-right double-base representations, and a tree-based approach. Then we consider several extensions of these ideas.  For small exponents (16-bits or less), we consider placing constraints on chains such that we can exhaustively compute all possible constrained representations.  We also consider an incremental search technique.  Knowing efficient representations for 16-bit integers allows us to use larger exponents by partitioning the exponent into 16-bit blocks. Finally, we compare exponentiating by the product of many small primes (known as a primorial) with that of exponentiating by each prime separately.
...
Here we consider two variations to the approach of maintaining the $k$ best partial representations.  By ``best'' we mean that we keep the $k$ smallest partial representations, but that when partial representations have equal $x$ terms, we compute the cost (with respect to the average cost of multiplication, squaring, and cubing) and say that the representation with the smaller cost is better.
...
The approach of maintaining a set of the $k$ best partial representations of an exponent can be adapted to that of the left-to-right 2,3 representation from Subsections \ref{subsec:ltorChains} and \ref{subsec:ltorChains2}.  For a given integer $x$ we say that $2^a3^b$ is a best approximation of $|x|$ when $\left||x| - 2^a3^b \right|$ is minimized.  The algorithm for the left-to-right representation finds a best approximation for $x$ and then repeats on the positive difference. However, instead of only iterating on the best approximation, here each value from the set of partial representations generates new values of the form $\left||x| - 2^a3^b\right|$ (being careful to record the sign of $x$), and the $k$ best partial representations are retained.  In this case, we iterate $b$ from $0 \le b \le \ceil{\log_3 |x|}$ and let $a_1 = \floor{\log_2 (x/3^b)}$ and $a_2 = \ceil{\log_2 (x/3^b)}$ (using $a_2 = a_1 + 1$ is sufficient since either $\ceil{\log_2 (x/3^b)} = \floor{\log_2 (x/3^b)}$ or $\ceil{\log_2 (x/3^b)} = \floor{\log_2 (x/3^b)} + 1$). We then use $\left||x|-2^{2_1}3^b\right|$ and $\left||x|-2^{a_2}3^b\right|$ as candidates for the new set.
...
Another way that we can use the 16-bit representations from the previous two Subsections is when we know the prime factorization of the exponent $n$ and $n$ is the product of primes smaller than $2^{16}$.  Since we are interested in exponentiation by primorials (i.e.\ the product of the first $n$ primes), the prime factorization is trivial. Given a primorial $P_n = \prod_{i=1}^n p_i$ where $p_i$ is the $i^{\textrm{th}}$ prime, we can compute $g^{P_n}$ as $g^{p_1} \cdot g^{p_2} \cdots g^{p_n}$ using a series of $n-1$ small exponentiations and $n-2$ multiplication.

\subsection{Experimental Results}

One reason to improve the performance of exponentiation in the ideal class group is to improve the performance of order finding in this group.  In particular, we exponentiate a random ideal class to a primorial so that the order of the resulting ideal class is likely to be coprime to our exponent.  Here, we first compare the techniques of this section using 16-bit exponents, and then compare the methods useful for larger exponents using the first 2500 odd primorials.

In the following figures we show the average time to exponentiate ideals with 59-bit and 118-bit discriminants for exponents 1 through 65535.  The graphs show every $512^{\textrm{th}}$ point where the approaches give different timings.
\begin{figure}[H]
\centering
\mbox{
	\subfigure{\includegraphics[width=3in]{memo_chains-64}}
	\subfigure{\includegraphics[width=3in]{memo_chains-128}}
}
...
\end{figure}
Figure \ref{fig:memoChains} show the average cost to exponentiate using the additive chains and $\pm 1$ chains from Subsection \ref{subsec:addChains}. For every exponent tested, $\pm 2, 3$ chains were on average as fast or faster than additive only chains\footnote{In the case of the 59-bit discriminant, the timings were the same for 21,816 exponents, and for the 118-bit discriminant, the timings were the same for 16,481 exponents.}.  This is to be expected since the function to compute additive chains computes a subset of the representations computed by the function which computes $\pm 1$ chains.  

Similarly, Figure \ref{fig:preVsMemo} compares the average time for a representation found using the incremental search of Subsection \ref{subsec:incrementalSearching} against a $\pm 1$ from the previous graph. For the 64-bit implementation, the incremental search was as fast or faster on average for all exponents tested, while the 128-bit implementation was slower on average for 2.8\% of the exponents tested\footnote{For the 59-bit discriminant, the timings were the same for 37,732 exponents and the incremental search always yielded a lower average cost. For the 118-bit discriminant, the timings were the same for 44,291 exponents. For 19,390 exponents, the incremental search produced lower average costs, but for 1854 exponents, the cost was actually higher.}.
\begin{figure}[H]
\centering
\mbox{
	\subfigure{\includegraphics[width=3in]{pre_vs_memo-64}}
	\subfigure{\includegraphics[width=3in]{pre_vs_memo-128}}
}
\caption{Time to exponentiate 16-bit integers using precomputed $\pm 1$ chains and chains discovered by an incremental search.}
\label{fig:preVsMemo}
\end{figure}

In the case of computing $g^{p_2} \cdot g^{p_3} \cdots g^{p_n}$ as a series of small exponentiations, we limited the primorials tested to consist only of primes no larger than 16-bits.  Specifically, we tested the odd primorials $P_i$ such that $2 \le i \le 2500$.  The largest prime used is $p_{2500} = 22307$.


\begin{itemize}
\item Exponentiation by fixed primorial
\item Exponentiation by list of prime powers
\item Exponentiation by general integers
\end{itemize}
