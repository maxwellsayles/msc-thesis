...
The basic Euclidean Algorithm is an algorithm for computing the greatest common divisor of two integers and is due to Euclid.  We start with two positive integers $a$ and $b$.  At each iteration of the algorithm, we subtract the smaller of the two numbers from the larger one, until one them is 0. At this point, the other number is the largest divisor of the original numbers $a$ and $b$.

Since the smaller number may still be smaller after a single iteration of the above algorithm, we use fewer steps by subtracting an integer multiple of one number from the other.  This idea is what is more commonly known as the Euclidean Algorithm.  We can extend this concept by using a system of equations of the form
\begin{align}
...
Initially, let
\[
\matrixot{s}{t} = \matrixot{a}{b}
\]
and
\[
\matrixtt{U}{V}{X}{Y} = \matrixtt{1}{0}{0}{1}
\]
and we can see that Equations \ref{eq:initialGcd1} and \ref{eq:initialGcd2} hold.  Using the above description of the Euclidean Algorithm, we can compute a solution of the form $s = Ua + Vb$ such that $s$ is the largest positive integer dividing both $a$ and $b$.  The algorithm is given in Algorithm \ref{alg:divRemEea}.
...
\IF {$t > s$}
	\STATE $\matrixThreeTwo{s}{U}{V}{t}{X}{Y} \gets \matrixtt{0}{1}{1}{0}
            \matrixThreeTwo{s}{U}{V}{t}{X}{Y}$ \COMMENT{Swap rows. Maintain $s \ge t$.}
\ENDIF
\WHILE {$t \neq 0$}
	\STATE $q \gets \floor{s / t}$
	\STATE $\matrixThreeTwo{s}{U}{V}{t}{X}{Y} \gets \matrixtt{0}{1}{1}{-q}
		    \matrixThreeTwo{s}{U}{V}{t}{X}{Y}$ \COMMENT{Subtract $q$ times and swap rows.}
\ENDWHILE
\RETURN $(s, U, V)$ \COMMENT{Such that $s = Ua + Vb$.}
\end{algorithmic}
\end{algorithm}

To compute the partially reduced coefficients of the product ideal, we also make use of the continued fraction expansion of $U/a_1$ using the recurrences
\begin{align*}
	q_i &= \floor{R_{i-2} ~/~ R_{i-1}} \\
	R_i &= R_{i-2} - q_i R_{i-1} \\
	C_i &= C_{i-2} - q_i C_{i-1}
...
\subsection{Lehmer GCD (w/ partial)}
