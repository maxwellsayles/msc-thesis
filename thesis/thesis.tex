...
The original publication of SPAR by Schnorr and Lenstra \cite{Schnorr1984} made the claim that every composite integer $N$ could be factored in $o\left(\exp\left(\sqrt{\ln N \ln\ln N}\right)\right)$ bit operations.  This was the first factoring algorithm for which this runtime had been conjectured, and it was also the first for which this conjecture had to be withdrawn \cite{Lenstra1992}.

The analysis by Schnorr and Lenstra \cite{Schnorr1984} was that Stage 1 dominated the running time of the algorithm.  In the first stage of the algorithm, we exponentiate a random ideal class $\aclass \in Cl_\Delta$ to the product of primes $\prod_{i=2}^t {p_i}^{e_i}$ such that $p_t = N^{1/2r}$ where $r = \sqrt{\ln N / \ln \ln N}$.  Using binary exponentiation, this will take $O(p_t)$ group operations \cite[p.290]{Schnorr1984}.  Since $p_t = N^{1/2r}$, the number of group operations for this exponentiation is
\begin{align*}
...
Stage 2 performed a random walk of at most $O({p_t}^2)$ group operations and so the running time overall is dominated by Stage 1.  The claim was that for a given multiplier $s \le r^r$ the discriminant $\Delta = Ns$ would lead to a factorization of $N$ with probability greater than or equal to $(r-2)^{-(r-2)}$.  One could 
...
\section{Motivation: Best way to do ideal exponentiation.}

\section{Extended GCD}

\subsection{Euclidean GCD (w/ partial)}
