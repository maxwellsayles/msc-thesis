...
Chapter \ref{chap:xgcdExperiments} discusses practical optimizations for computing the extended GCD used in Equations \ref{eq:idealProductSPart1} and \ref{eq:idealProductSPart2}, as well as the simple continued fraction expansion of $sU/a_1$, which is closely related to the extended GCD computation.  Chapter \ref{chap:idealExperiments} gives a more thorough treatment of our implementation of ideal class arithmetic.  Pseudo-code for the approach described here is given in Algorithm \ref{alg:nucomp}.
...
\begin{itemize}
\item $q_i = \floor{R_{i-2} ~/~ R_{i-1}}$
\item $R_i = R_{i-2}-q_i R_{i-1}$
\item $C_i=C_{i-2}-q_i C_{i-1}$
\end{itemize}
...
Section \ref{sec:gcdEEA} discuses the standard extended Euclidean Algorithm, which uses multiplication and division.  Binary extended GCD computations, in contrast, emphasize bit shifting instead of multiplication and division.  Section \ref{sec:gcdR2LBin} discusses an extended GCD algorithm that works from low-order to high-order, referred to as \emph{right-to-left} as this is the order in which bits are operated on in the binary representation.  Section \ref{sec:gcdR2LWindowed} discusses a windowed variant of this algorithm that operates on several bits of the input during each iteration.  An extended GCD can also be computed from high-order to low-order.  This technique is referred to as a \emph{left-to-right} binary extended GCD, and is discussed in Section \ref{sec:gcdL2R}.  When the inputs are larger than a single machine word, Lehmer \cite{Lehmer1938} observed that much of the computation of the extended GCD can still be performed using just the most significant machine word of the intermediates.  Section \ref{sec:gcdLehmer} discusses the Lehmer's standard extended GCD as well as some variations we tried, namely precomputing intermediate solutions for 8-bit machine words and the use of a binary extended GCD for 64-bit machine words.


The ideal class multiplication, squaring, and cubing from Subsections \ref{subsec:nucomp}, \ref{subsec:nudupl}, and ref{subsec:nucube}, use the recurrences
\begin{align*}
	q_i &= \floor{R_{i-2} ~/~ R_{i-1}} \\
	R_i &= R_{i-2} - q_i R_{i-1} \\
	C_i &= C_{i-2} - q_i C_{i-1}.
\end{align*}
Such computations are related to the extended GCD and are referred to as the \emph{partial extended GCD}, discussed in Section \ref{sec:gcdPartial}.

To improve the performance of these algorithms in practice, we specialized much of our implementation for the x64 architecture.  The details of this specialization are discussed in Section \ref{sec:gcdImpl}.  Many of our routines benefit when the input is bound by a single machine word, i.e.\ 64-bits, but we were also able to take advantage of integers that fit within two machine words by implementing a custom library for 128-bit arithmetic.  When integers are larger than 128-bits, we use the GNU Multiple Precision (GMP) arithmetic library \cite{Gmp}.  All the software in this chapter was developed using the \compiler{} on Ubuntu 12.10.   Assembly language was used for the inner loops of the routines presented here, as well as for processing features not available in the C programming language (such as 128-bit arithmetic). The hardware platform was a personal notebook with a \cpu{} and 8Gb of memory.  The CPU has four cores, only one of which was used during timing experiments.

Finally, for each of the extended GCD techniques discussed in this chapter, Section \ref{sec:gcdResults} provides the average time to compute the extended GCD for pairs of random positive integers $(a, b)$ where $a$ and $b$ are an equal number of bits from 1 to 127.
...
% TODO: 128-bit version is good up till 124-bits.  Bad at 125-bits.

The previous extended GCD algorithms use divide with remainder, which is often expensive.  Binary extended GCD algorithms emphasize bit shifting over multiplication and division.  Such algorithms may perform better than other extended GCD algorithms in practice (see Section \ref{sec:gcdResults} for results).  Here we describe a binary extended GCD algorithm that works from the least significant bit to the most significant bit.  We refer to this as right-to-left since this is the direction in which we process the written binary representation.  The concept was originally published by Stein \cite{Stein1967}.
...
When both $a$ and $b$ are even, $\gcd(a, b) = 2 \cdot \gcd(a/2, b/2)$.  So the first step of our binary extended GCD algorithm is to remove all common powers of two from $a$ and $b$.  Let $r$ be the number of times 2 is removed from both.  Now either $a$ or $b$ or both are odd.  If $a$ is not odd, then swap $a$ and $b$ so that $a$ is guaranteed to be odd.  We will compute the extended GCD of the reduced $a$ and $b$.  As such, the final solution to the original input is $s2^r = Ua2^r + Vb2^r$.
...
\section{Windowed Right-to-Left Binary Extended GCD}
\label{sec:gcdR2LWindowed}
...
\]
for positive integers $a$ and $b$.  Assume that $s \ge t$ (if this is not the case, then swap the rows of the matrix).  Compute $s'=\floor{s/2^k}$ for some $k$ such that $s'$ fits within a single machine word and is as large as possible (if $s$ already fits within a single machine word, then let $k=0$), and compute $t' = \floor{t/2^k}$ for the same value $k$.  Then perform an extended GCD computation on the values $s'$ and $t'$ but only for as long as the quotients $q'=\floor{s'/t'}$, generated by each step of the single precision extended GCD computation, are equal to the quotients $q=\floor{s/t}$, generated by a full precision extended GCD computation.  To determine when the quotients $q'$ differ from the quotients $q$, compute $q_1 = \floor{(s'+A)/(t'+C)}$ and $q_2 = \floor{(s'+B)/(t'+D)}$.  Let $q' = q_1$ and when $q_1 = q_2$ it follows that $q' = q$ \cite[p.229,~Theorem~A]{Lehmer1938}.
...
\]
Then continue with the outer loop of the computation until $t = 0$.  In the event that $B=0$, then $s$ and $t$ differ in length by more than a machine word, and we use a step of the full precision extended GCD computation to adjust their lengths.  The complete algorithm is given in Algorithm \ref{alg:lehmerGcd}.
...
\begin{align*}
	...
	C_i &= C_{i-2} - q_i C_{i-1}.
\end{align*}
These recurrences perform the same operation as a single step in the Extended Euclidean Algorithm of Section \ref{sec:gcdEEA}, but the initial and stopping conditions are different.  As such, this is referred to as the \emph{Partial Extended Euclidean Algorithm}.  The algorithm is listed in Algorithm \ref{alg:PartialEeaDivRem}.
...
Finally, Lehmer's extended GCD algorithm is especially useful when the arguments to the GCD are several words in size.  However, since our implementations are specialized up to 128-bit words, we consider 8-bit words for the inner extended GCD computation.  As such, it is possible to precompute the resulting $2 \times 2$ matrix for all possible inputs into the single precision extended GCD computation.  Since there are two inputs, each 8-bits in size, there are 65536 possible pairs of inputs.  The coefficients of the resulting $2 \times 2$ matrix can be bound by 8-bits each, and so the table requires 256Kb of memory. This simplifies the extended GCD computation since the inner loop of Lehmer's extended GCD becomes a lookup from this table.

\section{Experimental Results}
\label{sec:gcdResults}

% TODO: Subsections for 32-bit, 64-bit, 128-bit.

% TODO: Add Lehmer's with 64-bit L2R, etc.

% TODO: We use positive inputs for experimenting since the prologue and epilogue for each XGCD type is the same for converting numbers to positive and then adjusting the results.
...
For Lehmer's extended GCD, we precomputed a table of 8-bit inputs for the inner extended GCD computation -- see Figure \ref{fig:lehmer-32v64}. In each case, the 32-bit implementation is faster than the 64-bit implementation.
\mygraph{lehmer-32v64}{fig:lehmer-32v64}{Lehmer's extended GCD with 8-bit precomputed inner GCD.}
...
\mygraph{stein1-32v64}{fig:stein1-32v64}{Right-to-Left Binary Extended GCD.}
\mygraph{stein2-32v64}{fig:stein2-32v64}{2-bit Windowed Right-to-Left Binary Extended GCD.}
\mygraph{stein3-32v64}{fig:stein3-32v64}{3-bit Windowed Right-to-Left Binary Extended GCD.}
\mygraph{stein4-32v64}{fig:stein4-32v64}{4-bit Windowed Right-to-Left Binary Extended GCD.}
\mygraph{stein5-32v64}{fig:stein5-32v64}{5-bit Windowed Right-to-Left Binary Extended GCD.}
...
\mygraph{shallit-32v64}{fig:shallit-32v64}{Left-to-Right Binary Extended GCD of Shallit and Sorenson.}
...
\mygraph{binary_l2r-32v64}{fig:binary_l2r-32v64}{Simplified Left-to-Right Binary  Extended GCD.}

\clearpage

Note that the 32-bit versions of the right-to-left binary extended GCD are only accurate up to 29-bits (ignoring sign), while the 64-bit versions are only accurate to 60-bits. All other 32-bit implementations are accurate to 31-bits\footnote{The input is less than or equal to 31-bits, since in a two's complement representation the sign of the input variable takes the most significant bit and accounts for all 32-bits.} and all other 64-bit implementations are accurate to 63-bits\footnote{Similar to the 32-bit implementation, the most significant bit represents the sign of the integer.}.
...
\mygraphX{divrem-vs-lehmer}{fig:divrem-vs-lehmer}{Extended Euclidean Algorithm compared to Lehmer's GCD using precomputed 8-bit words.}{EEA vs Lehmer's Extended GCD.}

In Figure \ref{fig:steins}, we compare each of our windowed implementations of the right-to-left binary GCD.  Larger windows trade off precomputation against the likelihood that a number will be divisible by some power of two.  One can see that the non-windowed version is most efficient for inputs less than 16-bits in size, at which point the 3-bit windowed version is best until 30 bits.  After 30 bits, the 4-bit windowed version out performs other window sizes in the range with which we experimented.  In the 64-bit implementation, the slope of the 5-bit windowed version is the lowest and so presumably it would out perform the 4-bit window once inputs were large enough.
\mygraphTwo{steins-32}{steins-64}{fig:steins}{Windowed Right-to-Left Binary Extended GCDs.}
...
% TODO: We are trying to minimize the average time to factor integers for each given bit size.

%Also, I think I need to state a little more about my methodology for the SuperSPAR experiments in Chapter 8.   For GCD it's very fast (a few hundred nanoseconds), and I can time large sample sets (1,000,000 pairs for around 100 different bit sizes, for numerous different GCD algorithms) and get good timing estimates.   Ideal arithmetic is still pretty fast and again, I can time large sample sets (10,000,000 operations for over 100 different discriminant sizes, for three (now four) different implementations) and get good timings.  The GCD experiments complete in a few minutes and the ideal experiments complete in a few hours.

%SuperSPAR is a different beast.  A fast factorization is around 1 millisecond per integer, while slower factorizations were closer to 40 milliseconds (this is between 10,000 times and 400,000 times slower than a GCD).  And we have multiple dimensions of parameters to iterate (choice of largest prime in exponent, choice of prime power bounds, choice of primorial in search, choice of multiplier of primorial, choice of hashing technique, number of ideals before switching class groups, etc) and various bit sizes.   So even for small sets of integers (a few 1000), this can be quite slow.

%For this reason, I did some statistical sampling (section 8.2) to try to eliminate some of these variables.   I was able to sample 250,000 integers instead of only 4000 (for the final parameter search).   Mining these samples takes a few minutes, while iterating on the final 4000 integer sample sets, using the method proposed in section 8.5.1 (even after hand optimizing the search range (I forgot to mention this)), still took over a day.  So it's quite slow even for a small sample set.

%So the methodology in general was to emphasize empirical timings, except for when the timings took unreasonably long even for small sample sets.  Then we fell back to statistical data.

This chapter discusses the experiments and results that directed our implementation of the SuperSPAR integer factoring algorithm, discussed in Section \ref{sec:superSpar}.  The algorithm is based on the ideal class group, discussed in Chapter \ref{chap:idealArithmetic}, and the experiments and results leading to our implementation of arithmetic in the ideal class group is discussed in Chapters \ref{chap:xgcdExperiments} and \ref{chap:idealExperiments}.  SuperSPAR is an adaptation of the SPAR factoring algorithm, discussed in Section \ref{sec:spar}.  The algorithms differ in how they search for the order of a group element -- SuperSPAR uses the bounded primorial steps algorithm discussed in Section \ref{sec:primorial} -- but both algorithms first exponentiate a random ideal class to the product of many primes.  Chapter \ref{chap:exponentiation} discusses several methods for exponentiation in generic groups and Chapter \ref{chap:powExperiments} discusses our experiments and results of exponentiation in the ideal class group.  Practical improvements to each of these areas lead to practical improvements in our implementation of SuperSPAR.
...
% TODO: Since $P_w$ grows so quickly, we cannot use a very large $P_w$ in the search phase.  We can use a large $P_w$ in the exponentiation phase since it might remove larger factors.  
...
\section{SuperSPAR in Practice}
\label{sec:ssparInPractice}

% TODO: Exponentiation primorial must be larger than the search primorial
...
\begin{algorithm}[htb]
\caption[Repeatedly Square]{Try to find an ambiguous ideal class by repeated squaring.}
\label{alg:ssparRepeatedlySquare}
\begin{algorithmic}[1]
\Procedure{RepeatedlySquare}{$\bclass, \ell$}
\For{$i$ from 1 to $\ell$ while $\bclass \neq \idclass$}
	\If{$\bclass$ is an ambiguous ideal class}  \Comment{See Subsection \ref{subsec:forms}.}
...
% TODO: Justify that experimentation was taking too long and so it was supplemented by statistical sampling, but that experimentation was preferred when possible.
