...
The work of this thesis is to improve exponentiation in the ideal class group of imaginary quadratic number fields, with an application to integer factoring.  To this end, the results of Chapter \ref{chap:xgcdExperiments} show that we improved the average performance for computing the extended GCD for inputs smaller than 128-bits over the reference implementations provided by \Pari{} \cite{PariGP} and \GMP \cite{Gmp}.  Chapter \ref{chap:idealExperiments} demonstrates result that our implementations of 64-bit and 128-bit ideal class arithmetic are significantly faster than that provided by the reference implementation of Pari.  Furthermore, our reference implementation of ideal class arithmetic using GMP for multiple precision arithmetic, is often faster on average than the reference implementation provided by Pari, and certainly when the size of the discriminant is larger than 118-bits (the largest sized discriminant that works with our 128-bit implementation).  In Chapter \ref{chap:powExperiments}, we introduce a novel algorithm for computing 2,3 representations of exponents that take into consideration the average cost to multiply, square, and cube in a group.  For large exponents that are the product of several primes, we demonstrate that this technique generates representations that lead to the fastest on average exponentiations of elements within the ideal class group for our implementations of ideal class arithemtic.  Finally, Chapter \ref{chap:ssparExperiments} shows that when all of these improvements are applied to the SuperSPAR factoring algorithm, the result is an implementation of an integer factoring algorithm that is the fastest on average of all implementations tested for semiprime integers no smaller than 50-bits and no larger than 62-bits.

For the work of this thesis to be most useful to the future work of others, the source code for the libraries used throughout is available online.  An optimized implementation of 128-bit arithmetic and each of the implementations of the extended GCD are available at \url{https://github.com/maxwellsayles/liboptarith}, our implementations of ideal class arithmetic can be obtained from \url{https://github.com/maxwellsayles/libqform}, and finally, our implementation of the SuperSPAR factoring library is available at \url{https://github.com/maxwellsayles/libsspar}.   By making these publicly available, the hope is for eventual inclusion into common mathematical packages such as \PariGP{} \cite{PariGP}, Sage \cite{Sage}, and Dr. Michael J. Jacobson, Jr.'s algorithmic number theory library.
...
Practical improvements to arithmetic in the ideal class group in this thesis are based on the Algorithms for computing reduced (or almost reduced) representatives for multiplication (Subsection \ref{subsec:nucomp}), squaring (Subsection \ref{subsec:nudupl}), and cubing (Subsection \ref{subsec:nucube}).  While these Algorithms are certainly faster as the size of the discriminant grows, future work could include a comparison of the implementation in this thesis with an implementation of the basic ideal class multiplication presented in Subsection \ref{subsec:idealMultiply}, that does not produce a reduced ideal class representative.  The idea would be to optimize an implementation of the straightforward ideal multiplication algorithm, as well as to make improvements to the ideal reduction algorithm from Subsection \ref{subsec:reduction}.  Ideal reduction is similar to the standard Euclidean Algorithm, as such, future work might apply several of the approaches used for computing the extended GCD to that of ideal reduction. 

Additional techniques for computing the extended GCD can be explored.  A relatively straightforward approach is to apply windowing to the left-to-right binary GCD that performed well for 64-bit and 128-bit integers.

Also useful would be a more thorough study of each of the extended GCD techniques discussed in Chapter \ref{chap:xgcdExperiments} in the context of ideal class arithmetic.  What is proposed here is to time the performance of ideal class arithmetic for each of the different techniques mentioned, this includes each technique applied to a full, left-only, and partial extended GCD in the context of ideal class arithmetic.
...
% TODO: From the 3d image of 48-bits, it looks like we can go faster with smaller primorials.  We don't have primorials *small* enough!
