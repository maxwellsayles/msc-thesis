...
\end{align*}
then $b' = r$. We note here that instead of computing a new value for $c = (b^2 - \Delta)/4a$, we can compute the new value from $q$ and $r$ above as $c - (b + r)q/2$.

For GMP, we preallocate the memory needed to save time on memory allocation/releases.

Only compute left portion of GCD when we're only interested in the left part.  Only compute three part GCD if first part is not 1.
