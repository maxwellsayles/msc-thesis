...
In the primorial steps algorithm we would compute $E = \prod_{i=1}^n {p_i}^{\floor{\log_{p_i} M}}$ and $\bclass = \aclass^E$.  However, since we want to find an ambiguous ideal, i.e.\ some ideal class $\cclass$ such that $\cclass^2 = \idclass$, we instead only use the \emph{odd} primes $p_i \le p_n$ when computing $E$ and $\bclass = \aclass^E$.
...
\FOR{$1 \le k \le \floor{\log_2 h_\Delta}$}
	...
	\IF {$\cclass = \idclass$}
		\STATE attempt to factor $\Delta$ since $\bclass^{\left(2^{k-1}\right)}$ is an ambiguous ideal
	\ENDIF
\ENDFOR
...
\FOR{$1 \le j \le \phi(b)$}
	\IF{$\cclass^{2jb}$ is in the table}
		\STATE $h' \gets 2jb - i$, then go to line \ref{goto:findAmbiguous}
	\ENDIF
\ENDFOR
