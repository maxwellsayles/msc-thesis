...
% I would like to thank my supervisor, Dr. Michael J. Jacobson, Jr., for all of his great advice.  I am certain that without his patience I would not have made it this far.  I would also like to thank my wife, Faye, for her kind understanding, especially during ``crunch time''.  Thank you both.   Also thank Arthur and Laurent and Andrew.
...
With this in mind, this thesis focuses on practical improvements to arithmetic in the ideal class group of imaginary quadratic number fields, but also to that of exponentiating an element in this group to a primorial.  Such improvements are interesting in that they could be used to further the work of Ramachandran \cite{Ramachandran2006}.  However, for the purposes of this thesis, these improvements are applied to an extension of an integer factoring algorithm called SPAR \cite{Schnorr1984}.  This algorithm is based on arithmetic in the ideal class group, and works by computing the order of an ideal class.  In this thesis, we adapt this algorithm to use the ideal class cubing of Imbert, Jacobson, and Schmidt \cite{Imbert2010}, the bounded primorial steps algorithm of Sutherland \cite{Sutherland2007}, and the double base number system of Dimitrov and Cooklev \cite{Dimitrov1995a, Dimitrov1995b}.  We call this new algorithm ``SuperSPAR''.  Our implementation of SuperSPAR is the fastest integer factoring implementation on average for integers between 50-bits and 62-bits of size of the implementations tested.
...
To begin, one contribution of this thesis is improved arithmetic in the ideal class group of imaginary quadratic number fields.  The background material on ideal class groups useful in understanding the improvements discussed in this thesis are covered in Chapter \ref{chap:idealArithmetic}.  Much of the computational effort of ideal class multiplication is in computing solutions of the extended greatest common divisor.  Chapter \ref{chap:xgcdExperiments}, discusses several solutions to this problem, as well as an adaptation to a left-to-right binary extended GCD that we propose in Section \ref{sec:gcdL2R}.  The results in Section \ref{sec:gcdResults} show that this method is the fastest on average for a large range of integer sizes, when compared against \Pari{} \cite{PariGP} and \GMP{} \cite{Gmp} as reference implementation.  The background material on the ideal class group in Chapter \ref{chap:idealArithmetic} is also the basis of our implementation of ideal class arithmetic.  The experiments that lead to our improved implementation are discussed in Chapter \ref{chap:idealExperiments}.  The results in Section \ref{sec:idealResults} show that our implementation, on average, improves upon the implementation of \Pari{} \cite{PariGP}, which we use as a reference implementation, at least for arithmetic in class groups with negative discriminants of the sizes studied.

Another contribution of this thesis is improved exponentiation in the ideal class group.  Chapter \ref{chap:exponentiation} discusses some methods from the literature for exponentiating group elements.  In particular, we discuss double base number systems and some approaches from the literature for computing double base representations useful for exponentiation.  In Chapter \ref{chap:powExperiments}, we propose several adaptations to these methods.  In particular, we propose a method that we call \emph{left-to-right best approximations} in Section \ref{sec:expBestApproximations}.  In Section \ref{sec:expResults} we provide results that show that for our implementation of ideal class arithmetic and when exponentiating by large primorials, a left-to-right best approximations technique performs the best on average.
...
All the software for this thesis was developed using the \compiler{} on Ubuntu 12.10.  Timing experiments were run on a \cpu{} with 8Gb of memory.  The CPU has four cores, only one of which was used during timing experiments.
...
%\begin{defn}
%An ideal $\ideal a$ is called \emph{primitive} if it cannot be written $\ideal a = m \ideal b$ for some ideal $\ideal b \in \OO_\Delta$ and $|m| > 1, m \in \ZZ$.
%\end{defn}

Let $\OO_\Delta$ be a maximal order.  Then for $\alpha, \beta \in \OO_\Delta$, the set $\ideal a = \{x \alpha + y \beta : x, y \in \OO_\Delta\}$ is an ideal in the order $\OO_\Delta$ and is denoted $(\alpha, \beta)$ \cite[p.16]{Ramachandran2006}.  Every ideal of a quadratic order $\OO_\Delta$ can be represented by at most two generators \cite[p.125--126]{Cohn1980}, while some can be represented by a single generator.  An ideal represented by a single generator is a \emph{principal} ideal and is denoted $(\alpha) = \{ x \alpha : x \in \OO_\Delta \}$ \cite[p.87]{Jacobson2009}.
...
\begin{defn}
\cite[Definition 2.13]{Ramachandran2006} An $\ideal a$ of $\OO_\Delta$ is a \emph{fractional ideal} if there exists an ideal $\ideal b$ and a non-zero $c \in \OO_\Delta$ such that $\ideal a = c^{-1} \ideal b$.
\end{defn}

Using fractional ideals, we are able to define inverses.

\begin{prop}
\cite[Proposition 2.14]{Ramachandran2006} Let $\ideal a$ be a fractional ideal in $\OO_\Delta$, where $\OO_\Delta$ is a maximal order in the quadratic field $\QQ(\sqrt \Delta)$.  The inverse of $\ideal a$ is then
\[
...
All fractional ideals in $\OO_\Delta$ are invertible, and the set of invertible ideals forms a group under ideal multiplication.  We now define an equivalence relation on the invertible ideals of $\OO_\Delta$.
...
\end{equation}
and C. Siegel \cite[p.247]{Crandall2001} shows that 
\begin{equation}
\label{eq:hDeltaInfty}
h_\Delta = |\Delta|^{1/2 + o(1)} \textrm{ as } \Delta \rightarrow -\infty.
\end{equation}
These bounds on the class number will be useful in Chapters \ref{chap:superspar} and \ref{chap:ssparExperiments}.
...
\end{equation}
where $\mathcal N(\ideal a) = q^2a$ is the norm of $\ideal a$ and is multiplicative.  For a primitive invertible ideal $(a, b)$, the inverse is given by $\frac{1}{a}(a, -b)$.  Let $\aclass$ be an ideal class represented by the primitive ideal $\ideal a$.  The inverse $\idealclass{a^{-1}}$ is given simply by $(a, -b)$ \cite[p.20]{Ramachandran2006}.  As such, computing the inverse of an ideal class is virtually free. 
...
% Summarize that 32-bit EEA takes only 63\% of the time as the fastest reference implementation, i.e.\ Pari.  64-bit is our simplified l2r which takes only 77\% of the time as Pari.  
...
%One contribution of this thesis is efficient implementations of algorithms that compute the extended greatest common divisor (GCD), useful for ideal class arithmetic.  This chapter discusses each of our implementations and their average performance, and compares each implementation to several reference implementations, namely \Pari{} \cite{PariGP}, \GMP{} \cite{Gmp}, and \MPIR{} \cite{Mpir}.  Our

%Our best 32-bit implementation (the 32-bit extended Euclidean algorithm from Section \ref{sec:gcdEEA}) takes on average only 63\% of the time as the best reference implementation. Our best 64-bit implementation (a simplified left-to-right binary extended GCD introduced in Section \ref{sec:gcdL2R}) takes on average only 77\% of the time as the best reference implementation.  Finally, our best 128-bit implementations take between 95\% and 88\% of the time of the best reference implementations on average.   For integers larger than 31-bits and smaller than 127-bits, our best performing implementations use a variant of Shallit and Sorenson's left-to-right binary extended GCD \cite{Shallit1994}, which we introduce and refer to as a simplified left-to-right binary extended GCD.  On average, our 32-bit simplified left-to-right binary extended GCD uses only 78\% of the time as our 32-bit implementation of Shallit and Sorenson's left-to-right binary extended GCD.  For our 64-bit and 128-bit implementations the advantage is even larger.  Our 64-bit implementation uses on average only 71\% of the time, while our 128-bit implementation uses on average on 45\% of the time.
...
%, as well as a novel algorithm based on Shallit and Sorenson's left-to-right binary extended GCD \cite{Shallit1994}, that we refer to as a simplified left-to-right binary extended GCD.  
...
% TODO: Introduce Brent's method and state that ours is slightly different.  \cite{Brenth1976}
...
\[
	\matrixThreeTwo{s}{U}{V}{t}{X}{Y} = \matrixThreeTwo{R_{i-1}}{C_{i-1}}{B_{i-1}}{R_i}{C_i}{B_i}
\]
for integers $B_i$.  To expand the simple continued fraction $a/b$, let
\[
\matrixThreeTwo{R_{-2}}{C_{-2}}{B_{-2}}{R_{-1}}{C_{-1}}{B_{-1}} = \matrixThreeTwo{a}{-1}{1}{b}{0}{0}
\]
and apply the recurrences
\begin{align*}
...
\end{align*}
until we have $R_i$ less than or equal to the given termination bound.  This is essentially the same as computing the extended GCD except for the starting matrix and the termination bound.  Invariants of this method are $R_{i-1} \ge R_i$ and $C_i = (-1)^{i+1}B_i$, which can be shown by induction (for additional details, see \cite[p.5]{Imbert2010} and \cite[Equation 3.4]{Avanzi2010}).  Since the coefficients $B_i$ are never used, our implementations of the partial extended GCD does not compute them.
...
\]
during the partial extended GCD must be representable by an invertible $2 \times 2$ matrix with determinant $\pm 1$ (known as a \emph{unimodular} matrix).  This is necessary in order to make sure that the reduced ideal (after applying the coefficients from the partial extended GCD) is in the same equivalence class as the product ideal.  This is important since only extended GCD computations that are restricted to operations representable using invertible $2 \times 2$ matrices can be used for the partial extended GCD.
...
% TODO: The goal of this chapter is to find a method that works well with primorials since that's mostly what we'll use in SuperSPAR.

% TODO: Mention our proposed methods in the introduction.
% TODO: Mention alternate names for methods so people recognize them in the graphs.

Subection \ref{subsec:boundedPrimorialSteps} discussed the bounded primorial steps algorithm -- an algorithm that finds the order of an element by exponentiating it to the product of many primes.  Since this exponent is determined by a given bound, the algorithm benefits from precomputation of representations that lead to efficient exponentiation in practice.  Section \ref{sec:superSpar} discussed the SuperSPAR integer factoring algorithm, for which a bound on the group size and the order of an element lead to the factorization of an integer.  Representations that lead to efficient exponentiation in practice improve the performance of the SuperSPAR factoring algorithm, as well as any algorithm based on the bounded primorial steps algorithm, or one that uses exponentiation for fixed exponents.

This chapter discusses several approaches to exponentiating an ideal class to large exponents that are the product of all primes less than some bound (known as \emph{primorials}), particularly where the set of exponents is known in advance.  We begin by recalling exponentiation techniques from Chapter \ref{chap:exponentiation}.  These include techniques that use signed and unsigned base 2 representations, greedy right-to-left and left-to-right double base representations (for bases 2 and 3), and a tree-based approach.  We also propose several extensions to these.  For small exponents (16-bits or less), we are able to exhaustively compute all representations under certain constraints.  Unfortunately, doing so with the techniques proposed appears to require an exponential amount of time with respect to the size of the input.  Computing representations for all exponents 16-bits or less still has practical benefits, since we can represent a larger exponent by partitioning it into 16-bit blocks or by using its factorization.  Finally, we compare each method on a sequence of primorials to find the best method in practice.  Binary exponentiation is used as a reference implementation since it is the simplest of the exponentiation techniques considered here.
...
In this section, we propose a method combining the approach of maintaining a set of the $L$ best partial representations, suggested by Doche and Habsieger \cite{Doche2008} (see Section \ref{sec:expGreedyTrees}), with that of computing left-to-right 2, 3 representations, suggested by Berth{\'e} and Imbert \cite{Berthe2009} (see Section \ref{sec:expL2RChains}).  The results of Section \ref{sec:expResults} show that this method gives the best performance on average of the methods studied when exponentiating by power primorials.

For an integer $x$ we say that $2^a3^b$ is a \emph{best approximation} of $|x|$ when $\left||x| - 2^a3^b \right|$ is minimal.  The algorithm for the left-to-right representation (Algorithm \ref{alg:greedyltor}) finds a best approximation for $x$ and then repeats on the positive difference.  What is proposed here is that instead of only iterating on the best approximation, each value from the set of partial representations generates new values of the form $\left||x| - 2^a3^b\right|$ (being careful to record the sign of $x$), and the $L$ best partial representations are retained.  In this case, we iterate $b$ from $0 \le b \le B$ and let $a_1 = \floor{\log_2 (x/3^b)}$ and $a_2 = \ceil{\log_2 (x/3^b)}$ bounding both $a_1, a_2 \le A$.  Note that using $a_2 = a_1 + 1$ is sufficient since either $\ceil{\log_2 (x/3^b)} = \floor{\log_2 (x/3^b)}$ or $\ceil{\log_2 (x/3^b)} = \floor{\log_2 (x/3^b)} + 1$.  We then use $\left||x|-2^{a_1}3^b\right|$ and $\left||x|-2^{a_2}3^b\right|$ as candidates for the new set.  As before, iterating the bound on the number of squares and cubes varies the cost of the representations generated.  

% TODO: Elaborate on this technique and give the full algorithm here.


\section{Additive 2,3 Chains}
\label{sec:expAddChains}

% TODO: Small contribution here.
...
For our purposes, we propose two modifications of this algorithm to compute 2,3 chains that minimize the average cost of arithmetic in the ideal class group -- the resulting additive 2,3 chains should give a better average time for exponentiation, while they are not necessarily the shortest possible.  Future work should compare the cost of exponentiation using representations generated by the functions from this section, with representations generated by the function $s(n)$ in Subsection \ref{subsec:shortAddChains}.
...
We point out that the functions $s'(n)$ and $f(n)$ are highly redundant in their computation.  As such, our implementations memoize the result for each input in order to improve the running time.  Furthermore, since we compute the result for each exponent $n$ for $1 \le n < 65536$, computing $s'(n)$ can benefit from the computation of $s'(m)$ for each $m < n$, for example.  This reduces the running time to compute representations for all exponents $n$ for $1 \le n < 65536$, but even so, the running time of each algorithm appears to be exponential in the size of the largest input and so we only use these functions up to 16-bit inputs.  We recommend a thorough analysis of the running time as future work.


\section{Incremental Searching}
\label{sec:expIncrSearch}

% TODO: This is also a contribution.
% TODO: It's not obvious that we are computing representations rather than chains.

Section \ref{sec:expAddChains} imposes additional constraints on 2,3 chains such that we are able to compute all such chains for a given input.  Here we propose a function for generating a set of representations.  We then record the \emph{cheapest} representation for each integer represented by the set.

We first iterate over all single term representations $2^{a_1}3^{b_1}$ for $0 \le a_1 \le A$ and $0 \le b_1 \le B$, and then all two term representations $2^{a_1}3^{b_1} \pm 2^{a_2}3^{b_2}$ for $0 \le a_1 < a_2 \le A$, $0 \le b_1 \le B$, and $0 \le b_2 \le B$.   In general, we compute the set of representations
\[
...
The techniques of the previous two sections appear to require an exponential amount of time in the size of the input and as such, we limit our search to representations of 16-bit integers.  One way to use such representations is with a left-to-right $2^k$-ary algorithm \cite[\S 9.1.2]{Cohen2006}.  Write the exponent $n$ in 16-bit blocks as
\[
...
\end{equation}
As such, we compare the techniques discussed in this chapter on exponents $E$ for values of $t \le 1000$.  We point out that the theoretically optimal value of $t$ for a 100-bit number is about 5273, which is less than 7919, the $1000^{\textrm{th}}$ prime.

The results of Section \ref{sec:idealResults} are the average time to multiply, square, or cube an ideal class, and are based on a sample of 10,000,000 operations of each kind for each discriminant size from 16-bits to 140-bits.  We use these results to compute an estimate of the average time to exponentiate a given representation.  In each case, only the time to exponentiate using a representation is included.  Since negative discriminants of 59-bits are the largest known to work with our 64-bit implementation of ideal class arithmetic, and negative discriminants of 118-bits are the largest known to work with our 128-bit implementation of ideal class arithmetic, we take discriminants of these sizes as an upper bound on the performance of exponentiating for each of our implementations.

Section \ref{sec:expBigFromSmall} discusses two techniques to compute $g^n$ using a series of smaller exponentiations $g^b$ such that $0 \le b < 2^{16}$.  We begin by determining the best method to exponentiate using 16-bit exponents, so that we may then compare these methods using power primorial exponents.  To do so, we compute the average time to exponentiate for exponents 1 through 65535 using 59-bit and 118-bit discriminants.  Table \ref{tab:exp16bitWinners} shows the number of exponents for which each method was the fastest (methods that were not the fastest for any of the exponents tested are not shown).  Table \ref{tab:exp16bitTimes} shows the average time, in microseconds, for each method to exponentiate a 16-bit exponent.  This is based on the average of the average time to exponentiate for each exponent 1 through 65535.  For each of the greedy pruned methods and for the left-to-right best approximations method, we only maintain the best 4 leaf nodes.  We also use a bound $a \le 4$ and $b \le 4$ for each of the greedy pruned methods.
...
\begin{tabular}{| l | r | r |}
	...
	Greedy Pruned $(x \pm 2^a3^b)/(2^c3^d)$ & 47969 & 45468 \\
	Greedy Pruned $(x \pm 2^a \pm 3^b)/(2^c3^c)$ & 3333 & 2756 \\
...
\begin{tabular}{| l | c | c |}
	...
	Right-to-Left Non-Adjacent Form & 6.04918 & 13.59860 \\
	...
	Greedy Pruned $(x \pm 1)/(2^c3^d)$ & 5.34933 & 12.43606 \\
	Greedy Pruned $(x \pm 2^a3^b)/(2^c3^d)$ & 4.59421 & 10.75899 \\
	Greedy Pruned $(x \pm 2^a \pm 3^b)/(2^c3^c)$ & 5.98289 & 14.05320 \\
...
\mygraphTwoX{pm_variants-64}{pm_variants-64-zoom}{fig:expPmVariants64}{Greedy Pruned Trees using the 4 Best $(x\pm\cdots)/(2^c3^d)$ (59-bit Discriminants).}{4 Best $(x\pm\cdots)/(2^c3^d)$ (59-bit Discriminants).}
\mygraphTwoX{pm_variants-128}{pm_variants-128-zoom}{fig:expPmVariants128}{Greedy Pruned Trees using the 4 Best $(x\pm\cdots)/(2^c3^d)$ (118-bit Discriminants).}{4 Best $(x\pm\cdots)/(2^c3^d)$ (118-bit Discriminants).}
...
This chapter presented the approach and results used to improve exponentiation in the ideal class group of imaginary quadratic number fields.  Several methods for exponentiation including novel extensions to methods for computing 2,3 chains and representations were explored.  We found that for 16-bit exponents, using a greedy pruned trees approach and iterating on the 4 best partial representations $x + \sum s_i2^{a_i}3^{b_i}$ such that each $x$ generates new terms of the form $(x \pm 2^a 3^b)/(2^c3^d) \in \ZZ$ gives the fastest representation on average.  For large primorial exponents, a left-to-right best approximations technique (Section \ref{sec:expBestApproximations}) gave the fastest representations on average.  The next chapter brings together the improvements to ideal arithmetic and exponentiation in our implementation of SuperSPAR.

% TODO: At some point it looks like a greedy pruned tree using $(x \pm 2^a3^b)/2^c3^d$ crosses over the left-to-right best approximations method.  It would be good to see when that happens.
...
\Procedure{Search1}{$\timefnc, x, \ymin, \ymax, k$}
...
\While{$\yhi - \ylo > k$}
	...
	\If{$\timefnc(x, \yonethird) \le \timefnc(x, \ytwothird)$}
...
\State $(y, t) \gets (\ylo, \timefnc(x, \ylo))$ \Comment{$(y, t)$ represent smallest $\timefnc(x, y)$ encountered}
...
\Procedure{Search2}{$\timefnc, \xmin, \xmax, \ymin, \ymax, k$}
\Statex \{-- ternary search --\}
\State $(\xlo, \xhi) \gets (\xmin, \xmax)$
\While{$\xhi - \xlo > k$}
	\State $\xonethird \gets \floor{(\xhi - \xlo) / 3} + \xlo$
	\State $\xtwothird \gets \floor{2(\xhi - \xlo) / 3} + \xlo$
	\State $(\_, \tonethird) \gets$ \Call{Search1}{$\timefnc, \xonethird, \ymin, \ymax, k$}
	\State $(\_, \ttwothird) \gets$ \Call{Search1}{$\timefnc, \xtwothird, \ymin, \ymax, k$}
...
\Statex \{-- linear search --\}
\State $x \gets \xlo$ \Comment{$(x, y, t)$ represent smallest $\timefnc(x, y)$ encountered}
\State $(y, t) \gets$ \Call{Search1}{$\timefnc, x, \ymin, \ymax, k$}
\State $\xlo \gets \xlo + 1$
\While{$\xlo \le \xhi$}
	\State $(y', t') \gets$ \Call{Search1}{$\timefnc, \xlo, \ymin, \ymax, k$}
	\If{$t' < t$} $(x, y, t) \gets (\xlo, y', t')$ \EndIf
	\State $\xlo \gets \xlo + 1$
\EndWhile
\State \Return $(x, y, t)$
\EndProcedure
