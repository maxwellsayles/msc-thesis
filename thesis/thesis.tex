...
Finally, Lehmer's extended GCD algorithm is especially useful when the inputs are several machine words.  For 128-bit inputs, we implemented Lehmer's extended GCD using the extended Euclidean algorithm for 32 and 64-bit machine words, as well as Stein's 4-bit windowed right-to-left binary extended GCD and our simplified left-to-right binary extended GCD, both for 64-bits.  For 32-bit and 64-bit inputs, we implemented Lehmer's extended GCD using 8-bit machine words.  For 8-bit machine words it is possible to precompute the resulting $2 \times 2$ matrix for all possible 8-bit inputs.  Since there are two inputs, there exist 65536 possible pairs of values.  The coefficients of the resulting $2 \times 2$ matrix can be bound by 8-bits each, and so the resulting table requires 256Kb of memory. This simplifies the extended GCD computation since the inner loop of Lehmer's extended GCD becomes a lookup from this table.
