...
In our implementation, Lehmer's GCD is especially useful when the arguments to the GCD are larger than 128-bits.  However, when we consider artificially smaller machine words, it is possible to precompute the resulting $2 \times 2$ matrix for all possible inputs into the single precision Extended GCD computation.  We did this for 8-bit machine words.  As there are two inputs, each 8-bits in size, there are 65536 possible pairs of inputs.  The coefficients of the resulting $2 \times 2$ matrix can be bound by 8-bits each, and so the table requires 256Kb of memory. This simplifies the GCD computation dramatically since the inner loop becomes a lookup from this table.
...
When possible, we exploit the fixed size of 32-bit, 64-bit, and 128-bit integer bounds, and make use of their binary representation.  Many of the techniques we use are described in the book ``Hacker's Delight'' \cite{Warren2002}.  Simple techniques involve powers of 2.  Computing $x2^k$ corresponds to shifting $x$ left by $k$ bits (denoted $x \ll k$).  When $x$ is divisible by $2^k$, computing the integer $x / 2^k$ corresponds to shifting $x$ right by $k$ bits (denoted $x \gg k$).  When we want $X \bmod 2^k$, we achieve this using a bitwise $x \texttt{ and } 2^k-1$ operation.  Assuming a two's complement representation of machine words, the most significant bit of an integer $x$ is set if $x < 0$ and clear otherwise.  We can generate a bit-mask according to this test by using an arithmetic shift right operation (in the C programming language this is a right shift on a signed integer).  Let $m$ denote the number of bits in a machine word; then by using an arithmetic shift right on $x$ by $m-1$ bits, the result is either $m$ set bits when $x < 0$ or $m$ clear bits otherwise.  We denote this operation $\texttt{signmask}(x)$.

Using 

Let $\oplus$ denote bitwise exclusive-or, and let $\lnot$ denote bitwise negation.  
