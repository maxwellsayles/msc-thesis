...
\subsection{Optimal Additive 2, 3 Chains}
\label{subsec:optAddChains}
...
When the exponent is not too large, Imbert and Philippe \cite{Imbert2010b} give a method to compute optimal additive strictly chained 2,3 partitions.  These partitions do not permit subtraction and every term is strictly less than and divides all subsequent terms. These partitions take the form
\[
	n = \sum_{i=0}^k 2^{a_i}3^{b_i}
\]
for $a_i, b_i \in \ZZgez$ where $2^{a_i}3^{b_i}$ divides $2^{a_j}3^{b_j}$ and $a_i < a_j$ and $b_i < b_j$ for all $i < j$.

We discussed this technique in detail in Subsection \ref{subsec:optAddChains}.  Here, we only add that in our experiments, we were able to modify the algorithm of Imbert and Philippe so that it incorporates the average cost of multiplication, squaring, and cubing in the ideal class group, so that the resulting additive 2,3 chains should give the optimal average time for a chain under these restrictions.  Since their algorithm uses an exponential amount of time in the length of the input, we were only able to compute optimal additive chains for small exponents.  This is useful, however, when we consider methods that partition larger exponents into smaller blocks using their binary representation, or when we consider exponentiating by a list of prime powers, rather than by their large product.
