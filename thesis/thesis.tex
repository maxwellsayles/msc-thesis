...
TODO: Mention that giant steps are twice last baby step rounded up, to take advantage of inversions.  \cite[p.50]{Sutherland2007}

TODO: Small proof that baby-steps giant-steps work.  If it's a small proof
...
In practice, the efficiency of the SuperSPAR factoring algorithm depends on the smoothness of the order of the ideal class $\aclass$ with respect to the power primorial $E$ chosen.  The larger the divisor that $\ord(\aclass)$ and $E$ have in common, the smaller the order of $\cclass = \aclass^{\left(2^k\right)E}$ and the smaller the bound $b$ we need for the baby steps and giant steps.  Changing class groups by changing the square free multiplier $k$ for the discriminant $\Delta$ also changes the order of random elements in the new group.  As such, we trade off the number of ideal classes or multipliers $k$ against the largest prime used in the power primorial $E$.
