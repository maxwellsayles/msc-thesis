...
\]
where $p_i$ is the $i^{\textrm{th}}$ prime. Let $e_i = \floor{\log_{p_i} M}$ and $E = 2^{e_2} \times 3^{e_3} \times \cdots \times {p_n}^{e_n}$.  Then we compute $\beta = \alpha^E$, and use baby steps coprime to $P_n$ and giant steps that are a multiple of $P_n$.  Following Sutherland \cite[p.57]{Sutherland2007}, we select the largest $P_n \le \sqrt{M}$ and then maximize $m$ such that $m^2P_n \phi(P_n) \le M$ where $\phi(P_n)$ is number of integers coprime to $P_n$ given as
\[
	\phi(P_n) = (2-1) \times (3-1) \times \cdots \times (p_n - 1) = \prod_{i=1}^n (p_i - 1).
\]
The bound on the largest baby step $b$ is $b = m P_n$.  We then compute $h' = \ord(\beta)$ and use $\alpha^{h'}$, $E$, and a fast order finding algorithm (such as one from \cite[Chapter 7]{Sutherland2007}) to compute $h = \ord(\alpha)$.
