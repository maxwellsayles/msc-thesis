...
Theoretically, SuperSPAR takes the number of baby steps and giant steps proportional to the largest prime, $p_t$, used in the exponent of the exponentiation stage, such that $p_t \le N^{1/2r}$, $p_t$ is as large as possible, and $r = \sqrt{\ln N / \ln \ln N}$ (see Section \ref{sec:superSpar}).  The idea being that the search stage should use no more than a constant number of operations as the exponentiation stage.   However, by choosing the number of baby steps and giant steps independently from the exponent used in the exponentiation stage, SuperSPAR can perform better in practice.  Since we would like to take baby steps coprime to a multiple of some primorial, this section discusses the method used by our implementation to generate candidate baby step bounds, which we test against empirically in Section \ref{sec:ssparEmpirical}.

The success of the search stage depends on the largest exponent $2js$ generated by the giant steps.  The idea here is to generate a set $\stepBounds$ of primorial and multiplier pairs used as candidates for the baby step bound $s$.  For each pair, let $\stepCount$ be a function from some multiple $m$ of the primorial $P_w$ to the total number of baby steps and giant steps taken during the search stage of the algorithm,
\[
	\stepCount(m, w) = 2 m \phi(P_w).
\]
Notice we take $m \phi(P_w)$ baby steps plus $m \phi(P_w)$ giant steps.  Each giant step is of size $2mP_w$, so let $\lastStep$ be a function that computes the exponent of the final giant step,
\[
	\lastStep(m, w) = 2 m^2 \phi(P_w) P_w.
\]
In general, the set $\stepBounds$ will consist of pairs $(m, w)$ such that for all $(m', w')$ either
\[
	\lastStep(m', w') < \lastStep(m, w) \textrm{ or } \stepCount(m', w') > \stepCount(m, w).
\]
In other words, the set $\stepBounds$ consists of the pairs $(m, w)$ such that for all other pairs $(m', w')$, either the last giant step $\lastStep(m', w')$ is not as large, or the total number of baby steps and giant steps $\stepCount(m', w')$ is greater.  The set $\stepBounds$ is defined as
\begin{equation}
\label{eq:stepBounds}
...
\STATE let $(m_1, w_1), (m_2, w_2), ..., (m_n, w_n)$ be the list of elements generated by sorting the set $\mathcal T$ in ascending order by total number of steps $\stepCount(m, w)$
\STATE $\stepBounds_B \gets \{\}$
\STATE $(m, w) \gets (m_1, m_w)$
\FOR{$i = 2$ to $n$}
...
\end{algorithmic}
\end{algorithm}
First the algorithm generates all pairs $(m, w)$ such that $\stepCount(m, w) \le B$.  There is a finite number of these pairs, since for a given $w$ there are $\floor{B / 2\phi(P_w)}$ possible values of $m$, and there exists some $P_w > B$ so there are a finite number of possible values of $w$.  The algorithm then sorts these pairs in ascending order by the total number of steps $\stepCount(m, w)$ and chooses the pair that generates the fewest number of steps as a candidate $(m, w)$ for the set $\stepBounds_B$.  The pairs are then iterated from the second pair to the last, and the exponent of the final giant step of each pair is compared with the candidate pair.  If the final giant step has a larger exponent, then the candidate pair is output to the set $\stepBounds_B$ and the current pair becomes the next candidate pair.  The first $(m_1, w_1)$ is always in 
