...
Table \ref{tab:theorySteps} provides several values predicted by the theory in Section \ref{sec:superSpar} for integers of various sizes.  In theory, SuperSPAR takes the number of baby steps and giant steps proportional to the largest prime, $p_t$, used in the exponent of the exponentiation stage, such that $p_t \le N^{1/2r}$, $p_t$ is as large as possible, and $r = \sqrt{\ln N / \ln \ln N}$.  In the table, the total number of steps, $2m\phi(P_w)$, is the smallest value greater than or equal to $\floor{N^{1/2r}}$.  The table also shows the search range given by the specified multiple $m$ and primorial $P_w$.  Two things are of interest here.  The first is that for integers of size 128-bits, the theory predicts that the search stage should require around 21473 steps.  For this reason, in Section \ref{sec:ssparEmpirical}, we bound the search bound list so that each candidate takes no more than 32768 steps.  We choose 32768 to be $\ge 21473$ and provide some padding.  The second thing of interest is that for integers of size 128-bits, the largest exponent generated during the search stage, as predicted by the theory, is 1173110400, which fits within 31-bits.  This justifies our use of 32-bit values in the lookup table described in Section \ref{sec:ssparHashing}.
