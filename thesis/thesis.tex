...
Theoretically, SuperSPAR takes the number of baby steps and giant steps proportional to the largest prime, $p_t$, used in the exponent of the exponentiation stage, such that $p_t \le N^{1/2r}$, $p_t$ is as large as possible, and $r = \sqrt{\ln N / \ln \ln N}$ (see Section \ref{sec:superSpar}).  The idea being that the search stage should use no more than a constant number of operations more than the exponentiation stage.   However, by choosing the number of baby steps and giant steps independently from the exponent used in the exponentiation stage, SuperSPAR can perform better in practice.  Since we would like to take baby steps coprime to a multiple of some primorial, this section discusses the method used by our implementation to generate suitable bounds.  We then evaluate these bounds using SuperSPAR to split integers of various sizes in Section \ref{sec:ssparEmpirical}.
