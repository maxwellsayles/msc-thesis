...
\end{equation}
Therefore, we only need compute $\bclass^{\left(2^k\right)}$ for the smallest $k \le \floor{\log_2 {h_\Delta}}$ such that $\bclass^{\left(2^k\right)} = \idclass$ if such a $k$ exists. If such a $k$ does not exist, then the algorithm continues with the second stage.
...
In Stage 1 of SPAR, we take the first $t$ primes $p_i \le N^{1/2r}$ for $r = \sqrt{\ln N / \ln \ln N}$, and let $e_i = \max \{ v : {p_i}^v \le {p_t}^2 \}$.  We then compute
\[
	\bclass = \aclass^{\prod_{i=2}^t {p_i}^{e_i}},
\]
for a random ideal class $\aclass$ and $\cclass = {\bclass}^{\left(2^k\right)}$ for $k=\floor{\log_2 {h_\Delta}}$ (where $h_\Delta$ is given by Equation \ref{eq:hDelta}).  If $\bclass \neq \cclass = \idclass$, then we can find an ambiguous ideal and attempt to factor the discriminant.  Otherwise we proceed to Stage 2.  In Stage 2, we perform a random walk on the cyclic group generated by $\cclass$.  The purpose of Stage 1 is to find an ideal class $\bclass$ whose order is known not have any of the odd primes $p_2, ..., p_t$ as divisors.  If the odd part of $\ord(\bclass)$ divides $\prod_{i=2}^t {p_i}^{e_i}$ and the order of $\bclass$ is even, then computing $\cclass$ produces an ambiguous ideal and we can attempt to factor $N$.
...
TODO: describe all the steps of the implemented algorith, without saying the exact values of things, just that we determine them empirically in Chapter \ref{chap:superSparExperiments}
