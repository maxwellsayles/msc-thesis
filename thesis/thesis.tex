...
The bounded primorial steps algorithm from Section \ref{sec:primorial} and used by the SuperSPAR factoring algorithm (Section \ref{sec:superSpar}) computes baby-steps $\aclass^i$ for $1 \le i \le b$ where $i$ is coprime to some primorial $P_w$ and $b$ is a multiple of $P_w$.  By considering only values coprime to $P_w$ the running time complexity of the bounded primorial steps algorithm achieves an upper bound of $O(\sqrt{M/\log \log M})$.  If an implementation has to test whether each $i$ from $1$ to $P_w$ is coprime to $P_w$, the algorithm cannot achieve this.

This section discusses two methods to compute the set of values coprime to some primorial $P_w$, namely sieving and wheeling.  This set is precomputed separately from the bounded primorial steps algorithm, and so is available as input to the algorithm.  The runtime complexity of computing the set of coprime values is separate from the runtime complexity of the bounded primorial steps algorithm.  Since a practical method for computing baby-steps $\aclass^i$ for $1 \le i \le b$ is to iterate $i$ over consecutive coprime values, computing the delta between neighbouring coprime values is sufficient.  An implementation then iterates $i$ by adding the next delta to the current value of $i$ in order to move it to the next coprime value.

\subsection{Coprime Sieve}

\subsection{Coprime Wheel}

For some primorial $P_w$, there is a table of values 1 through $P_w$ inclusive.  Mark all entries coprime to $P_w$ with 1 and all other entries with 0.  This is extended to $P_{w+1}$ by creating copies of the $P_w$ wheel into a table of $P_{w+1}$ values and indexes that start at multiples of $P_w$.  There are $p_{w+1} - 1$ copies in total.  Then set each index that is a multiple of $p_{w+1}$ to 0.  The remaining indices with a value of 1 are coprime to $P_{w+1}$.

The idea is to do this with deltas.  We have a list of deltas that take us from one coprime value to another for some primorial $P_w$.  Starting at index $P_w$, we move to each coprime candidate in the larger wheel.  If the candidate is not a multiple of $p_{w+1}$, then the index is coprime to $P_{w+1}$ and we write out the delta to the new delta list, reset the delta counter, and continue with the next coprime delta from the list for $P_w$.  If the index is a multiple of $p_{w+1}$, then the index is not coprime to $P_{w+1}$ and we do not output it to the list, do not clear the delta counter, and continue with the next coprime.

\begin{algorithm}[h]
\caption{Compute deltas for $P_{w+1}$ given deltas for $P_w$.}
\label{alg:deltaWheel}
\begin{algorithmic}[1]
\REQUIRE A primorial $P_w$, \\
deltas $d_1, ..., d_{\phi(P_w)}$ such that $d_i = c_{i+1} - c_i$ for consecutive values $c_i$ and $c_{i+1}$ coprime to $P_w$ where $c_1 = 1$, \\
and the next prime $p_{w+1}$. \\
Let $P_{w+1} = p_{w+1} P_w$ and $\phi(P_{w+1}) = (p_{w+1}-1)\phi(P_w)$.
\ENSURE deltas $d'_1, ..., d'_{\phi(P_{w+1})}$ such that $d_j' = c'_{j+1} - c'_j$ for consecutive values $c'_j$ and $c'_{j+1}$ coprime to $P_{w+1}$ where $c'_1 = 1$.
