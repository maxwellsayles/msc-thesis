...
In Subsection \ref{subsec:rtolChains}, we introduced a method for computing 2,3 chains from low-order to high-order that is a natural extension of the binary representation or non-adjacent form of an exponent.  In this method, we reduce the exponent by 2 while it is even, and by 3 while it is divisible by 3.  Once it is neither divisible by 2 nor divisible by 3, we can add or subtract 1 to move it to the nearest multiple of both 2 and 3.
...
Division by 3 is relatively expensive when compared to computing the remainder.  A common approach is to precompute a single word approximation of the inverse of 3, and then to use multiplication by an approximation of the inverse and then to adjust the result \cite{Granlund1994,Warren2002,Moller2011}.  As the GNU Multiple Precision (GMP) library implements exact division by 3, we use this implementation.


\subsection{Windowed Right-to-Left Chains}

Windowing is a technique to improve the performance of right-to-left binary exponentiation and non-adjacent form exponentiation.  We also use it for our right-to-left binary GCD computation.  Similarly, we use windowing to improve the performance of a right-to-left 2,3 chain.  Specifically, we experimented with a window size of $2^2 3^2$.  Again, while the exponent is even, we reduce it by 2, and while it is divisible by 3, we reduce it by 3.  In a non-windowed variant, we would add or subtract 1 to the current exponent so that it was a multiple of 6.  In the windowed variant, we evaluate the exponent $n$ modulo $2^2 3^2$, which gives us $n \equiv 1, 5, 7, 11, 13, 17, 19, 23, 25, 29, 31, 35 \pmod {36}$.  As there are 12 different equivalence classes, and for each equivalence class we could either add or subtract 1 from $n$, we have $2^{12}$ different strategies for a right-to-left 2,3 chain using a window size of $2^2 3^2$.  For each strategy, we evaluated the cost to exponentiate the following 50 different primorials using our implementation of 64-bit and 128-bit ideal class arithmetic (see Section \ref{sec:idealArithmetic}).  For $1 \le k \le 50$, let $j=100k$ and the primorial $P_j=\prod_{i=1}^j p_i$ where $p_i$ is the $i^{\textrm{th}}$ smallest prime.  For 48 out of 50 of the primorials tested, the same strategy lead to the cheapest cost of exponentiation\footnote{For the primorial $P_{200}$, this strategy was the third most efficient, and for $P_{500}$ it was the second most efficient.}, which was to subtract 1 from $n$ when $n \equiv 1, 5, 13, 17, 25, 29 \pmod{36}$ and to add 1 otherwise.

Since the windowed variant of a 2,3 chain computes $n \bmod 36$, we give a fast method to compute this.  First write $n = 4x + r_4$ for integers $x$ and $r_4$ such that $0 \le r_4 < 4$.  Then write $x = 9y + r_9$ for integers $y$ and $r_9$ such that $0 \le r_9 < 9$.  Substituting the second equation into the first we have
\begin{align*}
	n &= 4(9y + r_9) + r_4 \\
	  &= 36y + 4r_9 + r_4.
\end{align*}
...
\]
and recursively compute $\floor{n/64} \pmod 9$.  To compute $n \bmod 9$, we partition $n$ into blocks of 6-bits (since $2^6 = 64$) and sum each 6-bit block modulo 64.  We then compute the sum $s \bmod 9$.  Again, since our target architecture has 64-bit machine words, we improve upon this by first partitioning $n$ into 192-bit blocks\footnote{We use 192-bit blocks since a 64-bit machine word evenly partitions a 192-bit block and $2^{192} \equiv 1 \pmod 9$.} and compute an intermediate sum of each 192-bit block.  We then partition the intermediate sum into 6-bit blocks and compute the final solution.



\subsection{Left-to-Right Chains}

In Subsection \ref{subsec:ltorChains} we introduced a greedy algorithm for computing 2,3 representations from high-order to low-order.  Briefly, for a given exponent $n$ and for $a_i, b_i \in \ZZgez$ and \mbox{$s_i \in \{-1, 1\}$}, we compute the term $s_i2^{a_i}3^{b_i}$ such that $\left|n-s_i2^{a_i}3^{b_i}\right|$ is as small as possible.  We then recurse on the result $n - s_i2^{a_i}3^{b_i}$.  This approach generates 2,3 representations with no restriction on the relationship between the number of squares or cubes in subsequent terms.  A chain restricts each term such that the number of squares and cubes is monotonically decreasing. Chains can be generated by restricting each $a_i$ and $b_i$ such that $0 \le a_i \le a_{i-1}$ and $0 \le b_i \le b_{i-1}$.  Furthermore, the cost of a left-to-right chain or representation varies when a global bound $A$ and $B$ are applied such that $a_i \le A$ and $b_i \le B$ for all terms.
...
Figure \ref{fig:dbnsL2rVaryBounds} shows the cost of an exponentiation using a left-to-right representation for an exponent that is the product of the first 1000 primes (a number with 11271 bits).  The horizontal axis indicates a global bound $A$ such that all $a_i \le A$.  A global bound $B$ is chosen for the exponent $n$ such that $B = \ceil{\log_3(n/2^A)}$.  The end points of the graph correspond to a representation that uses only cubing and multiplication (on the left side) and a representation that uses only squaring and multiplication (on the right side).  The slope of the graph varies with the ratio of the cost to square relative to the cost to cube, as can be seen between our 64-bit and 128-bit implementations of ideal arithmetic.  In general, bounding the maximum number of squares and cubes give representations that lead to faster exponentiation in the ideal class group than representations where the number of squares or cubes is left unbound.  For this particular exponent, a representation with an unbounded number of squares and cubes takes approximately 7.3 milliseconds for our 64-bit implementation and 17.2 milliseconds for our 128-bit implementation.

When the exponent $n$ is known in advance, we generate the 2,3 chain or representation ahead of time and do not include the time to compute the chain or representation in the overall time of the exponentiation.  Since varying the bounds on the number of squares and cubes also varies the cost of exponentiating, in practice we iterate over a range of bounds for the number of squares in order to find the most efficient chain or representation.  One may instead iterate the range of bounds for the number of cubes, as there are $\ceil{\log_3 n}$ integral bounds for the number of cubes and $\ceil{\log_2 n}$ integral bounds for the number of squares.
