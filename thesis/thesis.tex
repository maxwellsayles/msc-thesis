...
When selecting a multiple of a primorial for the search stage, candidates were selected from the list generated by Algorithm \ref{alg:stepBounds} in Section \ref{sec:ssparBabyStepBounds}.  The theory predicts that for integers 100-bits in size, that the search stage should require approximately 9802 steps (see Table \ref{tab:theorySteps}).  Since we did not expect our implementation of SuperSPAR to be competitive for integers larger than 100-bits, the candidate list was bounded such that no candidate used more than 16384 total steps for the search stage (16384 was chosen here because it is larger than 9802 and provides some padding).

To indicate the effect of iterating the largest prime in the exponent $E$ and the search bounds $(m, w) \in S_{16384}$, three sets of 10,000 random semiprime integers $N = p \cdot q$ were generated for $p$ and $q$ prime and half the size of $N$.  The three sets consist of integers of 48-bits, 56-bits, and 64-bits.  For 64-bit integers, the theory predicts that the largest prime, $p_t$, should be approximately 655 (see Table \ref{tab:theorySteps}), and that the total number of steps in the search phase should be same.  To provide some padding, the largest prime in the exponent was iterated from 13 to 947, and the search bound was iterated over the first 120 entries in the list $S_{16384}$ (the $120^{\textrm{th}}$ entry uses a total of 7488 steps).  We hoped to see that the average time to factor integers of a given size would initially decrease and then later increase as each parameter was iterated.  Ideally this would indicate the existence of a global minimum.  However, the purpose here is not to find values for the exponent $E$ and search bound $mP_w$ that perform well on average, but only to see the shape of the search space.  We chose these sets of integer sizes as they are relatively efficient with which to work.  In Subsection \ref{subsec:ssparNoisyCurve}, we propose a method for searching this 2 dimensional space more efficiently in order to find parameters that work well on average.  By applying a more efficient search technique than the linear search used here, we are able to empirically test larger sets of semiprime integers in order to get a better representation of parameters that work well on average.  We are also able to do this for every even integer size in the range 16-bits to 100-bits.
