...
\caption{Average time to multiply, square, or cube ideal classes.}
...
In this section, we discuss several approaches to exponentiating an ideal class representative to a large exponent.  In particular, we consider large exponents that are the product of all primes less than some bound\footnote{The product of all primes less than a bound $B$ is called a ``primorial''.  See Section \ref{sec:primorial} for the definition of ``primorial'' and our motivation using them.} where the exponent is known in advance.  This allows us to precompute the sequence of operations in the exponentiation, so that the exponentiation may be as fast as possible without incurring the cost of computing the sequence of operations themselves.

We begin by recalling exponentiation techniques from Chapter \ref{chap:exponentiation}.  These include techniques that use signed and unsigned base two representations, greedy right-to-left and left-to-right double-base representations, and a tree-based approach. We also consider several extensions to these ideas.  For small exponents (16-bits or less), we were able to exhaustively compute all representations under certain constraints.  This allows us to use larger exponents by partitioning the large exponent into 16-bit blocks or by using its factorization. Finally, we compare each method on several primorials to find the best method in practice.

In the previous Section, we discussed our implementations of ideal class arithmetic using 64-bit and 128-bit operations.  In the case of the 64-bit implementation, the largest discriminants supported are those that fit within 59-bits, and for the 120-bit implementation, the largest discriminants fit within 118-bits.  We also computed the average cost of multiplication, squaring, and cubing for class groups with discriminants of these sizes.  Size 59-bits and 118-bits represent an upper bound of the cost of arithmetic operations within each implemenation, our exponentiation experiments focus on improving the time to exponentiation assuming the average cost of operations for the largest discriminants supported by each implementation.
...
In Subsection \ref{subsec:ltorChains} we introduced a greedy algorithm for computing 2,3 representations from high-order to low-order.  Briefly, for a given exponent $n$ and for $a_i, b_i \in \ZZgez$ and \mbox{$s_i \in \{-1, 1\}$}, we compute the term $s_i2^{a_i}3^{b_i}$ such that $\left|n-s_i2^{a_i}3^{b_i}\right|$ is as small as possible.  We then recurse on the result $n - s_i2^{a_i}3^{b_i}$.  As is, this produces unchained representations -- a chain can be generated by restricting each $a_i$ and $b_i$ such that it is no greater than the $a_{i-1}$ and $b_{i-1}$ from the previous term.  Placing a bound on the number of squares and cubes can also be done globally, i.e.\ for every $a_i, b_i$ we have $0 \le a_i \le A$ and $0 \le b_i \le B$. When a bound is applied globally, the cost of a left-to-right chain or representation varies as the global bound varies.
...
Here we consider two variations to the approach of maintaining the $k$ best partial representations. We say that a partial representation is ``better'' when the $x$ term is smaller, or if the cost of the 2,3 summation is less when both partial representations have equal $x$ terms.  This way, we are able to incorporate the average cost of multiplication, squaring, and cubing into the search for representations.
...
For both variations, we bound $a$ and $b$ such that $0 \le a \le U$ and $0 \le b \le U$.  As the bound $U$ increases linearly, the number of representations increases quadratically.  Figure \ref{fig:pm2a3bVaryMax} shows the average time to exponentiate by the $1000^{\textrm{th}}$ primorial as the bound $U$ increases.  As $U$ increases, computing representations using this approach quickly becomes intractable. Representations using $U=0$ only require 22 milliseconds to compute, while $U=32$ uses 33 seconds.  We found that for our purposes, $U=16$ is an acceptable trade off between the average time to exponentiate and the time to generate the 2,3 representation.

\begin{figure}[H]
\centering
\mbox{
	\subfigure{\includegraphics[width=3in]{pm2a3b_vary_max-64}}
	\subfigure{\includegraphics[width=3in]{pm2a3b_vary_max-128}}
}
\caption{Performance of varying the bounds $a, b \le U$ on $(x \pm 2^a3^b)/(2^c3^d)$ when exponentiating by the primorial $P_{1000}$.}
\label{fig:pm2a3bVaryMax}
\end{figure}
...
For small enough inputs, the number of possible additive 2,3 chains is sufficiently reduced that we can compute all such chains in order to find the most cost effective.  Here we consider a different approach to searching for representations -- for a set of possible representations, we compute each integer represented and record the cheapest representation for each integer represented by the set.
...
In our experiments, we search for representations for all 16-bit integers. We start with $A=\ceil{\log_2\left(2^{16}\right)} = 16$, $B=\ceil{\log_3 \left(2^{16}\right)} = 11$, and iterate the number of terms $k$ from 1 through 5. For representations of $k=6$ terms, our implementation took several days to complete.  For our 64-bit implementation of ideal arithmetic, no integer was associated with a minimal cost representation of more than 5 terms.  When we ran our search again using $A=18$ and $B=12$ and found that none of our minimal representations were improved.  For our 128-bit implementation of ideal arithmetic, we found that (TODO) number of integers had better minimal representations with 6 terms.  This difference can be explained by the difference in the average cost to multiply, square, and cube ideals in the two different implementations.
...
Figure \ref{fig:expBinNaf} compares binary exponentiation against a right-to-left non-adjacent form representation.  The non-adjacent form representation leads to faster exponentiations in all cases.  Figure \ref{fig:expDbnsR2L} compares the non-windowed right-to-left 2,3 chain method to the $2^2 3^2$ windowed method.  The $2^23^2$ windowed method out performs the non-windowed method for all exponents.  Figure \ref{fig:expDbnsL2R} compares left-to-right 2,3 representations with that of maintaining the 4 best $\left||x|-2^a3^b\right|$ approximations.  In this case, maintaining the 4 best approximations performs best.  Figure \ref{fig:expPmVariants} compares the three different techniques of adding or subtracting a value to a partial representation and then reducing by a power of 2 and 3.  Here, computing candidates $(x-2^a3^b)/(2^c3^d)$ leads to representations that exponentiate the fastest.  Figure \ref{fig:expBlockList} compares the two methods that rely on the best found 16-bit representations.  In this case, when the factorization of the exponent is easily known, it is faster to exponentiation by the list of prime factors than it is to represent the exponent using 16-bit blocks.  Finally, Figure \ref{fig:expWinners} compares the best performer from each category.  For both our 64-bit and 128-bit implementations and for all primorials tested, the method of iterating on the 4 best $\left||x| - 2^a3^b\right|$ approximations leads to representations with the fastest time to exponentiate.

\begin{figure}[h]
\centering
\mbox{\subfigure{\includegraphics[width=3in]{binary_vs_naf-64}}
	  \subfigure{\includegraphics[width=3in]{binary_vs_naf-128}}}
\caption{Binary exponentiation compared to Right-to-Left Non-Adjacent Form exponentiation.}
\label{fig:expBinNaf}
\end{figure}

\begin{figure}[h]
\centering
\mbox{\subfigure{\includegraphics[width=3in]{dbns_r2ls-64}}
	  \subfigure{\includegraphics[width=3in]{dbns_r2ls-128}}}
\mbox{\subfigure{\includegraphics[width=3in]{dbns_r2ls-64-zoom}}
	  \subfigure{\includegraphics[width=3in]{dbns_r2ls-128-zoom}}}
\caption{Right-to-Left 2,3 Chain compared with a $2^2 3^2$ windowed Right-to-Left 2,3 Chain.}
\label{fig:expDbnsR2L}
\end{figure}

\begin{figure}[h]
\centering
\mbox{\subfigure{\includegraphics[width=3in]{dbns_l2rs-64}}
      \subfigure{\includegraphics[width=3in]{dbns_l2rs-128}}}
\mbox{\subfigure{\includegraphics[width=3in]{dbns_l2rs-64-zoom}}
      \subfigure{\includegraphics[width=3in]{dbns_l2rs-128-zoom}}}
\caption{Left-to-Right 2,3 Representations compared with iterating on the 16 best approximations of $\left||x|-2^a3^b\right|$.}
\label{fig:expDbnsL2R}
\end{figure}

\begin{figure}[h]
\centering
\mbox{\subfigure{\includegraphics[width=3in]{pm_variants-64}}
      \subfigure{\includegraphics[width=3in]{pm_variants-128}}}
\mbox{\subfigure{\includegraphics[width=3in]{pm_variants-64-zoom}}
      \subfigure{\includegraphics[width=3in]{pm_variants-128-zoom}}}
\caption{A comparison of iterating on the 4 best candidates where each candidate is either $(x \pm 1)/(2^c3^d)$, $(x \pm 2^a3^b)/(2^c3^d)$, or $(x \pm 2^a \pm 3^b)/(2^c3^d)$.  Iterating on $(x \pm 2^a3^b)/(2^c3^d)$ gives representations that lead to faster exponentiations.}
\label{fig:expPmVariants}
\end{figure}

\begin{figure}[h]
\centering
\mbox{\subfigure{\includegraphics[width=3in]{block_vs_list-64}}
      \subfigure{\includegraphics[width=3in]{block_vs_list-128}}}
\caption{The average time to exponentiate using a 16-bit block representation of an exponent as compared to using the prime factorization of the exponent.  In each case, we use the best 16-bit representations found earlier.  When the prime factorization is easily known, this method leads to faster exponentiations than a 16-bit block method for exponentiation.}
\label{fig:expBlockList}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics{winners-64}
\includegraphics{winners-128}
\caption{Here we compare the best performers from each category above.  For both our 64-bit implementation and our 128-bit implementation, representations generated by iterating on the 4 best $\left||x| - 2^a3^b\right|$ result in the fastest average times to exponentiate by a primorial.}
\label{fig:expWinners}
\end{figure}
