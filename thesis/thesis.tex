...
\begin{defn}
An \emph{ideal} $\mathfrak a$ is an additive subgroup of an order $\OO$ with the property that for any $a \in \mathfrak a$ and $\xi \in \OO$, it holds that $\xi a$ and $a \xi$ are both elements of the ideal $\mathfrak a$. When $\OO$ is a quadratic order, for $\alpha, \beta \in \OO$, the set $\ideal a = \{x \alpha + y \beta : x, y \in \OO\}$ is an ideal in the order $\OO$ and is denoted $(\alpha, \beta)$ \cite[p.16]{Ramachandran2006}.
\end{defn}

Every ideal of a quadratic order $\OO$ can be represented by at most two generators \cite[p.125, \S 10]{Cohn1980}, while some can be represented by a single generator.  An ideal represented by a single generator is a \emph{principal} ideal and is denoted $(\alpha) = \{ x \alpha : x \in \OO \}$ \cite[p.87]{Jacobson2009}.

\begin{thm}
\label{thm:idealZModule}
\cite[p.13]{Jacobson1999} When $\OO_\Delta = [1, f\omega_0]$ is an order of a quadratic field, a non-zero ideal $\ideal a$ of $\OO_\Delta$ can be uniquely written as a two dimensional $\ZZ$-module 
\[
	\ideal a = s\left[a, \frac{b+\sqrt{\Delta}}{2} \right]
\]
for $a, b \in \ZZ, s > 0, a > 0$, $\gcd(a, b, (b^2-\Delta)/4a)=1$, $b^2 \equiv \Delta \pmod{4a}$, and $b$ is unique $\bmod ~2a$. When $s \in \ZZ$, $\ideal a$ is an \emph{integral} ideal, and when $s \in \QQ$, $\ideal a$ is a \emph{fractional} ideal.  Finally, when $s = 1$, the ideal $\ideal a$ is \emph{primitive}.
\end{thm}

%TODO: Define multiplication before identity and inverse.  What that inverse doesn't use a fractional ideal. 

The \emph{identity} ideal is $\OO_\Delta = [1, f\omega_0]$ since $\ideal a = \ideal a \OO_\Delta = \OO_\Delta \ideal a$ \cite{Cohn1980}. An ideal $\ideal a = s[a, (b+\sqrt{\Delta})/2]$ is \emph{invertible} if there exists an ideal $\ideal b$ such that $\ideal a \ideal b = \OO_\Delta$, and an ideal $\ideal b$ exists when $\gcd(a, b, (b^2-\Delta)/(4a)) = 1$ \cite[p.14]{Jacobson1999}. The inverse is given by \cite[pp.14--15]{Jacobson1999}
\[
