...
\]
and record the lowest average cost of a representation for each integer discovered.

We wanted to search for representations for all 16-bit integers, so we started our search with $A=\ceil{\log_2\left(2^{16}\right)} = 16$, $B=\ceil{\log_3 \left(2^{16}\right)} = 11$, and the number of terms $i$ as 1.  We increased $i$ by 1 each round, so long as it resulted in at least one integer in the range 1 through $2^{16}$ having a reduced cost.  We found that for representations of 1 through 6 terms for integers in our range, that no integer was associated with a minimal cost representation of more than 5 terms.  We then ran our search again using $A=18$ and $B=12$ and found that none of our minimal representations changed.
