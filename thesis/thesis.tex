...
Here we consider two variations to the approach of maintaining the $k$ best partial representations, where each variation incorporates the average cost of multiplication, squaring, and cubing.  We say that a partial representation is ``better'' when the $x$ term is smaller, or if both representations have equal $x$ terms, when the cost of the 2,3 summation is less.

The first variation we consider is to generate several new nodes with integer values of the form $(x \pm 2^a3^b)/(2^c3^d)$ for bounded $a$ and $b$ such that $0 \le a \le A$ and $0 \le b \le B$.  The second variation is to include another term before each reduction step, i.e.\ each node generates values of the form $(x \pm 2^{a_1}3^{b_1} \pm 2^{a_2}3^{b_2}) / (2^c 3^d)$.  Notice that the first variation includes the forms $(x \pm 1)/(2^c 3^d)$ and so should give representation no worse than the method suggested by Doche and Habsieger \cite{Doche2008} (covered in Subsection \ref{subsec:pm1Tree}).  The assumption is that by adjusting $x$ by a fixed length 2,3 representation, we are more likely to find a multiple of a large $2^c3^d$ window than we are by only adding or subtracting one to $x$.

Just as when computing left-to-right 2,3 representations, placing a global bound on the maximum number of squares and cubes affects the average cost of exponentiation.  As before, we iterate a bound $B$ on the number of cubes from 0 to $\ceil{\log_3 n}$, and bound the number of squares $A = \ceil{\log_2 (n/3^B)}$, to find the best representation.
...
The approach of maintaining a set of the $k$ best partial representations of an exponent can be adapted to that of the left-to-right 2,3 representation from Subsections \ref{subsec:ltorChains} and \ref{subsec:ltorChains2}.  For a given integer $x$ we say that $2^a3^b$ is a best approximation of $|x|$ when $\left||x| - 2^a3^b \right|$ is minimized.  The algorithm for the left-to-right representation finds a best approximation for $x$ and then repeats on the positive difference. However, instead of only iterating on the best approximation, here each value from the set of partial representations generates new values of the form $\left||x| - 2^a3^b\right|$ (being careful to record the sign of $x$), and the $k$ best partial representations are retained.  In this case, we iterate $b$ from $0 \le b \le B$ and let $a_1 = \floor{\log_2 (x/3^b)}$ and $a_2 = \ceil{\log_2 (x/3^b)}$ bounding both $a_1 \le A$ and $a_2 \le B$.  Note that using $a_2 = a_1 + 1$ is sufficient since either $\ceil{\log_2 (x/3^b)} = \floor{\log_2 (x/3^b)}$ or $\ceil{\log_2 (x/3^b)} = \floor{\log_2 (x/3^b)} + 1$.  We then use $\left||x|-2^{2_1}3^b\right|$ and $\left||x|-2^{a_2}3^b\right|$ as candidates for the new set. Finally, we generate representations for global bounds on the number of cubes and squares as before.
...
One reason to improve the performance of exponentiation in the ideal class group is to improve the performance of order finding in this group.  In particular, we exponentiate a random ideal class to a primorial so that the order of the resulting ideal class is likely to be coprime to our exponent.  Here, we first show our results for 16-bit exponents, and then compare the various methods useful for larger exponents by computing the average time to exponentiate using the first 2500 odd primorials.

In the following, we compute the average time to exponentiate ideals with 59-bit and 118-bit discriminants  for exponents 1 through 65535.  The graphs sample points where the timings differ. Figure \ref{fig:memoChains} show the average cost to exponentiate using the additive chains and $\pm 1$ chains from Subsection \ref{subsec:addChains}. For every exponent tested, $\pm 2, 3$ chains were on average as fast or faster than additive only chains, which is to be expected since the one function computes a subset of the representations of the other function.
\begin{center}
\begin{tabular}{| l | l |}
\hline
greedy $\pm 1$ & 1.07260 \\
